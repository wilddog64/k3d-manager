...
**Theme: security hotfix — clear the open Dependabot alerts on `main` without shipping the in-progress v1.33.0 milestone.** One squash subtree pull of lib-foundation `9c0af5b` into `scripts/lib/foundation/`; nothing outside the subtree changes. The vendored tree is byte-identical to lib-foundation `9c0af5b` (tree `490bc62c`).
...
### Added
- **Hermes monthly security audit** (`scripts/lib/hermes/audit.py`, `bin/k3dm-hermes audit`) — a dependency-injected, read-only audit that mirrors `preflight.py`. **Group A** (API-only): open CodeQL alerts by severity, open Dependabot alerts by severity, branch-protection posture (`enforce_admins`, required reviews, "main" ruleset), and a credential-expiry sweep. **Group B** (optional, gated by `K3DM_HERMES_AUDIT_RUN_BATS=1`): runs the F1/F3 security-regression subset of `scripts/tests/lib/webhook.bats` (filter `role|fix mode|Slack host`), TAP-parsed to pass/fail. Cadence is a once-per-calendar-month state gate (`state["last_security_audit_month"]`) inside the existing poll via `monthly_audit_advisory(...)` — no new launchd job — plus a manual `bin/k3dm-hermes audit` subcommand that prints the digest + JSON with no dedup write. Uses a dedicated read-only fine-grained `k3dm-hermes-audit-token` (Code scanning, Dependabot, Administration, Metadata — all Read); the audit is optional and degrades gracefully if the token is absent. `audit.py` stays pure (the bats subprocess runner lives in `bin/k3dm-hermes`). Guide: `docs/guides/hermes.md#monthly-security-audit`. Spec: `docs/plans/v1.32.0-hermes-monthly-security-audit.md`.
...
### Added
- **`bin/k3dm-hermes preflight`** (`scripts/lib/hermes/preflight.py`) — a dependency-injected credential + Actions-scope preflight. It confirms the webhook, ArgoCD, and GitHub secrets are all present in the Keychain, then probes the PAT against the live GitHub API for Actions **read** (list runs) and **write** (`rerun-failed-jobs` on a prior successful run), reporting a missing write scope by keying on `"resource not accessible"`. Exits non-zero on any missing secret or scope.
- **Hermes GitHub token-expiry advisory** (`scripts/lib/hermes/sensors.py`, wired in `bin/k3dm-hermes`) — a standalone once-per-day Slack advisory (intentionally **not** a correlator sensor — a lone signal would be swallowed by the correlator's `>=2`-degraded, edge-triggered gate) that warns when `k3dm-hermes-gh-token` is within `K3DM_HERMES_TOKEN_WARN_DAYS` (default 14) of the expiry reported by the `github-authentication-token-expiration` response header, deduped per day via `state`. A classic no-expiration PAT sends no such header, so the advisory stays silent — exactly the desired end state once the token is renewed.

### Removed
- **Dead R4 GitHub App installation-token auth** — the GitHub App path added earlier in this cycle could not obtain the required Actions permission on a personal repo, so it was dropped; Hermes R4 stays on the least-privilege `k3dm-hermes-gh-token` PAT (`actions:write` only). See `docs/plans/v1.31.0-hermes-token-expiry-advisory.md`.

## [1.30.0] - 2026-09-06

**Theme: Hermes stops only watching and starts — carefully — repairing.** Phase 1 gave Hermes read-only eyes; Phase 2 gives it a **closed, code-defined allowlist** of four repairs and a human-approved execution path. The poll cycle only **proposes** — it writes proposals to state and Slack and mutates nothing — until the operator runs `bin/k3dm-hermes approve <action-id>` at their own shell, which re-validates the precondition, executes exactly one allowlisted action, and re-samples afterward. There is no auto-execution path anywhere in the codebase (grep-proven invariant-1). Three of the four repairs need **no new privilege** — R1 restarts the webhook via a make target, R2 kicks a zombie port-forward via `launchctl`, R3 refreshes the Hostinger edge via a lib-foundation function — and only R4 (rerun a transient CI run) gains a remote scope: a fine-grained `actions:write` PAT that holds no `contents:write`, no branch-protection, and no PR/merge.

### Added
- **Hermes Phase-2 allowlisted, approval-gated repairs** (`scripts/lib/hermes/repairs.py`) — four repairs behind a structured-signal precondition: R1 restart-webhook, R2 kick zombie port-forward, R3 refresh Hostinger edge, R4 rerun transient CI. The poll cycle calls `propose()` only; execution is gated behind `bin/k3dm-hermes approve <action-id>` (with `list` to see pending proposals), which re-checks the precondition, runs a single action, and re-samples. Proposal `action_id` is a stable sha1 of key+command (no timestamp), so re-proposing the same repair is idempotent and `pending_repairs` stays bounded. Scope + invariants: `docs/architecture/hermes-phase2-repair-scope.md`.
- **R4 CI-rerun via a least-privilege `actions:write` PAT** — R4 reruns a failed workflow run through the real `gh` code path using the Hermes Keychain token (`k3dm-hermes-gh-token`), which carries only `actions:write`. `approve()` refuses (skips) rather than falling back to the laptop's ambient `gh` OAuth when the token is absent, and missing-scope detection keys on `"resource not accessible"` only — so a business-logic 403 (`"This workflow run cannot be retried"`) falls through to `failed` instead of being misreported as a missing scope.
...
### Added
- **Fleet node lifecycle (count-agnostic)** — the k3s-aws provider now derives agent hosts and total node count from `ACG_AGENT_COUNT` and fans the shopping-cart agent joins out in parallel with per-node readiness, collected failures, and idempotent skips over both SSH and SSM. The CloudFormation fleet is generated for N agents by the lib-foundation `v0.4.12` awk emitter (dynamic `Agent*PublicIP` discovery, validate-before-AWS), and `make fleet-render|fleet-validate|fleet-plan|fleet-up` provide an offline-to-live verification ladder. Live-verified at `ACG_AGENT_COUNT=4` (1 server + 4 agents = the 5-node ACG cap): all agents joined concurrently and every node reached `Ready`. See `docs/plans/v1.26.0-fleet-node-lifecycle.md`.
- **E2E promotion gate + observability** — an E2E run now writes a durable exit-code-faithful JSON artifact under `$E2E_REPORT_DIR` **and** an in-cluster result-event ConfigMap (`k3dm.k3d.io/e2e-result=true`); the `vulnerability-inventory-exporter` reads those events and emits `e2e_run_info`, `e2e_last_run_pass`, `e2e_last_run_timestamp_seconds`, `e2e_last_run_duration_seconds`, and `e2e_last_success_timestamp_seconds`, surfaced by a new E2E Grafana dashboard and `E2EVerificationFailing` / `E2EVerificationStale` alerts. Proven live end-to-end: a failed run was captured faithfully and drove `e2e_last_run_pass=0` → dashboard red → alert.
...
### Fixed
- **Stale-cluster cleanup no longer hangs** — `bin/cleanup-stale-clusters --confirm` now deletes the cluster registration Secret **first** (stopping ApplicationSet regeneration and letting ArgoCD fast-path finalizer removal), then force-removes any lingering generated Applications with a finalizer-null patch and non-blocking `kubectl delete --wait=false`. The previous order deleted Applications first against a dead cluster and blocked forever on the re-added `resources-finalizer`; a hub-only re-verify now completes in ~3s with zero orphans. Live-accepted on a real expired sandbox (23 unrelated survivors exact-match, hostinger untouched) — see `docs/bugs/2026-08-21-lifecycle-e2e-live-acceptance-findings.md`.
- **Fleet live-verification fixes** — `_k3s_agent_is_ready` now resolves each agent's private IPv4 over SSH and does an exact InternalIP-column match (`kubectl get nodes -o wide`) instead of matching the SSH alias/public IP, killing a false-negative that failed `fleet-up` on a healthy 5-node cluster; `make fleet-plan` was rewritten to `aws cloudformation create-change-set --change-set-type CREATE` against a throwaway plan stack with all required parameters and a cleanup trap — see `docs/bugs/2026-08-21-fleet-phaseb-live-verification-findings.md`.
- **js-yaml DoS advisory (CVE-2026-59870, dev-only transitive)** — closed via the lib-foundation `v0.4.11` subtree bump (`3.15.0`→`3.15.1`, within the existing `^3.13.1` range); the Dependabot alert clears once this branch reaches `main`.
