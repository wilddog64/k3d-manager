...
### Fixed
- `data-layer/postgresql/orders/configmap.yaml`: replace `SERIAL PRIMARY KEY` with `UUID DEFAULT gen_random_uuid()` — aligns init SQL with `Order`/`OrderItem` JPA entities using `@GeneratedValue(strategy = GenerationType.UUID)`; removes stale enum types, sample INSERTs, and functions referencing `orders_id_seq`
