...
### Changed
- `acg.sh` / `antigravity.sh`: exported `antigravity_acg_extend` in `antigravity.sh` was renamed/moved to `acg_extend_playwright` (public) / `_acg_extend_playwright` (impl) in `acg.sh` — `acg_watch` and `_acg_watch_write_wrapper` call it directly; `antigravity.sh` no longer exports the helper; removes false coupling between ACG watch loop and Antigravity IDE (`291a60dc`)
