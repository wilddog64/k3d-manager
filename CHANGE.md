...
- Hardened tracing for secret operations by streaming Vault payloads through temp files so `ENABLE_TRACE=1` no longer exposes base64-encoded credentials (`scripts/lib/system.sh`, `scripts/tests/lib/sync_lastpass_ad.bats`, `docs/plans/trace-hardening.md`).
