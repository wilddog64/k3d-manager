...
- **Browser automation**: `scripts/plugins/gemini.sh` — `_browser_launch`. Launches Chrome with `--remote-debugging-port=9222 --password-store=basic`. On Linux, `--no-sandbox` is added only when running as root (`$EUID -eq 0`) or `ANTIGRAVITY_CHROME_NO_SANDBOX=1`.
