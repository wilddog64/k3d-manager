...
- **⚠️ Pulled forward early (2026-08-07):** the `Makefile` `show-service-passwords` Grafana hunk from `40c8b08e` was cherry-picked onto **v1.23.0** (`31db9732`) because the rotation is already live and the target was showing the stale k8s-secret password. When cutting v1.24.0 for E, **skip the Makefile Grafana hunk** — it is already on main via v1.23.0. The rest of E (externalsecret, rotator CronJob, `vault.sh`/`observability.sh`, Slack notify) still lands in v1.24.0.
