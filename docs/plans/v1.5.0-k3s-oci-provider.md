...
**Token / credential storage rule:** OCIDs (compartment, region, AD) are non-secret identifiers — plaintext `env` file is fine. Any actual credentials (e.g. OCI session tokens from `oci session authenticate`, API key passphrases) must be stored in macOS Keychain:

```bash
# Store
security add-generic-password -s "k3d-manager-oci" -a "${USER}" -w "${TOKEN}"
# Retrieve
TOKEN=$(security find-generic-password -s "k3d-manager-oci" -a "${USER}" -w)
```
