...
**Token / credential storage rule:** OCIDs (compartment, region, AD) are non-secret identifiers — plaintext `env` file is fine. Any actual credentials (e.g. OCI session tokens, API key passphrases) must go through `_oci_secret_set` / `_oci_secret_get` — platform-aware helpers that abstract the credential store:

```bash
function _oci_secret_set() {
  local _key="${1}" _value="${2}"
  case "$(uname -s)" in
    Darwin)
      security add-generic-password -U -s "k3d-manager-oci" -a "${_key}" -w "${_value}"
      ;;
    Linux)
      if command -v secret-tool >/dev/null 2>&1; then
        printf '%s' "${_value}" | secret-tool store --label="k3d-manager-oci" service k3d-manager-oci key "${_key}"
      else
        # Fallback: chmod 600 file under state dir
        printf '%s\n' "${_value}" > "${_OCI_STATE_DIR}/secret-${_key}"
        chmod 600 "${_OCI_STATE_DIR}/secret-${_key}"
      fi
      ;;
  esac
}

function _oci_secret_get() {
  local _key="${1}"
  case "$(uname -s)" in
    Darwin)
      security find-generic-password -s "k3d-manager-oci" -a "${_key}" -w 2>/dev/null || true
      ;;
    Linux)
      if command -v secret-tool >/dev/null 2>&1; then
        secret-tool lookup service k3d-manager-oci key "${_key}" 2>/dev/null || true
      else
        [[ -f "${_OCI_STATE_DIR}/secret-${_key}" ]] && cat "${_OCI_STATE_DIR}/secret-${_key}" || true
      fi
      ;;
  esac
}
...
Never write tokens or passphrases to `~/.local/share/k3d-manager/oci/env`.
