...
Run once before `deploy_cluster`:

```bash
# 1. OCI CLI installed and configured
oci setup config   # creates ~/.oci/config with API key + tenancy/user OCIDs

# 2. SSH key pair for instance access (use existing or generate)
ssh-keygen -t ed25519 -f ~/.ssh/oci-k3s -C "k3d-manager-oci"

# 3. Required env vars (add to .envrc or shell profile)
export OCI_COMPARTMENT_ID="ocid1.compartment.oc1..xxxxxxxx"
export OCI_REGION="us-ashburn-1"
export OCI_AVAILABILITY_DOMAIN="qIZq:US-ASHBURN-AD-1"
export OCI_IMAGE_ID="ocid1.image.oc1.iad.xxxxxxxx"   # Ubuntu 22.04 ARM64 (region-specific)
export OCI_SSH_KEY_FILE="${HOME}/.ssh/oci-k3s"
```

**Finding `OCI_IMAGE_ID`:**
```bash
...
function _oci_validate_prereqs() {
  local _missing=()
  command -v oci >/dev/null 2>&1 || _missing+=("oci (OCI CLI)")
  [[ -f "${HOME}/.oci/config" ]] || _missing+=("~/.oci/config (run: oci setup config)")
  ...
  [[ -n "${OCI_IMAGE_ID:-}" ]] || _missing+=("OCI_IMAGE_ID")
  [[ -f "${_OCI_SSH_KEY}" ]] || _missing+=("SSH key: ${_OCI_SSH_KEY}")

  if [[ "${#_missing[@]}" -gt 0 ]]; then
    _err "[k3s-oci] Missing prerequisites: ${_missing[*]}"
    return 1
  fi
}
