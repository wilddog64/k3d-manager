...
**What requires a one-time user action (prompted interactively):**
- OCI CLI not installed → error with install instructions (`brew install oci-cli`)
- `~/.oci/config` missing → `oci setup config` is launched interactively (prompts for tenancy OCID, user OCID, region, API key path)
- `OCI_COMPARTMENT_ID`, `OCI_REGION`, `OCI_AVAILABILITY_DOMAIN` missing → single prompt listing all missing vars with example values; user adds them to `.envrc` then re-runs
...
function _oci_validate_prereqs() {
  ...
  # Required env vars — collect missing ones, prompt once
  local _missing=()
  ...
  if [[ "${#_missing[@]}" -gt 0 ]]; then
    _err "[k3s-oci] Missing required env vars: ${_missing[*]}"
    _err "Add to .envrc:"
    _err "  export OCI_COMPARTMENT_ID=\"ocid1.compartment.oc1..xxxxxxxx\""
    _err "  export OCI_REGION=\"us-ashburn-1\""
    _err "  export OCI_AVAILABILITY_DOMAIN=\"qIZq:US-ASHBURN-AD-1\""
    return 1
  fi

  # OCI_IMAGE_ID — resolve automatically if not set
  if [[ -z "${OCI_IMAGE_ID:-}" ]]; then
    _info "[k3s-oci] OCI_IMAGE_ID not set — resolving Ubuntu 22.04 ARM64 image for ${OCI_REGION}..."
    ...
    if [[ -z "${OCI_IMAGE_ID}" || "${OCI_IMAGE_ID}" == "null" ]]; then
      _err "[k3s-oci] Could not resolve OCI_IMAGE_ID automatically. Set it manually."
      return 1
    fi
    _info "[k3s-oci] Resolved OCI_IMAGE_ID=${OCI_IMAGE_ID}"
  fi
}
