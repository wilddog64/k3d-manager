# Codex Task: Smoke Test — Cluster Health Gate (v0.9.4)

## Before You Start

1. Read `memory-bank/activeContext.md` and `memory-bank/progress.md`
2. Run `git pull origin k3d-manager-v0.9.4`
3. Read `bin/smoke-test-jenkins.sh` lines 1–40 (style + header pattern)
4. Read `scripts/k3d-manager` `_test_run_smoke` function (lines 67–93)

---

## Problem

Gemini has no automated gate to verify baseline cluster health before reporting a task
done. Missing `ghcr-pull-secret`, Unknown ArgoCD sync status, or crashlooping pods have
all caused false "done" reports. This script gives Gemini a single command to run at the
end of every cluster task.

---

## What to Build

One new file. No existing files modified.

---

### File — `bin/smoke-test-cluster-health.sh` (new)
...
- Run `shellcheck bin/smoke-test-cluster-health.sh` — zero warnings
...
- [ ] `bin/smoke-test-cluster-health.sh` created and executable
- [ ] `shellcheck bin/smoke-test-cluster-health.sh` — zero warnings
- [ ] Commit on `k3d-manager-v0.9.4` with message:
  `feat(bin): add smoke-test-cluster-health.sh — baseline health gate for cluster tasks`
