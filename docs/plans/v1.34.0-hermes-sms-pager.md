...
## Follow-up F1 — no duplicate app password: use the existing Alertmanager Keychain item

**Status:** added 2026-09-15 after verification of `a18450d4`.

The Google App Password already lives in the login Keychain as `k3dm-alertmanager-gmail-app-password` (account = login user; backup for `make restore-google-app-password`, and checked MATCH against Vault and the live Alertmanager Secret). A second item `k3dm-hermes-sms-app-password` would duplicate a credential and let the copies drift on rotation. Operator decision: do not duplicate tokens.

- `scripts/lib/hermes/pager.py`: `SMS_PASSWORD_SERVICE = "k3dm-alertmanager-gmail-app-password"`.
- `bin/k3dm-hermes`: `_sms_keychain(service)` — for `SMS_PASSWORD_SERVICE` runs `security find-generic-password -s <service> -w` without `-a` (value from stdout only, `""` on failure); every other service goes through `_keychain_secret` (account `k3dm`). `_page` passes `_sms_keychain` to `send_sms`.
- Tests: the password read carries only the service name in argv and never uses account `k3dm`; sender/recipient use `_keychain_secret`; failure returns `""`.
- `docs/guides/hermes.md` and `CHANGELOG.md`: name the shared item; drop `k3dm-hermes-sms-app-password`.
- Operator provisions only `k3dm-hermes-sms-from` and `k3dm-hermes-sms-to`.
