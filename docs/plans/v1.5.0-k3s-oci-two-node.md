# v1.5.0 — k3s-oci two-node cluster (server + agent)
...
from `10.0.0.0/16` — no networking changes needed.
...
### 1. Variables — fix defaults + add agent name
...
Provision a two-node k3s cluster on OCI Always Free (ARM64, 2×2OCPU/12GB):
  ...
  3. Wait for server SSH reachable + install k3s server
  4. Wait for agent SSH reachable + install k3s agent (joins server)
  5. Fetch kubeconfig → merge into ~/.kube/k3s-oci.yaml
  6. Register cluster secret with hub ArgoCD (environment: prod)
  7. Wait for platform-helm to deploy ArgoCD on OCI
  8. Bootstrap OCI ArgoCD with app-of-apps for stable stack
  9. Smoke test: 2 nodes Ready + expected namespaces + key pods Running
```
...
```bash
  ...
  _info "[k3s-oci] Step 1/9 — Provision OCI infrastructure (idempotent)..."
  _oci_provision_infrastructure || return 1
  ...
  _info "[k3s-oci] Step 2/9 — Waiting for server SSH reachable (up to 5 min)..."
  _oci_wait_ssh "${_server_ip}" || return 1

  _info "[k3s-oci] Step 3/9 — Installing k3s server ${_OCI_K3S_VERSION}..."
  _oci_install_k3s_server "${_server_ip}" || return 1

  _info "[k3s-oci] Step 4/9 — Waiting for agent SSH reachable (up to 5 min)..."
  _oci_wait_ssh "${_agent_ip}" || return 1

  _info "[k3s-oci] Step 5/9 — Installing k3s agent (joining server)..."
  _oci_install_k3s_agent "${_agent_ip}" "${_server_ip}" || return 1

  _info "[k3s-oci] Step 6/9 — Fetching kubeconfig → ${_OCI_KUBECONFIG}..."
  _oci_fetch_kubeconfig "${_server_ip}" || return 1

  _info "[k3s-oci] Step 7/9 — Registering cluster with hub ArgoCD (environment: prod)..."
  _oci_register_cluster "${_server_ip}" || return 1

  _info "[k3s-oci] Step 8/9 — Waiting for platform-helm to deploy ArgoCD on OCI (up to 10 min)..."
  _oci_wait_argocd || return 1

  _info "[k3s-oci] Step 9/9 — Bootstrapping OCI ArgoCD with stable app-of-apps..."
  _oci_bootstrap_argocd || return 1

  _info "[k3s-oci] Step 9/9 — Smoke test..."
  _oci_smoke_test || return 1

  _info "[k3s-oci] Cluster ready — 2 nodes."
  _info "[k3s-oci] Kubeconfig: KUBECONFIG=${_OCI_KUBECONFIG}"
  _info "[k3s-oci] Verify: kubectl --kubeconfig=${_OCI_KUBECONFIG} get nodes"
```
...
### 6. Rename `_oci_install_k3s` → `_oci_install_k3s_server` and add `_oci_install_k3s_agent`

**Old function signature and display name:**
```bash
...
function _oci_install_k3s_server() {
  ...
  _info "[k3s-oci] Installing k3s server ${_OCI_K3S_VERSION} (ARM64)..."
```

Close of the server function is unchanged — only the function name and the log message change.
...
### 7. `_oci_smoke_test` — check 2 nodes Ready
...
- [ ] `_oci_install_k3s` renamed to `_oci_install_k3s_server`
- [ ] `_oci_install_k3s_agent` added — fetches node token, joins via private IP
- [ ] Deploy steps renumbered 8→9; agent install added as Step 4/9 + Step 5/9
- [ ] `_oci_smoke_test` checks 2 nodes Ready
...
- [ ] Commit message: `feat(k3s-oci): two-node cluster — server + agent (2×2OCPU/12GB)`
- [ ] `memory-bank/activeContext.md` and `memory-bank/progress.md` updated with commit SHA
- [ ] Report back: commit SHA + paste the memory-bank lines you updated

## What NOT to Do

- Do NOT modify any file outside `scripts/lib/providers/k3s-oci.sh`
