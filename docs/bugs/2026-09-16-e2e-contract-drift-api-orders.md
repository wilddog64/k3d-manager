# Bug: e2e contract-drift — api-orders
...
## Failing tests (2)
- `api/orders.spec.ts` — should update order status to CONFIRMED
- `api/orders.spec.ts` — should track status history

## Sample errors
- Error: expect(received).toBe(expected) // Object.is equality / Expected: "CONFIRMED" / Received: undefined
- Error: expect(received).toBe(expected) // Object.is equality / Expected: "SHIPPED" / Received: undefined
