# Bug Fix: implement GCP cluster provisioning in k3s-gcp provider
...
2. Read `scripts/lib/providers/k3s-gcp.sh` in full (current skeleton — 19 lines)
3. Read `scripts/lib/providers/k3s-aws.sh` in full — understand the deploy/destroy pattern
4. Read `scripts/plugins/shopping_cart.sh` lines 100–125 (`_ensure_k3sup`)
5. Confirm prerequisites on the live sandbox:
   - `echo $GCP_PROJECT` — must be set
   - `test -f ~/.ssh/k3d-manager-gcp-key && echo OK`
   - `gcloud auth list` — cloud_user must be ACTIVE

---

## Problem

`_provider_k3s_gcp_deploy_cluster` is a no-op placeholder. `CLUSTER_PROVIDER=k3s-gcp make up`
exits immediately after the credential step with no cluster created.

---

## Sandbox Topology (verified 2026-04-20)

- **Project:** `$GCP_PROJECT` (set by `gcp_get_credentials`)
- **Zone:** `us-west1-a` (GCP equivalent of AWS `us-west-2`); configurable via `GCP_ZONE`
- **SSH key:** `~/.ssh/k3d-manager-gcp-key` (private), `~/.ssh/k3d-manager-gcp-key.pub` (public)
- **SSH user:** `ubuntu`
...
- **Local kubeconfig:** `~/.kube/k3s-gcp.yaml`

---

## Fix

Replace the entire content of `scripts/lib/providers/k3s-gcp.sh` with the following
**exact content:**
...
_GCP_IMAGE_FAMILY="ubuntu-2004-lts"
...
| File | Change |
|------|--------|
| `scripts/lib/providers/k3s-gcp.sh` | Full replacement — implement deploy + destroy |
...
- Do NOT modify `scripts/plugins/gcp.sh`, `scripts/plugins/shopping_cart.sh`, or `bin/acg-up`
...
### Test G3 — GCP_PROJECT guard

```bash
source scripts/lib/system.sh && source scripts/lib/core.sh
_info(){ :; }
source scripts/lib/providers/k3s-gcp.sh
GCP_PROJECT="" _provider_k3s_gcp_deploy_cluster 2>&1 | grep -q "GCP_PROJECT is not set" && echo "guard OK"
```
Expected: `guard OK`.

### Test G4 — destroy requires --confirm

```bash
source scripts/lib/system.sh && source scripts/lib/core.sh
_info(){ :; }
source scripts/lib/providers/k3s-gcp.sh
GCP_PROJECT="test" _provider_k3s_gcp_destroy_cluster 2>&1 | grep -q "requires --confirm" && echo "guard OK"
```
Expected: `guard OK`.

### Test G5 — live E2E (run with active GCP sandbox)

```bash
CLUSTER_PROVIDER=k3s-gcp make up
kubectl --context ubuntu-gcp get nodes
```
Expected: one node shows `Ready`.

---

## Definition of Done

- [ ] `scripts/lib/providers/k3s-gcp.sh` replaced with full implementation above
- [ ] Tests G1–G4 all pass — paste actual outputs
- [ ] `shellcheck` passes with zero warnings
- [ ] Live E2E (Test G5) — `kubectl --context ubuntu-gcp get nodes` shows Ready — paste output
- [ ] Committed and pushed to `k3d-manager-v1.1.0`
- [ ] `memory-bank/activeContext.md` and `memory-bank/progress.md` updated with commit SHA

**Commit message (exact):**
```
feat(gcp): implement GCP cluster provisioning — firewall, GCE instance, k3sup, kubeconfig
```
...
- Do NOT run destroy_cluster unless explicitly asked
