...
- `bats scripts/tests/lib/observability.bats scripts/tests/plugins/trivy_operator_observability.bats scripts/tests/plugins/observability_no_exit_remove.bats`
...
- `bats scripts/tests/plugins/argocd_metrics_servicemonitor.bats` — **KNOWN PRE-EXISTING
  FAILURE, NOT YOURS.** Tests 12 and 13 fail on this branch already, caused by `4c89dabb`
  (the trivy split moved those panels off the hub dashboard; this suite's assertions were
  never retargeted). Verified 2026-07-18: `15/15` pass at `8714721b`, `13/15` at `3fa6e5ef`.
  Expected result for this task is **exactly `13/15`, failing ONLY tests 12 and 13.**
  - If any OTHER test in this suite fails, that IS yours — revert it.
  - Do NOT fix tests 12/13. Do NOT edit this file. Tracked separately in
    `docs/bugs/2026-07-18-argocd-dashboard-bats-regression.md`.
...
- [ ] Three pre-existing observability BATS suites still pass — zero edits to them
- [ ] `argocd_metrics_servicemonitor.bats` returns exactly `13/15`, failing ONLY 12 and 13
      (known pre-existing regression from `4c89dabb` — not this task's to fix)
