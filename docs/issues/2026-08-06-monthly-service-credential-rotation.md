...
On successful LDAP or Grafana rotation, the job posts a redacted Slack message using the existing `platform-ops-notifications` webhook. The message names the service and states that rotation completed; it never includes a password or Vault token.
