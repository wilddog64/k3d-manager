...
@test "_ensure_lpass succeeds when command already exists" {
  _command_exist() {
    if [[ "$1" == "lpass" ]]; then
      return 0
    fi
...
@test "_ensure_lpass installs via apt when missing" {
  LPASS_PRESENT=0
  _command_exist() {
    case "$1" in
      lpass) (( LPASS_PRESENT )) && return 0 || return 1 ;;
  ...
  _run_command() {
    ...
    local cmd="$*"
    echo "$cmd" >>"$RUN_LOG"
    if [[ "$cmd" == "env DEBIAN_FRONTEND=noninteractive apt-get install -y lastpass-cli" ]]; then
      LPASS_PRESENT=1
    fi
    return 0
  }
  ...
  grep -q 'apt-get install -y lastpass-cli' "$RUN_LOG"
}

@test "_ensure_lpass warns when platform unsupported" {
  ...
  run _ensure_lpass
  [ "$status" -eq 1 ]
  [[ "$output" == *"Unsupported platform for automatic LastPass CLI installation."* ]]
}
