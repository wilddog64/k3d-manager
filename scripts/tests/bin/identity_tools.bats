...
@test "vault-exec wraps kubectl exec through _run_command" {
  run grep -nF '_identity_exec_pod "$VAULT_NAMESPACE" "$VAULT_POD" "$VAULT_CONTAINER" "${CMD[@]}"' bin/vault-exec.sh
  [ "$status" -eq 0 ]
...
@test "ldap-search reads the bind password from the live secret" {
  run grep -nF '_identity_secret_field "$LDAP_NAMESPACE" "$LDAP_ADMIN_SECRET_NAME" "$LDAP_ADMIN_PASSWORD_KEY"' bin/ldap-search.sh
  [ "$status" -eq 0 ]
  [[ "$output" == *'_identity_secret_field'* ]]

  run grep -nF 'tmp_pw="$(mktemp)"' bin/ldap-search.sh
  [ "$status" -eq 0 ]
  [[ "$output" == *'mktemp'* ]]

  run grep -nF 'printf '\''%s\n'\'' "$LDAP_BIND_PASSWORD" | _run_command --quiet -- kubectl exec -i' bin/ldap-search.sh
  [ "$status" -eq 0 ]
  [[ "$output" == *'kubectl exec -i'* ]]
}

@test "identity log helpers use kubectl logs through _run_command" {
  run grep -nF '_identity_logs_pod "$KEYCLOAK_NAMESPACE" "$KEYCLOAK_POD" "${LOG_ARGS[@]}"' bin/keycloak-logs.sh
  [ "$status" -eq 0 ]
  [[ "$output" == *'_identity_logs_pod'* ]]

  run grep -nF '_identity_logs_pod "$LDAP_NAMESPACE" "$LDAP_POD" "${LOG_ARGS[@]}"' bin/ldap-logs.sh
  [ "$status" -eq 0 ]
  [[ "$output" == *'_identity_logs_pod'* ]]
