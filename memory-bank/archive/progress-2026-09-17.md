...
- [x] **E2E result label clarification 2026-09-17.** Renamed the boolean run field to `E2E run passed`.
- [x] **E2E failure ratio 2026-09-17.** Added and displayed the failed/total ratio in Recent runs.
...
- [x] **E2E Grafana trends/drill-down spec filed 2026-09-16.** Fifth and final v1.34.0 plan slot: `docs/plans/v1.34.0-e2e-grafana-trends-and-drilldown.md`; **the milestone plan cap is now reached — any further spec opens v1.35.0.** Implemented and verified live (see the following entries); the plan doc is marked IMPLEMENTED. Only the HTTP/2 failure-rate panel remains deferred.
...
- [x] **Live dashboard verification** — ArgoCD hard-refresh advanced `hub-grafana-dashboards` to `387f019e`; the ConfigMap now has trend panels at y=14/22 and the owning-service-only table transformations. Grafana rollout completed successfully. Stale self-heal behavior is recorded in `docs/issues/2026-09-16-e2e-grafana-dashboard-stale-layout.md`.
...
- [x] **Service credential display investigation** — `make show-service-passwords` returned `N/A` during a transient Vault port-forward outage; the LaunchAgent was restored/running and a rerun resolved the values. Documented recovery and the remaining diagnostic-improvement follow-up in `docs/issues/2026-09-16-show-service-passwords-vault-forward.md`.
- [x] **Service recovery hardening** — `show-service-passwords` now self-heals the local Vault port-forward (health probe, installer restart, bounded retry); `restart-webhook` uses `launchctl kickstart -k` and avoids the bootout/bootstrap race. Live restart and credential-display checks passed; webhook bootstrap error 5 is documented in `docs/issues/2026-09-16-restart-webhook-bootstrap-io-error.md`.
- [x] **Authenticated credential preflight** — `show-service-passwords` now verifies `vault-root` plus an authenticated KV lookup before rendering any rows, so token/context failures no longer degrade silently to all `N/A`; it restarts/retries the port-forward only when the lookup is unavailable.
- [x] **Webhook/status recovery** — added bounded `quick=1` webhook liveness and changed status summary to use it; live status now reports service results instead of UNKNOWN when the full health sweep is slow. Prometheus unauthenticated readiness is a warning, not an outage; full authenticated checks remain separate.
- [x] **Webhook token rotation resilience** — status retries the Keychain token when `K3DM_WEBHOOK_TOKEN` is stale/rejected. Verified with an intentionally invalid environment token; status still returns service results.
- [x] **Empty webhook token recovery** — setup now regenerates an existing-but-empty Keychain token record instead of treating it as valid. Public Grafana availability is documented as independent from local webhook authentication.
- [x] **Webhook token rotation recovery** — corrected the obsolete Cloudflare Keychain service name; `make rotate-webhook-token` now synchronizes Worker + Keychain and verifies the route. Live `make status` returns WARN/rc 0 and `show-service-passwords` resolves all fields.
- [x] **User-session Keychain authorization** — resolved by unlocking the login Keychain; token rotation was unnecessary. Status now returns service results with only the expected Prometheus HTTP 401 warning. No plaintext fallback added.
- [x] **Keychain failure diagnostics** — status and webhook setup now provide actionable unlock/token guidance without printing secrets.
- [x] **E2E run identity labels (`58376441`, pushed)** — Recent runs now distinguishes runner/orchestration commit from application image digest and test suite, removing ambiguity around the former `Commit`/`Project` labels. The Grafana plan includes an operator-facing field-by-field interpretation and M2 checkout semantics.
- [x] **E2E chart owning-service labels (`f0115c90`)** — Failure trend/cause/spec charts use `exported_service`; the raw `service` label is the Prometheus scrape target (`vulnerability-inventory-exporter`).
- [x] **E2E Recent runs owning-service list (`2830b32c`)** — Added the derived `Services under test` label to replace the misleading consolidated service value in the Recent runs table. Platform-ops was reapplied and Grafana restarted.
- [x] **E2E Recent runs column order (`b0ccb760`)** — Moved Run ID and Runner to the leftmost columns; platform-ops was reapplied and Grafana successfully rolled out.
- [x] **E2E failure table column order (`0f877444`)** — Moved Run ID and Runner to the leftmost columns in Failure groups and Failure details; live ConfigMap verification confirms the ordering.
- [x] **Hermes findings visibility (`60a04c19`, label fix pending)** — Added redacted status publication, exporter metrics, and the dedicated Hermes Status Grafana dashboard. The feed also publishes regular sensors while optional `status_checks` is disabled. Live snapshot was relabeled after the first publication exposed a label-application gap. Pytest Hermes 106 passed; observability BATS 15/15; shellcheck and `_agent_audit` clean.
...
- [x] **E2E failure-details live backfill 2026-09-16.** Published the retained redacted sidecar as `e2e-result-m2-detail-1789549631`; live exporter verified **33** `e2e_failure_info` series. Concrete details now support service/test fixes rather than aggregate triage only.
- [x] **E2E owning-service labels and table cleanup 2026-09-16 (`dece8c63`).** Groups/details now identify basket/order/payment/product-catalog/cross-service; Grafana hides container/endpoint and related scrape metadata. Platform-ops deployed; BATS 114/114, shellcheck clean, `_agent_audit` exit 0.
- [x] **Live Grafana panel reload 2026-09-16.** Directly applied `grafana-dashboard-e2e.yaml` after the platform-ops path left the old ConfigMap; verified live exclusions for container/endpoint/namespace/pod in both failure tables.
- [x] **Removed E2E Project column 2026-09-16 (`2d79b2ba`).** Failure groups/details now hide `project` (`api+flows`); live dashboard ConfigMap applied. Recent runs retains the project scope for run-level context.
- [x] **Grafana reload for Project-column removal 2026-09-16.** Reapplied the dashboard ConfigMap and restarted `kube-prometheus-stack-grafana`; live panels verified with `project: true` exclusions.
- [x] **Cross-service label fallback fix 2026-09-16 (`c691eb92`, remote tip `d83f1940`).** Explicitly maps `cross-service.spec.ts` to `cross-service`; live exporter verified distribution basket 11 / cross-service 10 / payment 9 / order 2 / product-catalog 1.

- [~] **Live SSO/E2E recovery 2026-09-16.** Corrected live ArgoCD canonical URL to `https://argocd.3ai-talk.org` and verified HTTP 303 callback to Keycloak; verified frontend PKCE returns a login form. Configured M2 publish-back and replayed all three retained runs; exporter now exposes their `e2e_run_info` and failed-run metrics. k3d-manager URL changes are uncommitted pending tests/commit. Shopping-cart-infra durable no-`awk` hook branch `fix/keycloak-hook-runtime-tools` is pushed through `cb6e414`; live hook still fails with a flow-execution Resource-not-found/BackoffLimitExceeded after the flow itself was repaired. See `docs/issues/2026-09-16-live-sso-e2e-recovery.md`; merge and successful PostSync rerun remain pending.

- [~] **2026-09-16 Hermes scheduled status continuation — code verified and PUSHED `e3e37f9671b9de4a14a87176e494de3a578642fa`; live acceptance pending.** Inherited implementation plus review fixes: suppress duplicate unknown-status SMS, redact complete status records, file truthful status bug reports, and remind at local midnight. Schedule defaults off pending Prometheus authenticated-probe verification; explicit opt-in remains available. Hermes pytest **106 passed in 2.90s**, Python compile exit 0, status-summary BATS **8/8**, `_agent_audit` exit 0 and `_agent_lint` exit 0 (AI gate disabled); checkpoint safely skipped on clean tree. Full 919-test run stopped at unrelated test 3's unbounded `kubectl get nodes --no-headers` (exit 143); follow-up recorded with exact output in `docs/issues/2026-09-16-hermes-status-triage-review.md`. PR none. This supersedes the earlier “assigned” entry below.

- [ ] **Hermes scheduled `make status` sensor + triage** — spec `docs/plans/v1.34.0-hermes-scheduled-status-triage.md` (2026-09-16, **assigned to Codex** via `codex exec` background dispatch). Runs `bin/cluster-status --json` on a ~40 min gate inside the existing 300s Hermes poll, classifies reds by check id, notifies on state change only, files bugs via the e2e worktree path. No new launchd agent, no new REPAIRS entry, no auto-execution (proposal + Slack approval only). Implementation is stub-only so it is not blocked; *enabling* the schedule live is blocked on fixing the false Prometheus 401 red first.
- [x] **Keycloak browser-flow regression (all shopping-cart SSO)** — spec `docs/bugs/2026-09-15-keycloak-browser-flow-regression-blocks-all-sso.md`; fix `c3a2c37` + Copilot follow-up `78403f0` on shopping-cart-infra `fix/keycloak-browser-flow-top-level-otp`; **PR #98 MERGED 2026-09-16 as `af4b053d`** on shopping-cart-infra. CI green on `78403f0` (Kubeconform / Kustomize Build / YAML Lint / GitGuardian). Copilot reviewed with verdict 'Needs a closer look' and filed 1 inline finding (the level-0 guard conflated a wrong execution count with unexpected execution names, printing an empty name list) — fixed in `78403f0`, replied, thread resolved. `enforce_admins` on shopping-cart-infra main RESTORED to true. Remaining: ArgoCD sync of `identity/keycloak/` and the PostSync reconcile hook run (live verification, not yet confirmed; in progress).
- [x] **Hermes scheduled e2e (Wed+Sat 02:00, rule triage, docs/bugs filing)** — plan #3 `docs/plans/v1.34.0-hermes-scheduled-e2e.md`; Codex impl, Claude-verified + committed `3ba1b8af`. Claude fixes on review: `pid_alive(0)` returned True, so a `retry_claim` (pid 0) blocked its slot forever; sorted the dedup glob; added the git-level `file_bugs` tests Codex skipped (created/ongoing/REOPENED/non-release-branch refusal/worktree isolation vs operator checkout) + claim pid tests. Gates: pytest scripts/tests/hermes 94 passed, bats 100/100. Live acceptance pending (next Wed/Sat 02:00, or `bin/k3dm-hermes e2e now`).
...
- [x] **e2e substrate missing payment** — `docs/bugs/2026-09-15-e2e-substrate-missing-payment.md`; Codex impl, Claude-verified + committed `cb2982b1` (payment Deployment/Service 8084, `payments` DB, mock gateway, generated encryption key, image `sha-a672ee42`, `PAYMENT_URL`; RabbitMQ not needed — no AMQP usage in payment main). Gates: shellcheck clean, bats e2e+e2e_remote 95/95, offline kustomize renders payment. Live acceptance pending (next `make e2e-remote RUNNER=m2`: no `:8084` failures).
- [x] **e2e remote results never reach Grafana** — `docs/bugs/2026-09-15-e2e-remote-results-never-reach-grafana.md`; Codex impl, Claude-verified + committed `45ce5428` (B1 conf-file publish-back host, B2 failures sidecar <=200, B3 copy-back). Claude extracted `_e2e_remote_copy_results` because `_agent_audit` flagged the if-count in `e2e_runner_dispatch`. Gates: shellcheck clean, bats 100/100. OPERATOR STEP: `printf 'E2E_M2_PUBLISH_BACK_HOST=cliang@m4-air.local\n' > ~/.config/k3d-manager/e2e-remote.env` then `make e2e-replay RUNNER=m2` (publishes the retained 08-27 + 09-15 results).
- [x] **make status login checks not operator accounts** — `docs/bugs/2026-09-15-status-login-checks-not-operator-accounts.md`; Codex impl, Claude-verified + committed `384b0202` (Keycloak admin-cli/master, frontend SSO code-flow for admin/developer/operator, ArgoCD Vault-only + new SSO line, Prometheus/Alertmanager basic auth with unauth-200 = red, synthetic checks renamed). Frontend realm `shopping-cart`, client `frontend`, redirect `https://frontend.3ai-talk.org/callback` (from `shopping-cart-frontend` `src/config/auth.ts` + `.env.example`), overridable via `K3DM_SMOKE_FE_REALM/_CLIENT/_REDIRECT`. Claude fix on review: Codex dropped the monitoring-paused softening for Grafana; restored + tested. Gates: pytest bin+hermes 106 passed, bats 100/100. EXPECT RED LINES on next `make status` — that is the point; fixing each underlying login is follow-up work.
...
- [x] Operator: `make observability` on hub — DONE 2026-09-14 (see v1.34.0 operator line below).
...
- [x] ApplicationSet reapply + `argocd_check_values_branch` — VERIFIED 2026-09-13 (read-only, NO apply needed): all 6 values refs on k3d-manager-v1.33.0; every live AppSet git revision = k3d-manager-v1.33.0; `kubectl diff` of all 12 rendered sets vs live = 0 changes when rendered with live values (istio-ambient APP_CLUSTER_NAME=ubuntu-k3s + CNI /var/lib/rancher/k3s/agent/etc/cni/net.d,/bin; observability-acg APP_CLUSTER_NAME=ubuntu-hostinger). DO NOT run blanket `deploy_argocd_applicationsets --confirm`: one APP_CLUSTER_NAME for all sets would rename istio-*-ubuntu-k3s apps (preserveResourcesOnDeletion rename trap) and vars.sh CNI defaults would replace the working dirs. v1.34.0 follow-up spec: per-set APP_CLUSTER_NAME + reuse istio_ambient CNI resolution.
- [x] Hermes↔Slack approval (Option A) — IMPLEMENTED `236389c3` (Codex; Claude verified: 10 spec files only, diff = §13, node --test 14/14, pytest hermes 68/68, shellcheck clean, repairs.py untouched; Claude pushed). Decisions resolved 2026-09-13 (user): all 5 code slices in v1.33.0, off until `K3DM_HERMES_APPROVAL_DRAIN_URL` set; worker drain endpoint; next-poll latency; Deny = dismiss only. Impl spec §13 of `docs/plans/v1.33.0-hermes-slack-approval.md` (`0a4eba22`). DISPATCHED to Codex 2026-09-13. Live KV/secrets/Slack interactivity + E2E = operator steps.
- [x] v1.34.0 follow-up: BATS bare `!` negations never fail mid-test — SPEC'D 2026-09-14 `docs/bugs/2026-09-14-bats-bare-negation-assertions-never-fail.md` (42 lines/14 files, 13 mid-test, signing.bats:124 guard can never fail; + lint test). Codex queue #3. DONE `81048cea` (+ signing.sh env://COSIGN_KEY fix).
- [x] v1.34.0 Codex chain 2026-09-14 — #1 `2f2474e9` acg lock dir; #2 `3eeaef5b` hub ESO status; #3 already `b71e587a`; #4 `397a7fcc` AppSet live overrides; #5 `31e69c56` Prometheus 19190; #6 `42c8723d` hub ServiceMonitors/promtail; #7 `6003a486` argocd-cve-scan kubectl/wget. All Claude-verified + pushed.
- [x] v1.34.0 operator (Claude ran, user go 2026-09-14): `make restart-webhook` OK (running); `make observability` first FAILED (unauthenticated Vault seed check) → fixed `b5505368` (`docs/bugs/2026-09-14-observability-grafana-seed-unauthenticated-vault-check.md`), rerun exit 0, Grafana cred kept, PrometheusRules + Istio VS applied, live federate-acg + acg-prometheus datasource = 19190; `make platform-ops` exit 0 + manual `argocd-cve-scan-manual` Complete but NO scan (artifacthub `appVersion` vs `app_version`; Docker Hub `argoproj/argocd` 404) → fixed `37118aab` (`docs/bugs/2026-09-14-argocd-cve-scan-wrong-app-version-and-image.md`, BATS 22/22), pushed; `deploy_argocd_applicationsets --confirm` → all 12 sets on k3d-manager-v1.34.0 (4 were stuck on v1.33.0), live destinations kept, values-branch check 6/6; hostinger apps Synced/Healthy.
- [x] (CLOSED 2026-09-15 — superseded: platform-ops redeployed + scanner live-verified via `96a4a326`/`4ca63233`/`8cea94a5`; SMS verified) v1.34.0 operator step: `make platform-ops` (deploy fixed scanner) + rerun `argocd-cve-scan-manual` — CAUTION: real CVE hit + newer chart relabels `cluster-ubuntu-hostinger` (environment=dev) argocd-chart-version 7.8.1 → 10.9.1. `make alertmanager-secret` DONE by user 2026-09-14; observability rerun created `alertmanager-smtp-secret` — Alertmanager configSecret path FIXED `de83d01f` (user go: fix + mute TrivyCriticalVulnerabilityDetected → null); Argo synced (CR configSecret set) BUT operator rejects secret `missing to address in email config`: user's `! make alertmanager-secret` had no stdin → empty creds stored in Vault. Guards FIXED `5de8cdf8` (spec `docs/bugs/2026-09-14-alertmanager-secret-accepts-empty-credentials.md`). PENDING USER: rerun `make alertmanager-secret` in a real terminal (NOT `!`), then Claude reruns `make observability` + verifies generated config. Scanner redeploy still HOLD (user did not decide).
- [x] **2026-09-14 — argocd-cve-scan major-upgrade guard + redeploy** — `96a4a326`; + `HOME=/tmp` `4ca63233`; live-verified: real scan, major upgrade held, hostinger 7.8.1
- [x] **2026-09-14 — argocd-cve-scan misleading "upgrade complete" notify** — `8cea94a5`; BATS 8/8; live-verified 2026-09-15 00:03Z
- [x] **2026-09-15 — Alertmanager sms-critical SMS end-to-end VERIFIED** — Gmail App Password fix; text received by user; Keychain backup `k3dm-alertmanager-gmail-app-password` stored; matches Vault + Alertmanager Secret (user-run compare)
- [x] **2026-09-14 — Alertmanager SMS live-verified** — configSecret live, sms-critical loaded, Trivy→null route first, operator errors 0 after secret update
...
- [x] app-cve-scan daily 01:00 LA + rebuild cooldown — `40a91f0e`, LIVE on hub 2026-09-13; argocd-cve-scan daily 01:30 LA — `2a226814`, LIVE on hub
- [x] (CLOSED 2026-09-15 — all follow-ups done: ServiceMonitors/promtail `42c8723d`; `make observability` ran; hub `ubuntu-k3s-app-cluster` in-cluster registration is intentional and its argocd-* ownership residue was reclaimed by `198584eb` (helm rev 6). Live re-check 2026-09-15 read-only: cicd cm/secrets with argocd-controller managedFields or instance label = 0; `argocd-dex-server` SA absent; no `ubuntu-k3s-platform` app; in-cluster secret has no `environment` label) Hub observability gaps (2026-09-13): LIVE stopgap DONE — ArgoCD ServiceMonitors created (4 targets up; helm rev 4 FAILED on SSA label conflict with argocd-controller, rev 3 still deployed), promtail applied (Loki receiving). Spec `docs/bugs/2026-09-13-hub-rebuild-skips-argocd-servicemonitors-and-promtail.md` OPEN (not assigned). Open follow-ups: `make observability` (PrometheusRule/Istio VS/alertmanager-smtp-secret missing); hub self-registered as `ubuntu-k3s-app-cluster` (kubernetes.default.svc) rewrote hub argocd-cm labels via argocd-7.8.1 on 09-13 — needs own spec
- [x] Hub ArgoCD foreign chart ownership (`docs/bugs/2026-09-13-hub-argocd-foreign-chart-ownership-blocks-helm-upgrade.md`) — Codex `198584eb` VERIFIED by Claude (shellcheck 0, BATS 38/38, 4 spec files; Claude pushed — Codex sandbox SSH push denied); LIVE DONE 2026-09-13 by operator: `--confirm` (9 stripped, dex SA deleted) → helm rev 6 deployed (rev 4/5 failed pre-reclaim); re-dry-run = no foreign ownership; all cicd workloads 1/1; 4 ServiceMonitors
- [x] Hub federate-acg self-scrape duplicates (`docs/bugs/2026-09-13-hub-federate-acg-self-scrape-duplicates-series.md`) — spec'd 2026-09-13 (+S2b `b4ae72e9`: existing `argocd_metrics_servicemonitor.bats` pinned old exprs — Codex correctly stopped), Codex `22feff51` VERIFIED by Claude (yq ok, BATS 27/27, 5 spec files; Claude pushed); dashboard applied live 2026-09-13; LIVE DONE 2026-09-14: user ran scoped `observability` AppSet reapply → hub kube-prometheus-stack/hub-loki/trivy-operator on v1.34.0, synced; Prometheus reloaded with drop rule 02:48Z; `count({cluster="acg"})` 82208 → 6 (only federate-acg synthetic scrape-health series: up/scrape_*), image-updater replica series 2 → 1, firing acg alerts 0. `observability-acg` AppSet reapplied by user 2026-09-14 (APP_CLUSTER_NAME=ubuntu-hostinger; diff = targetRevision only) → `argocd_check_values_branch` rc=0, all 6 values refs on v1.34.0, acg apps Synced/Healthy
- [x] **v1.33.0 PR gates — PR #126 MERGED 2026-09-14T01:13:54Z (`4b6ce874`)** — Codex live smoke PASS @08f15a18 (webhook 13/13 x2 incl --require-all-ok, cluster-health 9/0, node 14/14, pytest 69/69); Copilot 4 findings fixed (Kine registration base64 decode, %q Vault role path, duplicate CHANGELOG headings, BATS nesting simplified); GitGuardian false positive incident 37233897 resolved by user (base64 "root-token" BATS stub — intentional test data); Claude scope check PASS; CI PASS; enforce_admins re-enabled (verified true). Tag v1.33.0 + GitHub release published; ApplicationSet reapply deferred to v1.34.0 (read-only verify confirmed all sets already on v1.33.0); BATS negation follow-up deferred. Retrospective at `docs/retro/2026-09-13-v1.33.0-retrospective.md`.
...
- [x] **Codex: hub recovery automation specs (v1.33.0, 2026-09-13 — A `b71e587a`, B `35388b47`, C `45be3aec`, all Claude-verified)** — `docs/bugs/2026-09-13-hub-recovery-manual-fixes-not-declarative.md` (`hub_recovery_reconcile`), `docs/bugs/2026-07-17-ambient-istio-cni-conf-bin-dir-mismatch.md` §Spec 2026-09-13, `docs/bugs/2026-09-13-grafana-port-forward-plist-overwritten-by-acg-writers.md`. Verify each SHA on origin before trusting.
- [x] **Bug (DONE `ae661433`, Claude-verified): keycloak.sh leaked RETURN trap → `hub_recovery_reconcile --confirm` rc=1 `wd: unbound variable`** (all steps succeeded) — spec `docs/bugs/2026-09-13-keycloak-leaked-return-trap-unbound-wd.md`, READY FOR CODEX.
- [x] **Bug: hub restore leaves no Vault `secret/argocd/admin` (show-service-passwords ArgoCD N/A)** — spec `docs/bugs/2026-09-13-hub-restore-argocd-admin-vault-mirror-missing.md`, DONE `b231fb51` (Claude verified shellcheck rc 0, BATS 19/19). Live applied 2026-09-13 (reconcile --confirm rc=0; show-service-passwords ArgoCD populated). Follow-up `[ ]`: silence expected-miss ERROR noise from the Vault kv get check.
- [x] **Bug: `bin/smoke-test-cluster-health` stale defaults** — APP_CONTEXT `ubuntu-k3s` + pre-rename app names; exits 1 silently at ArgoCD step. Spec `docs/bugs/2026-09-13-smoke-test-cluster-health-stale-defaults-silent-exit.md` DISPATCHED to Codex (chain F) 2026-09-13; run 1 halted on spec conflict (`:-` vs empty override), S2 fixed to `-`; run 2 halted on pass count 11→9, fixed, RE-DISPATCHED (run 3). DONE `77008edb` — verified BATS 4/4, shellcheck 0, live 9/0. (found 2026-09-13 live smoke)
- [x] **Drift: `hub-platform-ops` perpetually OutOfSync on ES `platform-ops/app-cluster-kubeconfig`** after Succeeded sync — cause: SSA without ServerSideDiff (server-side `kubectl diff` = no spec diff). Spec `docs/bugs/2026-09-13-hub-platform-ops-externalsecret-perpetual-outofsync.md` DISPATCHED to Codex (chain G) 2026-09-13. Code DONE `79b8a10e` (verified BATS 5/5); surgical live AppSet reapply 2026-09-13 15:15Z (user go) → hub-platform-ops Synced/Healthy. DONE.
- [x] **HUB CRITICAL: rogue ArgoCD v2.14.2 in hub `cicd` (2026-09-13 13:40Z)** — `hub_recovery_reconcile` step 3 → `register_app_cluster` labels the (intended) in-cluster `ubuntu-k3s-app-cluster` Secret with `environment=infra`/`argocd-chart-version`, so platform-helm generated `ubuntu-k3s-platform`. It downgraded Argo CRDs, wiped argocd-rbac-cm policy, repointed cmd-params repo/redis. Live cleanup blocked by classifier — needs user-run runbook (strip finalizer FIRST, then drop labels, delete argocd-ubuntu-k3s-*, reapply v3.5.2 CRDs + helm cms, restart). Code fix spec `docs/bugs/2026-09-13-register-app-cluster-in-cluster-labels-trigger-platform-helm.md` written 2026-09-13, Codex DONE `1fe0e7fd` — Claude-verified: on origin, argocd.bats 26/26 (new tests 23/24 ok), shellcheck clean, only argocd.sh/argocd.bats/CHANGELOG changed. Runbook `hubfix-step1.sh` pipefail bug fixed + shellcheck clean; live run denied by classifier a 3rd time. User run #1 (2026-09-13 ~16:59Z) ABORTED safely at step 1: platform-helm appset controller re-adds resources-finalizer (template has none; flag unset). Runbook revised: step 1 sets `preserveResourcesOnDeletion: true` on platform-helm so the controller strips it, verify 30s; step 2 reverts `/spec/syncPolicy` after the app is gone. Awaiting user re-run. Must run BEFORE any reconcile with `1fe0e7fd` (cascade trap). In-cluster registration itself is correct — do not repoint. **RESOLVED 2026-09-13 ~17:05Z** (user run #2 of revised runbook, rc 0): finalizer stripped via preserveResourcesOnDeletion (then reverted), labels dropped, apps 24→23, 0 `argocd-ubuntu-k3s-*` left, CRD server-side diff vs v3.5.2 = 0/0/0, cmd-params repo/redis back to argocd-*, server.insecure=true, rbac-cm 68-line policy.csv restored, argocd-cm url + oidc.config restored; all 5 ArgoCD components v3.5.2 Running; istiod apps Synced/Healthy; smoke-test-cluster-health 9/9.
- [x] **Stale `operationState.phase=Running` on 3 hub apps** — kube-prometheus-stack (started 14:11Z), shopping-cart-identity (14:30Z), ubuntu-k3s-data-layer (13:52Z). Orphaned by the dual-controller window; no `.operation`, apps Synced/Healthy, controller skips auto-sync. Cosmetic but blocks manual sync ("another operation in progress"). Clearing needs a live sync/terminate — user go required. 2026-09-13: user asked to complete; Claude `kubectl patch ... remove /status/operationState` DENIED by classifier (4th [Modify Shared Resources]) — user must run it via `!`. DONE 2026-09-13: user ran the patch via `!` (3/3 patched); Claude verified read-only: all three Synced/Healthy, operationState phase empty.
- [x] **Bug: smoke-test-cluster-health checks pods on the wrong cluster** — `APP_CONTEXT` default `ubuntu-hostinger`, but checked apps `ubuntu-k3s-shopping-cart-*` deploy in-cluster to the hub; hostinger pods are orphaned `ubuntu-hostinger-*` tracking ids with no hub Application. Spec `docs/bugs/2026-09-13-smoke-test-cluster-health-pods-checked-on-wrong-cluster.md` (APP_CONTEXT → INFRA_CONTEXT) DISPATCHED to Codex 2026-09-13. Codex implemented + BATS 6/6 but sandbox denied `git add`; Claude reviewed diff vs spec, reran shellcheck rc0 + BATS 6/6, committed DONE `57f8af1f` (on origin). Live run: pods on k3d-k3d-cluster 5/5, 9/0.
- [x] **Hostinger shopping-cart workloads orphaned** — no `ubuntu-hostinger-*` Applications / cluster Secret on the hub; pods still Running on srv1754834. Needs user decision (re-register or retire). (found 2026-09-13) 2026-09-13: user chose RE-REGISTER (additive). Found `make refresh CLUSTER_PROVIDER=k3s-hostinger` is unsafe now (istio-ambient single-destination would move istiod off hub; access layer moves edge). Spec `docs/bugs/2026-09-13-hostinger-app-cluster-registration-lost-orphaned-workloads.md` (registration-only `refresh_registration`, refuses exclusive) — Codex IMPLEMENTED `14f26f3d` (on origin); Claude verified diff == S1-S3, 5 files, BATS 57/0, shellcheck 0 warnings, make -n OK + k3d guard errors. Live run by user 2026-09-13: DONE — cluster-ubuntu-hostinger role=app-cluster env=dev; hub secret + 9 ubuntu-k3s apps unchanged; 12 ubuntu-hostinger-* + acg-kube-prometheus-stack/acg-trivy-operator/loki adopted existing workloads, all Synced/Healthy; smoke 9/0. Note: register path also reconfigures Vault auth mount kubernetes-ubuntu-hostinger (idempotent).
- [x] **istiod HPA replica churn (hub)** — HPA 1→5→1 (50m request, ~48m hub load, no behavior). Fix in git: istio-ambient `pilot.autoscaleMax: 2` + scaleUp 120s / scaleDown 900s stabilization; request stays 50m (hostinger budget). helm-template verified; kubectl diff vs live appset = only those lines. Spec `docs/bugs/2026-09-13-istiod-hpa-replica-churn.md`. User ran `deploy_istio_ambient --confirm` live 2026-09-13 — DONE: HPA max 2 + behavior live, mesh apps Synced/Healthy, smoke 9/0.
- [x] **Hub OrbStack restart fallout (hub)** — memory bump 12→16GiB (user-approved restart). Exposed: (1) serverlb `values.yaml` empty since 2026-09-10 rebuild → host API EOF; (2) node-health-watch counts host-API-unreachable as NotReady → restarted healthy agent-0 ~6×; (3) ambient pods stale redirection → frontend timeout; (4) hostNetwork istio-cni stale IP. All RECOVERED LIVE (user ran fixes): 37/37 apps Synced/Healthy, frontend 200, smoke 9/0. Durable fixes SPECCED + dispatched to Codex 2026-09-13: (1) `docs/bugs/2026-09-13-hub-recovery-serverlb-empty-upstreams.md` (reconcile step 1 asserts/repairs LB upstreams by container name); (2) `docs/bugs/2026-09-13-node-health-watch-restarts-on-host-api-unreachable.md` (`/readyz` guard; `_tick` + source guard; behavioral BATS). Codex `8e6d84b2` + `2eb9c7cf` (committed, not pushed by Codex); Claude verified (scope 3 files each, shellcheck clean, BATS 31/31), fixed header comment `c7ee484e`, pushed. Watchdog kickstarted by user 13:52 (new code loaded, log quiet, 4/4 Ready) — DONE. Doc `docs/bugs/2026-09-13-hub-orbstack-restart-serverlb-empty-config.md`.
...
- [x] 2026-09-15 — `make restore-google-app-password` added (Keychain → Vault merge of `gmail_app_pw` only → `make observability`); spec `docs/bugs/2026-09-14-restore-google-app-password-target.md`; dry-run OK; operator live run PASSED (Vault updated, smtp secret unchanged = values already matched)
