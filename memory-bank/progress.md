...
- [ ] **HUB CRITICAL: `ubuntu-k3s` cluster Secret points at the hub (`kubernetes.default.svc`, empty config) since the 09-11 restore** — all `ubuntu-k3s-*` apps deploy onto the hub; platform-helm generated `ubuntu-k3s-platform`, which installed a second ArgoCD v2.14.2 (chart 7.8.1) in hub `cicd` at 2026-09-13 13:40Z, owning argocd-cm/argocd-secret/rbac-cm and the Argo CRDs. Two controllers fight → istiod flap + orphaned data-layer op (terminate-op refused: no .operation). Awaiting user decision on remediation. (found 2026-09-13)
