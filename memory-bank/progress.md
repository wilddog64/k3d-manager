...
- [ ] **`_ensure_gcloud`** — **ASSIGNED to Codex** (`docs/plans/v1.0.7-ensure-gcloud.md`). Auto-install `gcloud` CLI when missing; replace raw `command -v gcloud` check in `k3s-gcp.sh`; BATS tests. Blocker: live smoke test hit `ERROR: [k3s-gcp] gcloud CLI not found in PATH`.
