...
- [ ] **Live LDAP bind DN mismatch** — Keycloak was bound to `cn=admin,dc=shopping-cart,dc=local` while the live LDAP admin secret exposes `ldap-admin`. The shopping-cart-infra source has been patched to `cn=ldap-admin,dc=shopping-cart,dc=local`, PR #48 is open (`https://github.com/wilddog64/shopping-cart-infra/pull/48`), and the Copilot thread has been resolved after adding `LDAP_ADMIN_USERNAME: "ldap-admin"` to `identity/ldap/deployment.yaml`. It still needs to merge before SSO can be re-tested. Issue doc: `docs/issues/2026-05-12-shopping-cart-infra-keycloak-ldap-bind-dn-mismatch.md`.
