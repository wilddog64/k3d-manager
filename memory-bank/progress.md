...
- [ ] **deploy_argocd not loaded + invalid flag** — SPECCED → Codex. Spec `docs/bugs/v1.1.0-bugfix-gcp-deploy-argocd-not-loaded.md`. `argocd.sh` not sourced → "command not found" at step 6/7; `--skip-ldap` is not a valid flag. Fix: source `argocd.sh` inside `gcp_provision_stack`; use `deploy_argocd --bootstrap --skip-istio`.
