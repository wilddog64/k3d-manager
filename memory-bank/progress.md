...
- [ ] **SEEDER SELF-HEAL FIX — SPEC'D + DISPATCHED TO CODEX 2026-09-05 (user go "dispatch to codex to fix the issue").** Spec `docs/plans/v1.29.0-vault-seeder-self-heal-grafana-cosign.md` (plan doc #4/5 for v1.29.0), committed `c7fa6cbd` on `origin/k3d-manager-v1.29.0`. Design (verified against code): **grafana = fresh-generate-if-absent** (user chose this; NO backup exists — rotator reads old pw from Vault itself, so nothing to restore; new `_observability_seed_grafana_if_absent` seeds `secret/observability/grafana` username=admin + random hex pw using the rotator's own generator, called first in `_observability_apply_grafana_rotator`). **cosign = non-destructive restore** (real Keychain backup `k3d-manager-signing` exists): new `signing_restore` + `_signing_restore_vault_from_keychain` (loads key/pw from Keychain, hex-decodes PEM via xxd if `security -w` encoded, derives pub on host, reuses `_signing_write_vault`) + `_signing_keychain_backup_exists`; `signing_init` absent-branch now PREFERS restore over regenerate when backup present. Codex writes code+BATS only; live seeder run against hub = separate Claude step (deferred, classifier-gated). Verify Codex per protocol (SHA on origin, BATS green, scope) — expect Codex can't commit (sandbox `.git` lock) → Claude commits.
