...
- [ ] **HUB CRITICAL: rogue ArgoCD v2.14.2 in hub `cicd` (2026-09-13 13:40Z)** — `hub_recovery_reconcile` step 3 → `register_app_cluster` labels the (intended) in-cluster `ubuntu-k3s-app-cluster` Secret with `environment=infra`/`argocd-chart-version`, so platform-helm generated `ubuntu-k3s-platform`. It downgraded Argo CRDs, wiped argocd-rbac-cm policy, repointed cmd-params repo/redis. Live cleanup blocked by classifier — needs user-run runbook (strip finalizer FIRST, then drop labels, delete argocd-ubuntu-k3s-*, reapply v3.5.2 CRDs + helm cms, restart). Code fix spec `docs/bugs/2026-09-13-register-app-cluster-in-cluster-labels-trigger-platform-helm.md` written 2026-09-13, dispatched to Codex (verify SHA on origin + BATS). Runbook `hubfix-step1.sh` pipefail bug fixed + shellcheck clean; live run denied by classifier a 3rd time — user must run it BEFORE any reconcile with the fix (cascade trap). In-cluster registration itself is correct — do not repoint.
