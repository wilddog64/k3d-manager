...
- [ ] **Open issue — Keycloak admin token mismatch after rebuild** — live admin login against `keycloak-secrets` returns `401 invalid_grant`, so `acg-up` cannot reliably reconcile the rebuilt cluster's `argocd` client redirect URIs. Spec: `docs/issues/2026-05-11-keycloak-admin-token-reconciliation-failure.md`.
