...
- [x] **Bug: hub restore leaves no Vault `secret/argocd/admin` (show-service-passwords ArgoCD N/A)** — spec `docs/bugs/2026-09-13-hub-restore-argocd-admin-vault-mirror-missing.md`, DONE `b231fb51` (Claude verified shellcheck rc 0, BATS 19/19). Live apply pending user go on `hub_recovery_reconcile --confirm`.
