...
- [ ] **E2E chart owning-service labels** — Failure trend/cause/spec charts must use `exported_service`; the raw `service` label is the Prometheus scrape target (`vulnerability-inventory-exporter`).
- [ ] **E2E Recent runs owning-service list** — Add the derived `Services under test` label to replace the misleading consolidated service value in the Recent runs table.
