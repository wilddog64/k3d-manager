...
- [ ] **User-session Keychain authorization** — terminal Keychain reads are zero-length and writes fail with `User interaction is not allowed`; operator must unlock/authorize the login Keychain before token rotation can complete. Do not store the webhook token in plaintext as a workaround.
