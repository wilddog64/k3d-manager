...
- [ ] **k3d-provider RETURN trap scope** — OPEN. Spec `docs/bugs/2026-04-24-k3d-provider-return-trap-scope.md`; bash RETURN traps are shell-global — re-fire in parent functions with local variables out of scope. Fix: prepend `trap - RETURN;` in both handlers (lines 100 + 142) to self-clear on first fire.
