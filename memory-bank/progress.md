...
- [ ] **acg-up Hub cluster auto-create** — OPEN. Spec `docs/bugs/2026-04-24-acg-up-hub-cluster-not-created.md`; Step 3.5 aborts when Hub cluster is missing instead of creating it; `make down → make up` always fails. Fix: replace `_err` with `deploy_cluster --provider k3d`.
