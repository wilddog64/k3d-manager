...
- [ ] **Keycloak browser-flow regression (all shopping-cart SSO)** — spec `docs/bugs/2026-09-15-keycloak-browser-flow-regression-blocks-all-sso.md`; fix `c3a2c37` + Copilot follow-up `78403f0` on shopping-cart-infra `fix/keycloak-browser-flow-top-level-otp`; **PR open: https://github.com/wilddog64/shopping-cart-infra/pull/98** (2026-09-15). CI green on `78403f0` (Kubeconform / Kustomize Build / YAML Lint / GitGuardian). Copilot reviewed with verdict 'Needs a closer look' and filed 1 inline finding (the level-0 guard conflated a wrong execution count with unexpected execution names, printing an empty name list) — fixed in `78403f0`, replied, thread resolved. NOTE: the reviewer-request API no-ops on this repo (both forms return 200 with `requested_reviewers: []`) but Copilot still reviews on its own, so give it time rather than assuming it will not attach. `enforce_admins` DISABLED on shopping-cart-infra main — **re-enable with a bodyless POST after merge**. Merging is what makes ArgoCD sync `identity/keycloak/` and run the PostSync reconcile hook; until then Frontend SSO + ArgoCD SSO stay red as `authorization request rejected (HTTP 400)`.
