...
- [ ] **acg-sync-apps https vs http readiness** — OPEN. Spec `docs/bugs/2026-04-24-acg-sync-apps-https-vs-http-readiness-check.md`; readiness curl uses `https://` but ArgoCD is deployed with `server.insecure=true` (HTTP only) — TLS handshake fails, port-forward deemed never ready, exits 1.
