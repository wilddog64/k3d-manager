...
| `gcp_login` wrong click target — `getByText` exact | OPEN | `docs/bugs/v1.1.0-bugfix-gcp-login-use-another-locator.md`; fix: `getByText({exact:true})` + URL diagnostics |
| Live smoke test `make up CLUSTER_PROVIDER=k3s-gcp GHCR_PAT=<pat>` | PENDING | After click target fix |
