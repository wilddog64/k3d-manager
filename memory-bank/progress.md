...
- [ ] Hub observability gaps from 2026-09-10 rebuild (awaiting go): helm upgrade argocd (ServiceMonitors skipped — CRD created 1 min after install); apply `scripts/etc/observability/promtail.yaml` (no log shipper → Loki panels empty); spec a rebuild-ordering fix
