...
| Jenkins smoke test fails on macOS — Istio LB IP unreachable | OPEN | `172.18.0.x` ingress IP not routable from macOS host. Fix: `_jenkins_run_smoke_test` detects private IP + macOS, spins up `kubectl port-forward` in background, runs smoke against `127.0.0.1:8443`, tears down via `trap`. `JENKINS_SMOKE_URL` as CI escape hatch only. See `docs/issues/2026-02-25-jenkins-smoke-test-ingress-retries.md`. |
