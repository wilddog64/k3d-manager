...
- [ ] **ArgoCD rendered unbound EXIT trap bugfix** — **ASSIGNED → Codex**. Spec `docs/bugs/v1.1.0-bugfix-argocd-rendered-unbound-exit-trap.md`. Fix dangling EXIT trap in `_argocd_deploy_appproject` that fires with unbound `$rendered` after function returns; replace RETURN trap with explicit `rm -f` + `trap - EXIT`.
...
- [x] **deploy_argocd not loaded + invalid flag** — COMPLETE (`7e65364e`). Spec `docs/bugs/v1.1.0-bugfix-gcp-deploy-argocd-not-loaded.md`. `gcp_provision_stack` now sources `argocd.sh` before calling `deploy_argocd` and uses `--bootstrap --skip-istio` (LDAP already disabled by default); shellcheck + bats pass.
