...
- [x] **ArgoCD CLI login plaintext prompt** — COMPLETE (`unassigned`). Bug `docs/bugs/2026-04-24-argocd-cli-login-plaintext-prompt.md`; `_argocd_ensure_logged_in()` now uses `--plaintext --skip-test-tls` and closes stdin, which removes the TLS confirmation prompt and lets `deploy_argocd` complete bootstrap.
