...
- [ ] **Bug 15 — Argo CD port-forward flaps after sandbox rebuild** — `localhost:8080` can stay bound while the backend pod sandbox is lost, so the browser still fails to reach the Argo CD login page after a fresh `make up`. Spec: `docs/issues/2026-05-11-acg-up-argocd-port-forward-flapping.md`.
