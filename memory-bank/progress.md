...
| Keycloak plugin missing | FIXED | 2026-03-03: `deploy_keycloak` plugin added with Vault/LDAP integrations and Bats coverage. **2026-03-04:** `$KEYCLOAK_LDAP_USERS_DN` whitelist added, PR #13 follow-ups (config CLI flag, non-Vault admin secret, shared SecretStore) implemented, and `_keycloak_ensure_admin_secret` now quotes passwords. |
