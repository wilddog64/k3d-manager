...
## PENDING
- [ ] **Claude live re-verify pending for `ce4d83f0`** — re-sync the istio-ambient ApplicationSet from git on k3s-aws, confirm `istio-cni-node` mounts `/etc/cni/net.d` + `/opt/cni/bin`, returns `1/1` on all nodes, and the ambient dataplane capture still passes before declaring the blocker closed.
