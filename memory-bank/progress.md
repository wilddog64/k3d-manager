...
- [x] **Argo CD browser auto-open on macOS** — `bin/acg-up` now opens `https://argocd.shopping-cart.local` automatically once the browser listener is healthy, preventing stale localhost tabs from being the default browser entrypoint. Issue doc: `docs/issues/2026-05-12-acg-up-auto-opens-canonical-argocd-browser.md`.
