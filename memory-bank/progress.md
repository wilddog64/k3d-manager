...
- [x] `64168cc7` — permit `secrets` namespace in the `shopping-cart` AppProject for `vault-bridge`. One file `scripts/etc/argocd/projects/shopping-cart.yaml.tmpl`; added 4 `secrets` destinations after the payment block. Gates: `yq` clean, `grep -c 'namespace: secrets'` → 4, `grep -c 'namespace: shopping-cart-'` → 12, `bats scripts/tests/plugins/argocd.bats` 15/15, `./scripts/k3d-manager _agent_audit` exit 0. Pushed to `origin/k3d-manager-v1.16.0`.
