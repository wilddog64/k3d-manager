...
- [ ] **ESO cosign-public-key SecretSyncedError (hub platform-ops + hostinger kyverno)** — 2026-09-13 Slack alert. Vault 403 on `secret/data/cosign/signing`: ESO roles `eso-ldap-directory` (hub) and `eso-app-cluster` (mount `kubernetes-ubuntu-hostinger`) lack `cosign-verify`. Role rewriters (`_vault_configure_secret_reader_role`, `vault.sh` app-cluster role `policies=app-cluster-reader`) replace policies wholesale; grant loss masked by ESO's cached Vault token until the OrbStack restart forced re-login. Impact low: existing `cosign-public-key` Secrets retained; 24/25 hub + 19/20 hostinger ES synced. Also open: `make status` "webhook unavailable" (Keychain NOT proven locked — Claude/`!` shell cannot use Keychain; user to run diag in a real terminal) and `show-service-passwords` all N/A. Live fix = `signing_restore` per cluster (user runs). Durable fix SPECCED `docs/bugs/2026-09-13-vault-eso-role-rewrite-drops-cosign-verify.md` (`_vault_role_merged_policies` merge helper in both role writers) — dispatched to Codex 2026-09-13. Codex `b3bc737c` (committed; push failed — sandbox has no GitHub auth); Claude verified diff == S1–S3, shellcheck 0, BATS 101/101, pushed. Pending: operator live grant restore (hub: `./scripts/k3d-manager signing_restore`; hostinger: same with `SIGNING_ESO_ROLE=eso-app-cluster SIGNING_ESO_AUTH_MOUNT=kubernetes-ubuntu-hostinger`, both from hub context — hub has no kyverno ns so ES apply is skipped) then verify SecretSynced both clusters.
- [ ] **Login Keychain k3dm-* entries missing (user terminal) — 2026-09-13.** User's fresh Terminal.app window: `k3dm-webhook-token`, `k3dm-vault-root-token`, `k3dm-hermes-{audit,argocd,gh}-token`, `k3dm-slack-webhook`, `k3dm-stripe-sk-test`, `k3d-manager-signing`, `cloudflare-api-token` all rc=44. Claude's sandboxed shell reports metadata rc=0 for some but reads len=0 — sandbox Keychain view is NOT trustworthy; treat user's rc=44 as truth. No renamed login keychain; file 409KB, mtime 13:36. Nothing this session writes/deletes Keychain items. Explains `make status` webhook unavailable + `show-service-passwords` N/A. Recoverable without retyping: webhook token (`bin/k3dm-webhook-setup --rotate` + deploy-worker + restart-webhook + `argocd_sync_webhook_token_secret`), vault root (`_hub_recovery_sync_vault_root_token` reseeds from hub Secret `secrets/vault-root`). Must re-enter from source: hermes gh/audit PATs, Slack webhook, Stripe sk, Cloudflare token; hermes-argocd regenerable. Proposed: `make keychain-restore` spec (awaiting user go).
