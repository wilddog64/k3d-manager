...
- [x] **Keycloak readiness gate timeout is configurable** — `bin/acg-up` now waits longer by default and exposes `KEYCLOAK_READY_TIMEOUT_SECONDS` / `KEYCLOAK_READY_POLL_INTERVAL_SECONDS` so cold rebuilds do not die at 5 minutes. Issue doc: `docs/issues/2026-05-12-keycloak-api-readiness-timeout-too-short.md`.
...
- [x] **Identity troubleshooting helpers** — added `bin/vault-exec.sh`, `bin/ldap-search.sh`, `bin/keycloak-logs.sh`, and `bin/ldap-logs.sh` with shared `_run_command`-backed wrappers in `scripts/lib/identity_tools.sh`. Added `docs/howto/identity-troubleshooting.md` for quick operator reference.
