...
## Known Bugs / Gaps
- [ ] **ACG credential extraction misses visible sandbox** — SOURCE FIX IN LIB-ACG PR #2 (`https://github.com/wilddog64/lib-acg/pull/2`). Copilot review comments addressed and resolved; latest code commit `1ddbf7c` has CI green. Merge is blocked only because PR #2 remains draft; k3d-manager subtree pull still pending. Temporary local verification with the same patch passed: `acg_get_credentials` and `aws sts get-caller-identity`. Spec: `docs/issues/2026-04-28-acg-credentials-cdp-context-miss.md`.
