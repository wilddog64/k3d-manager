...
- [ ] **Hub OrbStack restart fallout (hub)** — memory bump 12→16GiB (user-approved restart). Exposed: (1) serverlb `values.yaml` empty since 2026-09-10 rebuild → host API EOF; (2) node-health-watch counts host-API-unreachable as NotReady → restarted healthy agent-0 ~6×; (3) ambient pods stale redirection → frontend timeout; (4) hostNetwork istio-cni stale IP. All RECOVERED LIVE (user ran fixes): 37/37 apps Synced/Healthy, frontend 200, smoke 9/0. Durable fixes SPECCED + dispatched to Codex 2026-09-13: (1) `docs/bugs/2026-09-13-hub-recovery-serverlb-empty-upstreams.md` (reconcile step 1 asserts/repairs LB upstreams by container name); (2) `docs/bugs/2026-09-13-node-health-watch-restarts-on-host-api-unreachable.md` (`/readyz` guard; `_tick` + source guard; behavioral BATS). Codex `8e6d84b2` + `2eb9c7cf` (committed, not pushed by Codex); Claude verified (scope 3 files each, shellcheck clean, BATS 31/31), fixed header comment `c7ee484e`, pushed. Remaining: operator `launchctl kickstart -k` for the watchdog. Doc `docs/bugs/2026-09-13-hub-orbstack-restart-serverlb-empty-config.md`.
