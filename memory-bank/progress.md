...
- [ ] **COSIGN RESTORE (pending, this incident).** `secret/cosign/signing` also missing → `cosign-public-key` ES errored (Audit gate = no outage). Restore from Keychain `k3d-manager-signing` (`k3dm-cosign-key`/`k3dm-cosign-password`), derive `cosign.pub` via `cosign public-key`, write key/password/pub, force-sync `platform-ops/cosign-public-key` ES. **NEVER `signing_init`/`deploy_image_signing`** (regenerates keypair + clobbers Keychain backup).
