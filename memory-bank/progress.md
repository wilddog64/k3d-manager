...
- [ ] **GCP IAM CLI pivot** — **ASSIGNED to Gemini** (`867e45d1`). `docs/plans/v1.0.7-gcp-iam-cli-pivot.md`; replace Playwright-based IAM grant with `gcp_login` + `gcp_grant_compute_admin` using `gcloud projects add-iam-policy-binding`. `gcp_iam_grant.js` already deleted. Adds BATS test with `gcloud` stub.
