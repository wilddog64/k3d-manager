...
- [ ] **Hermes scheduled `make status` sensor + triage** — spec `docs/plans/v1.34.0-hermes-scheduled-status-triage.md` (2026-09-16, not assigned). Runs `bin/cluster-status --json` on a ~40 min gate inside the existing 300s Hermes poll, classifies reds by check id, notifies on state change only, files bugs via the e2e worktree path. No new launchd agent, no new REPAIRS entry, no auto-execution (proposal + Slack approval only). Blocked on fixing the false Prometheus 401 red first.
