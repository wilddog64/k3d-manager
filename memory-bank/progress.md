...
- [x] **GCP provider skeleton** — **COMPLETE** (`1c620795`). `docs/plans/v1.0.7-gcp-provider.md`; added `scripts/plugins/gcp.sh`, `scripts/lib/providers/k3s-gcp.sh`, and `scripts/tests/providers/k3s_gcp.bats`. Deploy flow wraps `gcp_get_credentials` → `gcloud` compute → `k3sup` install → kubeconfig merge/label; destroy requires `--confirm`. `shellcheck -S warning` clean on both new shell files; `bats scripts/tests/providers/k3s_gcp.bats` passes. Post-review fixes (`d4e73a66`, `13ec1f67`) pin Ubuntu image family, use `--local-path`, disable Traefik, clean kubeconfig context, validate `gcp.sh` output, and add `_ensure_gcloud` auto-install path.
- [x] **GCP provider post-review bugfix** — **COMPLETE** (`d4e73a66`). Ubuntu image pins, `--local-path`, `--disable=traefik`, destroy kubeconfig cleanup, `gcp.sh` validate-before-export.
- [x] **GCP ensure gcloud helper** — **COMPLETE** (`13ec1f67`). `docs/plans/v1.0.7-ensure-gcloud.md`; adds `_ensure_gcloud` auto-install helper, wires `_provider_k3s_gcp_deploy_cluster` to use it, and expands provider BATS suite with `_ensure_gcloud` stubs/tests. `shellcheck scripts/plugins/gcp.sh scripts/lib/providers/k3s-gcp.sh` + `bats scripts/tests/providers/k3s_gcp.bats` pass. ADC follow-ups (`effa1982`, `153fc922`) replace `gcloud auth activate-service-account` with ADC env vars, per-command `--project`, and the new `CLOUDSDK_AUTH_ACCESS_TOKEN` flow that calls `gcloud auth application-default print-access-token`.
- [x] **GCP provider pre-flight compute check** — **COMPLETE** (`7360d96e`). Issue: `docs/issues/2026-04-12-gcp-provider-fixes.md` (Change 6). Adds `_gcp_preflight_check_compute` helper that runs `gcloud compute instances list --limit=1` up front and fails fast with actionable IAM guidance if the sandbox service account lacks compute permissions.
...
- [ ] **Stale kubeconfig merge** — SPECCED → Codex. Spec `docs/bugs/v1.1.0-bugfix-gcp-kubeconfig-stale-merge.md`. Delete stale k3s-gcp context/cluster/user from `~/.kube/config` before flatten merge so IP-change re-runs pick up fresh `k3s-gcp.yaml` credentials.
- [ ] **k3s API server readiness probe** — SPECCED → Codex. Spec `docs/bugs/v1.1.0-bugfix-gcp-k3s-api-readiness-probe.md`. `nc -z` loop on port 6443 (10s backoff, 30 retries) between kubeconfig merge and `kubectl label nodes`.
- [ ] **make provision depends on make up** — SPECCED → Codex. Spec `docs/bugs/v1.1.0-bugfix-gcp-provision-depends-on-up.md`. k3s-gcp provision case calls `$(MAKE) up` before `gcp_provision_stack`; makes provision idempotent end-to-end.
