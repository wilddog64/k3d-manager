...
- [ ] **acg-sync-apps default app wrong** — OPEN. Spec `docs/bugs/2026-04-24-acg-sync-apps-argocd-app-default-wrong.md`; `ARGOCD_APP` defaults to `data-layer` but no such app exists — only `rollout-demo-default` and `rollout-demo-staging` are created by the static list generator in `demo-rollout.yaml`.
