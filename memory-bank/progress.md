...
- [~] **v1.29.0 PR — CREATED 2026-09-06 (user go "yes, please").** PR **#120** (https://github.com/wilddog64/k3d-manager/pull/120), base `main` ← head `k3d-manager-v1.29.0` @ `1f56d04e`, 40 files. Before opening: **release-ledger backfill** commit `1f56d04e` (`origin/k3d-manager-v1.29.0`) — README releases table was 3 versions behind (added v1.28.0/v1.27.0/v1.25.0 rows from canonical `docs/releases.md`) + CHANGELOG missing `[1.28.0]` (added; `[1.25.0]` gap intentional/documented — folded into 1.26.0). Local pre-flight green before create: BATS 797/801 (4 known pre-existing local-macOS), shellcheck clean, scope check (40 files in-scope, 4 plan-doc changes under the 5-cap), live read-only smoke clean (ArgoCD healthy post-reapply, Hermes 5/5 sensors no false incident, cosign key material present). Copilot requested via raw-JSON `requested_reviewers` POST — **verified attached via GraphQL** (Bot `copilot-pull-request-reviewer`; the REST `requested_reviewers` GET is blind to bots, do not trust its empty array). CI on the PR went **all green** (Analyze actions/js/python, CodeQL, lint, GitGuardian, detect; stage2 conditional-skip). **Copilot review addressed 2026-09-06 — commit `60c01632`:** Copilot (COMMENTED) raised 2 real xtrace secret-leak findings — `_no_trace` only disables xtrace inside its own body, so under `set -x` Bash prints the fully-expanded calling line (with the secret) before `_no_trace` runs. Fixed both: (1) observability.sh `_observability_seed_grafana_if_absent` — xtrace guard around fresh-password gen + `put_cmd` build + streamed `vault kv put`; (2) signing.sh `_signing_restore_vault_from_keychain` — xtrace guard after Keychain-secret load through the subshell (covers private-key file write, `COSIGN_PASSWORD` cosign call, `_signing_write_vault` password argv), restored after. signing.bats 28/28, observability.bats 14/14, shellcheck clean (2 pre-existing SC2016). Both threads replied + **resolved via GraphQL** (0 unresolved). New head `60c01632` — CI re-running. **REMAINING: confirm CI green on `60c01632`, then STOP at merge gate for user go. NEVER auto-merge.** v1.29.0's own README/releases.md row deferred to the v1.30.0 branch per convention.
