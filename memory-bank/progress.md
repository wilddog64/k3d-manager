...
- [ ] **HUB CRITICAL: rogue ArgoCD v2.14.2 in hub `cicd` (2026-09-13 13:40Z)** — `hub_recovery_reconcile` step 3 → `register_app_cluster` labels the (intended) in-cluster `ubuntu-k3s-app-cluster` Secret with `environment=infra`/`argocd-chart-version`, so platform-helm generated `ubuntu-k3s-platform`. It downgraded Argo CRDs, wiped argocd-rbac-cm policy, repointed cmd-params repo/redis. Live cleanup blocked by classifier — needs user-run runbook (strip finalizer FIRST, then drop labels, delete argocd-ubuntu-k3s-*, reapply v3.5.2 CRDs + helm cms, restart). Code fix needs docs/bugs spec (no platform-helm labels for in-cluster registration). In-cluster registration itself is correct — do not repoint.
