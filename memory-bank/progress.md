...
- [ ] Hub federate-acg self-scrape duplicates (`docs/bugs/2026-09-13-hub-federate-acg-self-scrape-duplicates-series.md`) — spec'd 2026-09-13 (+S2b `b4ae72e9`: existing `argocd_metrics_servicemonitor.bats` pinned old exprs — Codex correctly stopped), Codex `22feff51` VERIFIED by Claude (yq ok, BATS 27/27, 5 spec files; Claude pushed); dashboard applied live 2026-09-13; PENDING operator go: scoped `observability` AppSet reapply to v1.34.0 (server dry-run clean: only targetRevision v1.33.0→v1.34.0; values diff = the 4-line relabel only); hub `count({cluster="acg"})`=82208 before
