...
- [ ] **Live LDAP bind DN mismatch** — Keycloak still binds as `cn=admin,dc=shopping-cart,dc=local` while the live LDAP admin secret exposes `ldap-admin`, so SSO cannot proceed. Issue doc: `docs/issues/2026-05-12-shopping-cart-infra-keycloak-ldap-bind-dn-mismatch.md`.
