...
- [x] `1af15217` — right-size istiod (100m/512Mi) + ztunnel (100m/256Mi) for 2-CPU hostinger node. Code PASS; **live-untested** (blocked by istio-ambient dest validation).
...
## PENDING
- [ ] **`shopping-cart` AppProject secrets destination — HANDED TO CODEX (2026-07-19).** `docs/bugs/2026-07-19-shopping-cart-appproject-secrets-destination.md`. Single file `scripts/etc/argocd/projects/shopping-cart.yaml.tmpl`; add 4 `secrets` destinations. Commit `fix(argocd): permit secrets namespace in shopping-cart AppProject for vault-bridge`. Live-proved by Claude; k3s-aws-only. **Awaiting Codex SHA → verify on origin (1 file, gates: grep secrets→4 / shopping-cart-→12, bats 15/15, `_agent_audit` 0), then confirm committed template == proven live patch.**
- [ ] **istio-ambient dest validation** — `no clusters with this name: ubuntu-hostinger (and 3 more)`; blocks pod-level `1af15217` verify. NOT specced.
