...
- [ ] **"I understand" welcome screen inside Allow polling loop** — OPEN. Spec `docs/bugs/v1.1.0-bugfix-gcp-login-i-understand-in-poll-loop.md`. After password + Next on enterprise accounts, Google navigates to a "Welcome to your new account" page requiring "I understand". The existing handler runs before the loop when the page is still on `challenge/pwd`; fix adds handler inside the polling loop + increases password post-Next wait to 2000ms. Assigned to Codex.
