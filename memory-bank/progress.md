...
| AccountChooser URL case mismatch + post-email chooser | OPEN | `docs/bugs/v1.1.0-bugfix-gcp-login-accountchooser-case-and-post-email.md`; `toLowerCase().includes('accountchooser')` fix + post-email-Next chooser handler |
| Live smoke test `make up CLUSTER_PROVIDER=k3s-gcp GHCR_PAT=<pat>` | PENDING | After case fix |
