...
- [ ] **GCP stale auth `invalid_grant` on sandbox rotation** — OPEN. Spec `docs/bugs/v1.1.0-bugfix-gcp-stale-auth-invalid-grant.md`. `invalid_grant: Account has been deleted` when ACG sandbox rotates; `_gcp_load_credentials` does not revoke stale accounts before re-auth; `_provider_k3s_gcp_status` calls gcloud without auth guard. Fix: revoke + active-account check. Assigned to Codex.
