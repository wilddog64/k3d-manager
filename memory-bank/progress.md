...
| Jenkins smoke test fails on macOS — Istio LB IP unreachable | OPEN | Port-forward architecture implemented (RFC-1918 detection, trap cleanup, SNI via RESOLVE_ARGS) but port-forward targets wrong service — `svc/jenkins:443` does not exist; must target `svc/istio-ingressgateway` in `istio-system`. See `docs/issues/2026-02-25-jenkins-smoke-test-routing-service-mismatch.md`. |
