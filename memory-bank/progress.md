...
- [ ] **Open issue — Keycloak admin token mismatch after rebuild** — live admin login against `keycloak-secrets` returns `401 invalid_grant`. Root cause: `bin/acg-up` reseeds `secret/keycloak/admin` on every rebuild instead of preserving the existing Vault password, so Keycloak's live DB and Vault drift apart. Specs: `docs/bugs/2026-05-11-keycloak-admin-password-reseed-on-rebuild.md` and `docs/issues/2026-05-11-keycloak-admin-token-reconciliation-failure.md`.
