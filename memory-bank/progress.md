...
- [x] **Argo CD browser auto-open deferred until SSO wiring completes** — `bin/acg-up` now waits until after Step 10f/14 (ArgoCD SSO wiring + host rewrites) before opening Safari, so the browser no longer launches before Keycloak is ready. Commit `044aae1f`. Issue doc: `docs/issues/2026-05-12-acg-up-browser-auto-open-before-keycloak-ready.md`.
- [ ] **Argo CD Safari stale localhost session follow-up** — generic `open` can still leave Safari on an old localhost tab in some cases, so the browser handoff may still need a more reliable Safari-specific path. Issue doc: `docs/issues/2026-05-12-argocd-safari-stale-localhost-tab-keeps-reopening.md`.
