...
- [ ] **Bug: `bin/smoke-test-cluster-health` stale defaults** — APP_CONTEXT `ubuntu-k3s` + pre-rename app names; exits 1 silently at ArgoCD step. Spec `docs/bugs/2026-09-13-smoke-test-cluster-health-stale-defaults-silent-exit.md` DISPATCHED to Codex (chain F) 2026-09-13; run 1 halted on spec conflict (`:-` vs empty override), S2 fixed to `-`; run 2 halted on pass count 11→9, fixed, RE-DISPATCHED (run 3). (found 2026-09-13 live smoke)
- [ ] **Drift: `hub-platform-ops` perpetually OutOfSync on ES `platform-ops/app-cluster-kubeconfig`** after Succeeded sync — cause: SSA without ServerSideDiff (server-side `kubectl diff` = no spec diff). Spec `docs/bugs/2026-09-13-hub-platform-ops-externalsecret-perpetual-outofsync.md` DISPATCHED to Codex (chain G) 2026-09-13; live AppSet reapply = operator step after.
