...
- [ ] **k3d-provider EXIT trap leak** — OPEN. Spec `docs/bugs/2026-04-24-k3d-provider-exit-trap-leak.md`; `_provider_k3d_configure_istio` uses EXIT trap for temp file cleanup — leaks into caller shell when called inline. Fix: change EXIT → RETURN (matching `_provider_k3d_create_cluster` pattern).
