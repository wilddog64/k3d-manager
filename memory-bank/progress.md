...
- [ ] v1.34.0 operator PENDING user go: `make platform-ops` (deploy fixed scanner) + rerun `argocd-cve-scan-manual` — CAUTION: real CVE hit + newer chart relabels `cluster-ubuntu-hostinger` (environment=dev) argocd-chart-version 7.8.1 → 10.9.1. `make alertmanager-secret` (Alertmanager Vault secret absent; SMS config skipped).
