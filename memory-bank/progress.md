...
| Smoke script silent failure (unbound PLUGINS_DIR) | OPEN | `bin/smoke-test-jenkins.sh` crashes instantly due to unbound `PLUGINS_DIR` in `vault.sh` when sourced. See `docs/issues/2026-02-25-smoke-script-standalone-dependency-failure.md`. |
| Jenkins VirtualService hostname detection fails | OPEN | `_jenkins_run_smoke_test` looks for `jenkins` VS in `istio-system` instead of the deployment namespace. See `docs/issues/2026-02-25-jenkins-smoke-test-hostname-detection-failure.md`. |
