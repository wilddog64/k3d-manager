...
- [ ] **Drift: `hub-platform-ops` perpetually OutOfSync on ES `platform-ops/app-cluster-kubeconfig`** after Succeeded sync — cause: SSA without ServerSideDiff (server-side `kubectl diff` = no spec diff). Spec `docs/bugs/2026-09-13-hub-platform-ops-externalsecret-perpetual-outofsync.md` DISPATCHED to Codex (chain G) 2026-09-13. Code DONE `79b8a10e` (verified BATS 5/5); live AppSet reapply AWAITING USER GO, then confirm Synced.
