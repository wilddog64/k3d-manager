...
- [x] **Argo CD browser auto-open on macOS** — `bin/acg-up` now opens `https://argocd.shopping-cart.local` automatically once the browser listener is healthy, preventing stale localhost tabs from being the default browser entrypoint. Commit `5e4b4de0`. Issue doc: `docs/issues/2026-05-12-acg-up-auto-opens-canonical-argocd-browser.md`.
- [ ] **Argo CD Safari stale localhost session follow-up** — generic `open` was still leaving Safari on an old localhost tab in some cases, so the browser handoff now tries to drive Safari directly with `osascript` before falling back. Issue doc: `docs/issues/2026-05-12-argocd-safari-stale-localhost-tab-keeps-reopening.md`.
