...
- [ ] **LDAP CrashLoopBackOff Bug 6** — Bug 5 fix incomplete; emptyDir at `ldif/custom` is still a mount point; `rm -rf assets/config` hits EBUSY. Fix: `LDAP_SEED_INTERNAL_LDIF_PATH=/ldif-staging` + emptyDir at `/ldif-staging`. Spec: `docs/bugs/2026-05-09-ldap-emptydir-mountpoint-rm-fails.md`; branch: `shopping-cart-infra fix/ldap-ldif-staging`
