...
- [ ] **acg-sync-apps port-forward reuse** — OPEN. Spec `docs/bugs/2026-04-24-acg-sync-apps-port-forward-reuse.md`; `bin/acg-sync-apps` still needs a durable ownership marker so it can reuse its own active ArgoCD port-forward and replace foreign listeners automatically.
