...
- [x] **Argo CD browser auto-open removed from bootstrap** — `bin/acg-up` no longer opens Safari at the end of bootstrap, so `make up` finishes cleanly on both macOS and Linux without a GUI dependency. Issue doc: `docs/issues/2026-05-12-acg-up-browser-auto-open-removed-for-noninteractive-flows.md`.
