...
| `gcp_login` email input timeout after "Use another account" | ASSIGNED → Codex | `docs/bugs/v1.1.0-bugfix-gcp-login-email-input-timeout.md`; add `waitForLoadState` + increase timeout |
| Live smoke test `make up CLUSTER_PROVIDER=k3s-gcp GHCR_PAT=<pat>` | PENDING | After gcp_login email input timeout fix |
...
- [x] **Wait for DOM after "Use another account"** — COMPLETE (`c7930b93`). Spec `docs/bugs/v1.1.0-bugfix-gcp-login-email-input-timeout.md`. `scripts/playwright/gcp_login.js` now waits for `domcontentloaded` after clicking "Use another account" and increases the email input timeout from 10000 to 15000ms.
