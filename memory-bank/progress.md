...
- [ ] **shopping-cart-infra realm import placeholders still need rendering** — the Keycloak initContainer now needs to render `identity/config/realm-shopping-cart.json` before import so `${LDAP_BIND_DN}` and `${LDAP_BIND_CREDENTIAL}` do not land literally in the live LDAP provider config. Issue doc: `docs/issues/2026-05-13-keycloak-realm-import-placeholders-not-rendered.md`.
