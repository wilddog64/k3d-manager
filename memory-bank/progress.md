...
- [x] **`gcp_login` email navigation race** — COMPLETE (`1bcee5fd`). Spec `docs/bugs/v1.1.0-bugfix-gcp-login-email-navigation-race.md`. `gcp_login.js` now wraps the click in `Promise.all([page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 20000 }), useAnother.click()])` and raises the email input timeout to 20000ms.
