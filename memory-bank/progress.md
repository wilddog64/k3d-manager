...
- [x] **order-service kustomize workarounds** — COMPLETE (`9db9589c`, `2b1215e2`, `445b7696`). DDL-auto, health probe paths, Spring AMQP credentials all patched in `services/shopping-cart-order/kustomization.yaml`.
- [ ] **orders init SQL SERIAL vs UUID + SecurityConfig probe gap** — OPEN. Spec: `docs/bugs/2026-04-27-orders-init-sql-serial-vs-uuid.md`. Needs PRs in `shopping-cart-infra` (init SQL) and `shopping-cart-order` (SecurityConfig). Assign to Codex.
