...
- [ ] **lib-acg main branch protection** — BLOCKED. GitHub connector confirms admin metadata access but does not expose branch-protection mutation; local `gh` auth is invalid and the only available token returns GitHub API `401`. Issue: `docs/issues/2026-04-28-lib-acg-main-branch-protection-auth-blocked.md`.
