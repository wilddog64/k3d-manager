...
- [x] **acg-down expired credentials abort** — COMPLETE (`ae2fca66`). Spec `docs/bugs/2026-04-24-acg-down-expired-credentials-abort.md`; `acg_teardown` now logs a visible warning on expired AWS credentials and still performs local Hub + Vault PF cleanup.
- [ ] **acg-down credential check noise** — OPEN. Spec `docs/bugs/2026-04-24-acg-down-credential-check-noise.md`; pre-check AWS creds silently before calling `acg_teardown`; skip with single clean INFO when invalid — eliminates ERROR noise that makes expired vs live-but-stale indistinguishable.
