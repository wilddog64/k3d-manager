...
- [x] **Keycloak browser hostname no longer points at loopback** — `bin/acg-up` now rewrites `keycloak.shopping-cart.local` to the ingress gateway IP instead of `127.0.0.1`, so Safari can reach the Keycloak auth endpoint. Commit `99581cc7`. Issue doc: `docs/issues/2026-05-12-keycloak-browser-connectivity-loopback.md`.
