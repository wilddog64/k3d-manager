...
- [ ] **shopping-cart-infra realm import placeholders still need rendering** — the Keycloak initContainer now renders the local `identity/keycloak/realm-shopping-cart.json` import source before import so the bind credential does not land literally in the live LDAP provider config. Issue doc: `docs/issues/2026-05-13-keycloak-realm-import-placeholders-not-rendered.md`.
