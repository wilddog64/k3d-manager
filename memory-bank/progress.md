...
- [ ] **Flap: `istiod-ubuntu-k3s` Synced↔OutOfSync every few seconds** — server-side diff result flips on Deployment istiod (client-side diff empty; live object unchanged since 08-24; self-heal syncs are no-ops). Needs investigation + docs/bugs spec. (found 2026-09-13)
- [ ] **Stuck op: `ubuntu-k3s-data-layer`** — sync operation Running since 2026-09-13 13:52Z "waiting for healthy state of /Namespace/shopping-cart-data"; app Synced/Healthy but op blocks future auto-sync. Clear via terminate-op (live; needs user go).
