...
- [ ] WS3 `_install_hermes_agent` (lib-foundation) — **SPEC WRITTEN 2026-09-05** (filled the stub in `docs/plans/v1.29.0-hermes-ws0-ws3-access-and-installer.md`, no new plan doc). `_install_hermes_agent`/`_uninstall_hermes_agent` in lib-foundation `scripts/lib/system.sh` (read-only Keychain preflight of 3 WS0 creds, never mints); new consumer `scripts/etc/launchd/com.k3d-manager.hermes.plist.tmpl` (StartInterval=300, no KeepAlive, no secrets) + thin `bin/k3dm-hermes-setup` entrypoint + `K3DM_HERMES_JITTER` add to `bin/k3dm-hermes`. Heavyweight Claude-owned path: lib-foundation upstream edit → BATS + credential-test → PR (go) → tag → subtree pull → consumer files → verify. **lib-foundation UPSTREAM DONE — commit `dce8d2a` on `origin/feat/v0.4.15`** (`_install_hermes_agent`/`_uninstall_hermes_agent` in `system.sh` + 5 BATS + CHANGE.md); shellcheck-lib clean, BATS 129/129 green. **lib-foundation PR #46 OPEN, ALL GATES GREEN, AWAITING MERGE GO** (head `6dbdbb1`): CI success, `make credential-test PROVIDER=aws` passed, Copilot 4 findings all fixed + threads resolved (0 unresolved), full BATS 132/132, shellcheck-lib clean. NEVER auto-merge. On go: merge → tag v0.4.15 → subtree pull → k3d-manager consumer files (plist tmpl + `bin/k3dm-hermes-setup` + `K3DM_HERMES_JITTER`). Consumer files NOT yet written.
- [ ] WS4 `docs/guides/hermes.md` — pending, after WS3.
