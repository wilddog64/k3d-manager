...
- [ ] **Vault Preflight After Sleep** — OPEN. Spec `docs/bugs/2026-04-23-acg-up-vault-state-preflight-gap-after-mac-sleep.md`; impl spec added 2026-04-24 (OrbStack restart mechanism confirmed). Two insertions in `bin/acg-up`: Hub cluster check (Step 3.5) + Vault seal check (after Step 4 PF start).
- [ ] **acg-extend isPanelOpen false positive** — OPEN. Spec `docs/bugs/2026-04-24-acg-extend-ispanelopen-false-positive.md`; `isPanelOpen` uses "Auto Shutdown" text which is visible on listing-page card — fix: `isPanelOpen = clicked`; also make Open Sandbox click target running-sandbox card.
- [ ] **acg-sync-apps app not found** — OPEN. Spec `docs/bugs/2026-04-24-acg-sync-apps-argocd-app-not-found.md`; list available apps via `argocd app list --output name` before exiting.
- [ ] **acg-down provider dispatch** — OPEN. Spec `docs/bugs/2026-04-24-acg-down-provider-dispatch-gcp-teardown.md`; `bin/acg-down` is AWS-only; needs provider dispatch to call `destroy_cluster --confirm` (already in `k3s-gcp.sh`) for GCP.
