...
- [x] **GCP stale auth `invalid_grant` — revoke before re-auth** — PARTIAL (`b28b8925`). Spec `docs/bugs/v1.1.0-bugfix-gcp-stale-auth-invalid-grant.md`. `_gcp_load_credentials` now revokes all stale accounts before re-extraction. Status describe guard used `gcloud auth list` which does not validate tokens — regression tracked separately.
- [ ] **GCP status describe — `gcloud auth list` check does not validate tokens** — OPEN. Spec `docs/bugs/v1.1.0-bugfix-gcp-status-token-validity-check.md`. `gcloud auth list --filter="status:ACTIVE"` returns stale deleted account as ACTIVE; fix: use `gcloud auth print-access-token` to test actual token validity. Assigned to Codex.
