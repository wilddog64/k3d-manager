...
- [ ] Hub ArgoCD foreign chart ownership (`docs/bugs/2026-09-13-hub-argocd-foreign-chart-ownership-blocks-helm-upgrade.md`) — Codex `198584eb` VERIFIED by Claude (shellcheck 0, BATS 38/38, 4 spec files; Claude pushed — Codex sandbox SSH push denied); live `--confirm` + helm upgrade = operator go
- [ ] Hub federate-acg self-scrape duplicates (`docs/bugs/2026-09-13-hub-federate-acg-self-scrape-duplicates-series.md`) — spec'd 2026-09-13 (+S2b `b4ae72e9`: existing `argocd_metrics_servicemonitor.bats` pinned old exprs — Codex correctly stopped), Codex `22feff51` VERIFIED by Claude (yq ok, BATS 27/27, 5 spec files; Claude pushed); live observability AppSet reapply (scoped) + dashboard apply = operator go
