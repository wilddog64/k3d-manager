...
- [x] **acg-sync-apps error log preserved in scratch** — COMPLETE (`890ba2a6`). Spec `docs/bugs/2026-04-24-acg-sync-apps-error-log-preserved-in-scratch.md`; `bin/acg-sync-apps` now writes the failure log under `./scratch/logs/` and keeps it on non-zero exit while still cleaning up stale state metadata.
