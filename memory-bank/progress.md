...
| Allow timeout: late account chooser not handled before Allow wait | OPEN | `docs/bugs/v1.1.0-bugfix-gcp-login-late-account-chooser.md`; unconditional chooser check + Continue needed immediately before Allow |
| Live smoke test `make up CLUSTER_PROVIDER=k3s-gcp GHCR_PAT=<pat>` | PENDING | After late account chooser fix |
