...
- [ ] **acg-sync-apps port-forward hidden failure** — OPEN. Spec `docs/bugs/2026-04-24-acg-sync-apps-port-forward-hidden-failure.md`; `bin/acg-sync-apps` still hides the background `kubectl port-forward` stderr, so readiness timeouts do not show the actual failure cause yet.
