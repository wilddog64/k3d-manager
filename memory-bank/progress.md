...
- [ ] **Bug: `bin/smoke-test-cluster-health` stale defaults** — APP_CONTEXT `ubuntu-k3s` + pre-rename app names; exits 1 silently at ArgoCD step. Spec `docs/bugs/2026-09-13-smoke-test-cluster-health-stale-defaults-silent-exit.md` DISPATCHED to Codex (chain F) 2026-09-13; run 1 halted on spec conflict (`:-` vs empty override), S2 fixed to `-`, RE-DISPATCHED. (found 2026-09-13 live smoke)
