...
- [ ] **acg-sync-apps ArgoCD login still interactive** — OPEN. Bug `docs/bugs/2026-04-24-acg-sync-apps-argocd-login-noninteractive.md`; `bin/acg-sync-apps` still calls `argocd login` without the non-interactive flags already used by `_argocd_ensure_logged_in()`, so `make sync-apps` can die after the port-forward is ready.
