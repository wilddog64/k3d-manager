...
- [ ] **acg-sync-apps local port 8080 collision** — OPEN. Spec `docs/bugs/2026-04-24-acg-sync-apps-local-port-8080-collision.md`; `bin/acg-sync-apps` still lacks a preflight check for a listener already bound to 8080, so the user only sees the failure after starting port-forward.
