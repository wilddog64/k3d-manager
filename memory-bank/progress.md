...
- [ ] **Hub post-rebuild red items (2026-09-13)** — DONE: Loki values fix `e047a718`; grafana PF plist regenerated (reload owed); declarative-recovery spec filed (decisions open). Grafana PF reloaded (user) → login 200, make status WARN/0 errors. OWED (operator): `shopping-cart-identity` sync w/ hook; istio-ambient k3d CNI paths reapplied → 4/4 Ready + HBONE (DONE; code default needs spec); DECISION: seed or drop `platform-ops/app-cluster-hostinger`.
