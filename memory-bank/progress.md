...
- [ ] Verify ArgoCD: all 5 apps Synced + Healthy on Ubuntu k3s — **Blocked (2 fixes in flight)**:
  - 401 fix: `ghcr-pull-secret` patch — `fix/argocd-image-pull` branch, commit `f1e12b1` (infra) — pending PR + merge
  - Arch fix: amd64 images on arm64 k3s — **Codex**: add `platforms: linux/amd64,linux/arm64` to push step in `build-push-deploy.yml` — spec: `shopping-cart-infra/docs/issues/2026-03-17-multi-arch-ci-builds.md`; branch: `fix/multi-arch-ci-builds`
