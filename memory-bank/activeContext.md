...
- **Remove isolated profile fallback; require CDP** — OPEN (`docs/bugs/v1.1.0-bugfix-acg-credentials-remove-isolated-fallback.md`). `launchPersistentContext` fallback opens a new window demanding re-login when user already has a valid session in their real Chrome. Fix: throw actionable error when CDP fails; remove `launchPersistentContext` block entirely.
