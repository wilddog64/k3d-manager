...
- **Spec written + committed `de94a1c9`** → `docs/bugs/2026-07-24-trivy-reconcile-panel-query-matches-zero-lines.md`. Fix = filter on `level="error"` plus a `line_format` using the json parser's **real** field names. Claude verified the exact `line_format` against live Loki before writing it: LogQL renames `job`→`.job_extracted` and `container`→`.container_extracted` (stream-label collisions) and `status.reason`→`.status_reason` (dot invalid in a label), so the old `line_format`'s `.namespace`/`.name`/`.controller`/`.error` referenced fields that never existed — it would have rendered blank even on a matching line. Proven render: `reconciler.scan job: Scan job container job=trivy-system/scan-vulnerabilityreport-dd796cffd container=grafana reason=Error`.
