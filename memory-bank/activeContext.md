...
## OPEN blockers
1. **istio-cni conf/bin dir mismatch — NOW THE NEXT BLOCKER (live-confirmed 2026-07-19).** After `896d08ad`, the appset loop now derives `APP_CLUSTER_NAME` from the resolved active cluster (`ubuntu-k3s` on k3s-aws, `ubuntu-hostinger` on hostinger) instead of hardcoding `ubuntu-hostinger`; the destination-validation blocker is therefore fixed in git and the next remaining blocker is ztunnel `ContainerCreating`: `failed to find plugin "istio-cni" in path [/var/lib/rancher/k3s/data/cni]` (chart pins rancher paths; Cilium/k3s use `/etc/cni/net.d` + `/opt/cni/bin`). Already fully specced: `docs/bugs/2026-07-17-ambient-istio-cni-conf-bin-dir-mismatch.md` (fix lines 34-36 of istio-ambient.yaml → `/etc/cni/net.d` + `/opt/cni/bin`; spec hardened `67b48935` with Before-You-Start). Repo file still unpatched. **ASSIGNED to Codex 2026-07-19** (handed off — last blocker before ambient dataplane green on k3s-aws, then hostinger rebuild).
