...
- **IN PROGRESS:** `fix(acg-up)` follow-up — generic `open` still leaves Safari on an old localhost session in some cases, so the browser handoff now tries to drive Safari directly with `osascript` before falling back. Issue doc: `docs/issues/2026-05-12-argocd-safari-stale-localhost-tab-keeps-reopening.md`.
