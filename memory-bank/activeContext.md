...
- **Flagged for owner, NOT in Round 2 scope:** `_argocd_configure_post_deploy` is dead code (zero callers). Live path now covers image-updater/appproject/appsets; only `_argocd_configure_vault_eso` + Istio VirtualService live ONLY in this function. **Live-hub check 2026-07-23: no argocd VirtualService, no ESO in cicd (UI=port-forward, admin=`argocd-initial-admin-secret`)** → both orphaned relative to how the hub runs; deletion is low-risk. Decision options captured in `progress.md` PENDING.
...
- **v1.17.0 specs filed 2026-07-23 (`46524697`, both `docs/bugs/`, unassigned):** make-status login verification (`_smoke_test_logins` in webhook) + ambient cilium `ssh_cmd` string→array (Copilot PR #106 carry-forward). Each = its own Codex session.
