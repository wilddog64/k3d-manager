...
## Hub Grafana "ArgoCD Apps & Image Updater Hub" No-data — LIVE-FIXED 2026-07-23, durable spec assigned
...
- **Durable spec (Claude's job, docs/bugs):** `docs/bugs/2026-07-23-hub-argocd-servicemonitors-and-image-updater-not-enrolled.md` for Codex — idempotent "ensure argocd ServiceMonitors" step (render with `--api-versions`, apply, guarded on SM CRD; avoids the argocd-cm conflict) + ensure image-updater + `_argocd_ensure_ghcr_pull_secret` run in the standard hub bootstrap.
