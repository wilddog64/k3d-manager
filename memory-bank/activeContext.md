...
**acg_extend logic refinement (v1.0.4):** IN PROGRESS. Identified two edge cases: (1) Midnight "Auto Shutdown" causes negative TTL calculation; (2) Pre-opened modals block the script's navigation/clicking flow. Fix: Invert logic to check for button first and add date-wrap detection to TTL parsing. Issue: `docs/issues/2026-04-08-acg-extend-midnight-and-modal-trapping.md`.
