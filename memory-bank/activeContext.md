...
- **Live follow-up 2026-09-16 — failure-groups table still empty.** `make platform-ops` applied the exporter/dashboard manifests, but retained ConfigMaps predate groups. A backfill of run `1789549631-2079` was accepted by the API; the live exporter still emitted no `e2e_failure_group_info` and exposed that item with metadata fallback (`run_id=e2e-result-m2-d40431a29478`, empty dimensions). Issue: `docs/issues/2026-09-16-live-e2e-failure-groups-empty.md`. Latest sidecar groups: cart 11, cross-service 10, payments 9, orders 2, products 1.
