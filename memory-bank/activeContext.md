...
- **NEW FINDING:** Live LDAP bind still fails because Keycloak is configured with `LDAP_BIND_DN: cn=admin,dc=shopping-cart,dc=local` while the live LDAP admin secret exposes `ldap-admin`. This mismatch blocks SSO before any credential can succeed. Issue doc: `docs/issues/2026-05-12-shopping-cart-infra-keycloak-ldap-bind-dn-mismatch.md`.
