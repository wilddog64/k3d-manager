...
- **acg-sync-apps + acg-status dual-cluster** — SPEC READY (`docs/bugs/2026-04-23-acg-sync-apps-and-acg-status-dual-cluster.md`). Fix: poll port-forward readiness (replace `sleep 3`); make ArgoCD app name configurable via `ARGOCD_APP`; add Hub cluster section to `acg-status`. Assigned to Codex 2026-04-23.
