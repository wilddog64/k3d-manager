...
| Split GCP `make up` to cluster-only; move full stack to `make provision` | COMPLETE (`c054c7b4`) | `docs/bugs/v1.1.0-bugfix-makefile-gcp-up-provision-split.md`; `make up CLUSTER_PROVIDER=k3s-gcp` now runs only `deploy_cluster`, while `make provision CLUSTER_PROVIDER=k3s-gcp` runs `make up` then `gcp_provision_stack` |
...
| Wait for Cloudflare bot-check redirect before sandbox button search | COMPLETE (`ff27b6ea`) | `docs/bugs/v1.1.0-bugfix-acg-credentials-cloudflare-challenge.md`; `acg_credentials.js` now detects the Cloudflare "Checking your browser before accessing" page, waits for it to disappear, then waits for `domcontentloaded` before sandbox-button lookup |
| Add URL/title/button/screenshot diagnostics on sandbox-button wait timeout | COMPLETE (`810bfbf2`) | `docs/bugs/v1.1.0-bugfix-acg-credentials-button-wait-diagnostics.md`; `acg_credentials.js` now logs URL/title/visible buttons and saves a screenshot when the 30s button-wait times out |
| Detect Pluralsight `/id` redirect and throw actionable session-expiry error | COMPLETE (`413e90b2`) | `docs/bugs/v1.1.0-bugfix-acg-credentials-session-expiry-detection.md`; `acg_credentials.js` now pauses 1s after the aria-busy wait and throws a clear session-expiry error when Pluralsight redirects to `/id` |
| Wait for manual sign-in at `/id` instead of throwing immediately | COMPLETE (`41f573e5`) | `docs/bugs/v1.1.0-bugfix-acg-credentials-id-redirect-wait-for-login.md`; `acg_credentials.js` now waits up to 300s for the user to complete `/id` sign-in, then re-navigates to the sandbox page and re-waits for aria-busy |
| Replace `/id` waitForURL predicate with polling loop | COMPLETE (`528bc810`) | `docs/bugs/v1.1.0-bugfix-acg-credentials-id-poll-loop.md`; `acg_credentials.js` now polls `page.url()` every 2s for up to 300s instead of using `waitForURL`, which was not waiting correctly on an already-idle `/id` page |
| `/id` manual-login path times out at 120s outside Playwright | COMPLETE (`235df03e`) | `OVERALL_TIMEOUT_MS` was `IS_FIRST_RUN ? 300000 : 120000`; now fixed to `300000` unconditionally so the 300s `/id` poll loop can complete |
| Add text-based sandbox button fallback when `data-heap-id` selectors miss | COMPLETE (`f36d4826`) | `docs/bugs/v1.1.0-bugfix-acg-credentials-button-click-fallback.md`; `acg_credentials.js` now falls back to text-based Open/Start/Resume button clicks when no provider-specific `data-heap-id` buttons are visible |
...
- **Provider Make lifecycle inconsistency** — COMPLETE (`c054c7b4`). `make up CLUSTER_PROVIDER=k3s-gcp` now runs `deploy_cluster` only; `make provision CLUSTER_PROVIDER=k3s-gcp` runs cluster + full stack.
...
- **Playwright Stability Hardening** — COMPLETE (`5b6d030e`). Refactored `acg_credentials.js` to fix destructive cleanup, handle cold starts, and implement IDP safety guards.
- **ACG CDP/session mismatch** — LARGELY RESOLVED (`5b6d030e`). Issue `docs/bugs/2026-04-19-acg-credentials-current-blocker-cdp-session-mismatch.md`; unified orchestration resolves the main attachability mismatch, but observability/classification gaps remain open below.
- **ACG CDP/session contract** — MOSTLY COMPLETE (`5b6d030e`). Added unified Chrome/CDP helper in `scripts/plugins/acg.sh`; `gcp.sh` now delegates to it, but the shell/Playwright error-state contract still needs finer-grained observability.
- **ACG Credentials Observability Gap** — COMPLETE (`8978c2f4`). Granular state machine and reserved exit codes implemented in acg_credentials.js; acg.sh surfaces specific error modes.
- **GCP Identity Mismatch (SSH Blocked)** — OPEN (`docs/bugs/2026-04-19-gcp-identity-mismatch-ssh-blocked.md`). CLI is left authenticated as the restricted SA instead of `cloud_user`; prevents SSH access and instance management.
- **Fix CDP context reuse + process.exit/finally** — COMPLETE (`13194e57`).
 `docs/bugs/v1.1.0-bugfix-acg-credentials-cdp-and-singleton.md`; `browserContext` now set from CDP whenever Chrome is running and `throw error` in catch ensures `finally` runs. (Separate bug: `close()` in finally must become `disconnect()` — tracked below.)
- **CDP disconnect fix** — COMPLETE (`ecec0ff8`) | `docs/bugs/v1.1.0-bugfix-acg-credentials-cdp-disconnect.md`; both `_cdpBrowser.close()` calls in `scripts/playwright/acg_credentials.js` now use `_cdpBrowser.disconnect()` so CDP cleanup detaches without terminating the user's Chrome session.
- **Tab hijack + silent CDP failure** — COMPLETE (`75aa65a8`) | `docs/bugs/v1.1.0-bugfix-acg-credentials-newpage-and-cdp-log.md`; `scripts/playwright/acg_credentials.js` now logs CDP fallback explicitly and opens a fresh page with `context.newPage()` instead of hijacking `allPages[0]`.
- **CDP guidance log** — SUPERSEDED (`bf00be14`). Isolated profile fallback is wrong behavior and must be removed entirely — see next entry.
- **Remove isolated profile fallback; require CDP** — COMPLETE (`014f7df5`) | `docs/bugs/v1.1.0-bugfix-acg-credentials-remove-isolated-fallback.md`; `scripts/playwright/acg_credentials.js` now requires CDP, throws actionable errors when Chrome is not started with `--remote-debugging-port=9222`, and no longer falls back to `launchPersistentContext`.
- **`gcp_get_credentials` missing CDP auto-launch** — SUPERSEDED (`0bafdb55`). Used `pkill -x "Google Chrome"` which kills user's entire Chrome session. Replaced by entry below.
- **`gcp_get_credentials` CDP launch kills Chrome session** — SUPERSEDED (`a025f41a`). Erroring when Chrome runs without CDP breaks `make up` automation. Replaced by entry below.
- **`gcp_get_credentials` CDP warn + restart** — SUPERSEDED (`1b038b52`). Auto-restart kills user's session and times out; wrong approach.
- **`gcp_get_credentials` CDP no auto-restart** — COMPLETE (`7cf8fc6f`) | `docs/bugs/v1.1.0-bugfix-gcp-cdp-no-auto-restart.md`; `scripts/plugins/gcp.sh` now prints exact relaunch instructions and returns 1 when Chrome is already running without CDP, while still launching Chrome with CDP when it is not running.
- **macOS Chrome CDP relaunch timeout** — RESOLVED (`7cf8fc6f`) | `docs/bugs/2026-04-19-gcp-cdp-relaunch-timeout-macos.md`; no-auto-restart behavior removes the unreliable macOS kill/relaunch path entirely, so this timeout is no longer the active failure mode.
- **`_gcp_load_credentials` stale user auth triggers false Playwright re-run** — COMPLETE (`274a5989`) | `docs/bugs/v1.1.0-bugfix-gcp-sa-key-validation.md`; `scripts/lib/providers/k3s-gcp.sh` now activates the cached SA key before `gcloud projects describe`, avoiding false Playwright re-extraction when user auth is stale.
