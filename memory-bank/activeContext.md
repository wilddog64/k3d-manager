...
- **OPEN Bug 6 — LDAP still CrashLoopBackOff** — Bug 5 fix is wrong. emptyDir at `ldif/custom` is still a kernel mount point; `rm -rf assets/config` descends into it and fails with EBUSY. Fix: set `LDAP_SEED_INTERNAL_LDIF_PATH=/ldif-staging`, mount emptyDir at `/ldif-staging` instead. Spec: `docs/bugs/2026-05-09-ldap-emptydir-mountpoint-rm-fails.md`. Branch: `shopping-cart-infra fix/ldap-ldif-staging`.
