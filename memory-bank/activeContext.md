...
- **Spec 5 (Keycloak/Frontend login) — ASSIGNED to Codex, spec written 2026-07-23:** `docs/bugs/2026-07-23-smoke-login-keycloak-smoke-user-seed.md` (Path 1, owner-chosen). Live read-only verify of the `shopping-cart` realm found the blocker that killed naive B1: the `frontend` client has `directAccessGrantsEnabled=false` (public SPA, auth-code+PKCE only) so `grant_type=password` can NEVER succeed against it — the only DAG-enabled clients are `admin-cli`/`order-service`/`product-catalog`. Path 1 = k3d-manager seeds its OWN dedicated `k3dm-smoke` client (public, DAG on) + local user in the app realm via master-admin token (`identity/keycloak-secrets` keys `KEYCLOAK_ADMIN`/`KEYCLOAK_ADMIN_PASSWORD`), stores a random `openssl rand -hex 24` password in plain Secret `identity/k3dm-smoke-user` (keys `username`/`password`), webhook falls back to it with `client_id=k3dm-smoke`. Two files: new public `keycloak_seed_smoke_user` in `scripts/plugins/keycloak.sh` (idempotent, warns+returns 0 if Keycloak/realm absent, all secrets via temp files — no argv leak) + `bin/k3dm-webhook` Keycloak block fallback and a `kc_via_smoke_client` guard so **Frontend login stays ⚪** (smoke-client token isn't scoped for the frontend `/api/cart` audience — avoids a false red). Commit message (exact): `feat(keycloak): seed dedicated smoke user for real Keycloak login check`. NOT yet handed to Codex / not implemented — spec + this memory-bank entry only. Known limitation noted in spec: app-owned realm-reconcile could wipe `k3dm-smoke`; re-run the seed to restore.
