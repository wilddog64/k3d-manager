...
- **2026-09-14 — v1.34.0 items 1–6 spec'd (user: "spec one to six then drive codex")** — (1) `docs/bugs/2026-09-14-acg-lock-acquire-missing-state-dir-hangs.md` (root cause reproduced with `bash -x`: lock mkdir without parent spins 600s); (2) `docs/bugs/2026-09-14-status-hub-eso-unmonitored.md`; (3) no spec — already DONE `b71e587a`, stale Status flipped; (4) `docs/bugs/2026-09-14-applicationset-reapply-single-app-cluster-name.md`; (5) `docs/bugs/2026-09-14-bats-bare-negation-assertions-never-fail.md`; (6) `docs/bugs/2026-09-14-prometheus-port-19090-hub-acg-collision.md` (decision: hub keeps 19090, app-cluster → 19190+offset). Codex order: 1 → 2 → 5 → 4 → 6 → 7 (hub ServiceMonitors spec), sequential, commit-only; Claude verifies + pushes each. Items 8 (`make observability`) = operator; 9 (CVE scan runs) = Claude read-only check → argocd-cve-scan broken (trivy image lacks kubectl/curl), spec'd as Codex #7. Progress: #1 `2f2474e9` verified; #2 `3eeaef5b` verified (Claude committed; Codex blocked only on pytest under py3.14 — use pyenv python3.13). Chain resumes at #3.
