...
- **COMPLETE:** Argo CD canonical HTTPS hostname is now backed by a local listener. `bin/acg-up` installs a root-owned `launchd` daemon that forwards `argocd.shopping-cart.local:443` to the existing `localhost:8080` Argo CD port-forward, and `bin/acg-down` removes it. Spec: `docs/issues/2026-05-11-argocd-canonical-https-listener-missing.md` (fixed).
