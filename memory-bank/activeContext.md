...
**App namespace ExternalSecrets** — ASSIGNED to Codex. Spec: `shopping-cart-infra/docs/plans/v0.2.1-bugfix-app-namespace-secrets.md`. Create new ExternalSecrets in `shopping-cart-apps` for redis/cart, redis/orders-cache, order-service, and product-catalog to mirror Vault KV paths.
