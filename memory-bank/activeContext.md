...
- **order-service ddl-auto + health probe fix** — COMPLETE (`9db9589c`, `2b1215e2`, `445b7696`). Kustomize patches: (1) `SPRING_JPA_HIBERNATE_DDL_AUTO=update` on `order-service-config`; (2) readiness/liveness/startup probes patched to `/actuator/health`; (3) `SPRING_RABBITMQ_USERNAME/PASSWORD` env vars added so Spring AMQP uses the correct credentials. Permanent fix needed: update init SQL + SecurityConfig in shopping-cart repos. Spec: `docs/bugs/2026-04-27-orders-init-sql-serial-vs-uuid.md`.
