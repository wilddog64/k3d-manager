...
- **COMPLETE:** `fix(acg-up)` — browser-side `keycloak.shopping-cart.local` is now rewritten to the ingress gateway IP instead of loopback, fixing the Safari “can’t connect to server” redirect path. Commit `99581cc7`. Issue doc: `docs/issues/2026-05-12-keycloak-browser-connectivity-loopback.md`.
