...
> - **PAYMENT GHCR BLOCKER SPEC WRITTEN 2026-08-15 (clear before spending a TTL window):** `docs/bugs/v1.25.0-bugfix-payment-ghcr-externalsecret.md`. Root cause = TWO defects: (1) `ghcr-pull-secret` is created imperatively (`shopping_cart.sh:349-388`), not in git → `services-git` `prune:true` prunes it; (2) the bring-up patches only the **`default`** SA but payment runs as **`payment-service`** SA (no imagePullSecrets). Fix (all in `shopping-cart-payment/k8s/base`, branch `fix/payment-ghcr-eso-pull-secret`): NEW `externalsecret-ghcr.yaml` (ESO dockerconfigjson from Vault `secret/data/github/pat`, `vault-backend` CSS, `creationPolicy:Owner`, sync-wave -1) + add `imagePullSecrets:[ghcr-pull-secret]` to `payment-service` SA + list it in kustomization. Commit msg `fix(k8s): git/ESO-owned ghcr pull secret on payment-service SA`. NOT handed off yet. Follow-up (k3d-manager, out of scope): trim the now-redundant imperative payment-ns branch to avoid ESO-vs-imperative tug-of-war.
