...
- **PARTIAL:** `fix(identity-eso-bootstrap)` — spec `docs/bugs/v1.4.5-bugfix-identity-externalsecret-bootstrap.md`. Bug 1 COMPLETE (`f969b299`): `_eso_apply_vault_cluster_store` added to `eso.sh`; called from `deploy_ldap()` via `ldap.sh` source guard. Bugs 2+3 OPEN (shopping-cart-infra, branch `fix/identity-externalsecret-bootstrap`): (2) `keycloak-secrets` ExternalSecret template drops Vault-fetched keys; (3) `ldap-data-pvc` WaitForFirstConsumer deadlock with ArgoCD sync waves.
