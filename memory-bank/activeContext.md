...
- **OPEN:** Argo CD canonical HTTPS hostname is not backed by a local listener. Safari cannot connect to `https://argocd.shopping-cart.local` because only the `localhost:8080` port-forward is bound. Spec: `docs/issues/2026-05-11-argocd-canonical-https-listener-missing.md`.
