...
- **GCP status describe token validity regression** — OPEN (`docs/bugs/v1.1.0-bugfix-gcp-status-token-validity-check.md`). `b28b8925` used `gcloud auth list` to guard the describe call, but `auth list` returns stale deleted accounts as ACTIVE — it does not validate tokens. Fix: replace with `gcloud auth print-access-token 2>/dev/null` which returns empty on `invalid_grant`. Assigned to Codex.
