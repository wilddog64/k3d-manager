...
- **`gcp_get_credentials` missing CDP auto-launch** — COMPLETE (`0bafdb55`) | `docs/bugs/v1.1.0-bugfix-gcp-get-credentials-cdp-auto-launch.md`; `scripts/plugins/gcp.sh` now checks CDP before invoking Playwright, auto-launches Chrome on port 9222 with the user's default profile, and waits up to 15s for CDP readiness.
