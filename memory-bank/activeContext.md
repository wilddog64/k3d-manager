...
- **v1.31.0 PR #122 OPEN 2026-09-07** (https://github.com/wilddog64/k3d-manager/pull/122, base `main` ← `k3d-manager-v1.31.0`). Bundles `preflight` tool + token-expiry advisory + R4 App-auth revert + v1.30.0 ledger backfill. CI green on head `30122821`. **Copilot 2 findings both fixed (`30122821`):** preflight `per_page=1`→`30` (verdict was bound to the newest run); write-scope probe now requires non-empty output (empty/transport error no longer misread as write=true; rc is not the gate because a write-capable PAT still gets non-zero rc on `rerun-failed-jobs` against a successful run). +2 tests → 33 passed. Both Copilot threads resolved. **CodeQL 1 finding = FALSE POSITIVE** (`bin/k3dm-hermes:128` — `report` is booleans+`"pat"`, token only in subprocess env; non-blocking, CodeQL not a required check); thread left open + replied, accept-vs-dismiss surfaced to user. Findings: `docs/issues/2026-09-07-copilot-pr122-review-findings.md`. Ready to merge once enforce_admins disabled — **NEVER auto-merge**, user merges.
