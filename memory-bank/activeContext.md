...
- **CDP disconnect fix** — OPEN (`docs/bugs/v1.1.0-bugfix-acg-credentials-cdp-disconnect.md`). Both `_cdpBrowser.close()` calls (lines 163, 433) must become `_cdpBrowser.disconnect()` — `.close()` on CDP sessions tries to kill the user's Chrome and hangs for 300s on macOS when other windows are open.
