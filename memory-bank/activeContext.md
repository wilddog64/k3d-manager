...
- **ACG Credentials Observability Gap** — OPEN (`docs/bugs/2026-04-19-acg-credentials-observability-gap.md`). Implementation is still coarse; shell layer is blind to specific Node.js failure modes because `acg_credentials.js` exits with code `1` for distinct session-contract failures.
- **Fix CDP context reuse + process.exit/finally** — COMPLETE (`13194e57`). `docs/bugs/v1.1.0-bugfix-acg-credentials-cdp-and-singleton.md`; `browserContext` now set from CDP whenever Chrome is running and `throw error` in catch ensures `finally` runs. (Separate bug: `close()` in finally must become `disconnect()` — tracked below.)
