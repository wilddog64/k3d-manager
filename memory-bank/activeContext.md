...
- **Stale kubeconfig merge bugfix** — SPECCED → Codex (`docs/bugs/v1.1.0-bugfix-gcp-kubeconfig-stale-merge.md`). IP change causes old k3s-gcp context/cluster/user in `~/.kube/config` to override fresh entries from `k3s-gcp.yaml`. Fix: delete stale entries before flatten merge.
- **k3s API server readiness probe** — SPECCED → Codex (`docs/bugs/v1.1.0-bugfix-gcp-k3s-api-readiness-probe.md`). `kubectl label` fires before API server is ready on port 6443. Fix: `nc -z` probe loop (10s backoff, 30 retries) between kubeconfig merge and `kubectl label`.
- **make provision depends on make up** — SPECCED → Codex (`docs/bugs/v1.1.0-bugfix-gcp-provision-depends-on-up.md`). `make provision CLUSTER_PROVIDER=k3s-gcp` has no dependency on `make up`. Fix: call `$(MAKE) up` before `gcp_provision_stack` in the k3s-gcp provision case.
