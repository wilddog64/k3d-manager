...
**shopping-cart-order PR #25 OPEN** (`docs/next-improvements`). Fix pre-existing CI failure: `trivy-action@0.30.0` (invalid) → `@v0.35.0`. **This PR unblocks the `Build, Scan & Push` publish job** — after merge, CI builds new image with `rabbitmq-client 1.0.1` and ArgoCD auto-deploys to resolve `order-service` CrashLoopBackOff. CI run 24289805033 in progress.
