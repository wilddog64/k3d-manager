...
- **OPEN ISSUE:** Keycloak admin token fetch currently returns `401 invalid_grant` against the rebuilt cluster because `bin/acg-up` reseeded `secret/keycloak/admin` instead of preserving the existing Vault password. Spec: `docs/bugs/2026-05-11-keycloak-admin-password-reseed-on-rebuild.md` and `docs/issues/2026-05-11-keycloak-admin-token-reconciliation-failure.md`.
