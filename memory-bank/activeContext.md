...
- **OPEN:** `fix(identity-eso-bootstrap)` — spec `docs/bugs/v1.4.5-bugfix-identity-externalsecret-bootstrap.md`. Three bugs: (1) `ClusterSecretStore/vault-backend` never provisioned by k3d-manager (k3d-manager fix in `eso.sh`); (2) `keycloak-secrets` ExternalSecret template drops Vault-fetched keys (shopping-cart-infra fix); (3) `ldap-data-pvc` WaitForFirstConsumer deadlock with ArgoCD sync waves (shopping-cart-infra fix). Cluster hotfixes applied; not permanent.
