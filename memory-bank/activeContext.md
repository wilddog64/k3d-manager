...
- **2026-09-15 — first honest `make status` run: 3 reds, both root causes found (live, read-only)**
  ...
  2. **All shopping-cart SSO broken (frontend + ArgoCD)** — NOT a credential problem: Vault passwords bind against openldap-0 for all 3 users, and Keycloak direct-grant returns 200. Keycloak federates openldap-0 correctly, so infra PRs #96/#97 landed and the 2026-09-04 osixia issue doc is STALE. Real cause: the shopping-cart-infra reconcile hook adds `otp-conditional-subflow` as a TOP-LEVEL CONDITIONAL execution beside the ALTERNATIVE `forms` subflow, so Keycloak ignores `forms` and Username Password Form never runs → HTTP 400 "We are sorry... Invalid username or password." before any login form. Regression of v1.4.11, which was only ever hand-fixed live. Spec `docs/bugs/2026-09-15-keycloak-browser-flow-regression-blocks-all-sso.md`; FIX COMMITTED `c3a2c37` on shopping-cart-infra branch `fix/keycloak-browser-flow-top-level-otp` (pushed; **PR #98 OPEN** https://github.com/wilddog64/shopping-cart-infra/pull/98 — CI green on `78403f0` (Kubeconform/Kustomize Build/YAML Lint/GitGuardian); Copilot reviewed anyway despite the reviewer-request API no-opping (both `-f 'reviewers[]='` and raw-JSON `--input -` return 200 with `requested_reviewers: []`) — 1 inline finding, that the level-0 guard conflated a wrong execution count with unexpected execution names and logged an empty name list, fixed in `78403f0`, replied and thread resolved; `enforce_admins` DISABLED on shopping-cart-infra main 2026-09-15 — RE-ENABLE with a bodyless POST after merge; awaiting user merge). Codex drafted it; Claude found and fixed SIX defects by checking the real kcadm contract live, read-only, against the broken realm (Keycloak is on the k3d-k3d-cluster context, ns `identity`, NOT hostinger): (a) `authentication/flows/<alias>/executions` returns a FLATTENED listing of all nested executions, so Codex's "exactly 4 level-0 rows" guard would have rejected a CORRECT flow and failed every sync — now filtered on the `level` field; (b) `--format csv` emits NO header row, so every `NR == 1 { next }` silently dropped the first real record (Cookie); (c) a copied flow PREFIXES nested subflow names, so the real alias is `browser-with-conditional-otp Browser - Conditional OTP`, not the bare stock name Codex matched; (d) an execution is deleted at `authentication/executions/<id>`, not `authentication/flows/<alias>/executions/<id>`, so the stray subflow would never have been removed; (e) `authentication/flows` lists ONLY top-level flows (`topLevel: true`), so subflow aliases cannot be resolved from it — derived from the display name and percent-encoded instead, because kcadm rejects unencoded spaces (`Illegal character in path`); (f) the role-condition config was read from the create-only `authentication/executions/<id>/config` endpoint, so a second run would 409 and abort under `set -e` — the config id now comes from the execution's `authenticationConfig` field. Verified with a mock-kcadm dry run seeded from the live broken realm: stray removed, level 0 ends as Cookie/Kerberos/IdP Redirector/forms, passes 2 and 3 idempotent, injected rogue level-0 execution fails loudly. NOT yet applied live — merging PR #98 is what makes ArgoCD sync `identity/keycloak/` and run the PostSync hook. LIVE 2026-09-15 `make status` now reports the SSO reds as `authorization request rejected (HTTP 400)` for admin/developer/operator (was "credentials rejected"), confirming the pre-login-form diagnosis and the landed classifier wording.
