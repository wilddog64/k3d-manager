...
- **`_gcp_load_credentials` stale user auth triggers false Playwright re-run** — OPEN | `docs/bugs/v1.1.0-bugfix-gcp-sa-key-validation.md`; activate SA key via `gcloud auth activate-service-account` before `gcloud projects describe` so expired user auth does not cause unnecessary Playwright re-extraction.
