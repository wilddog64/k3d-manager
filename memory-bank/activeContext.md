...
- **Playwright Stability Hardening** — SPECCED (`docs/plans/v1.1.0-playwright-stability-hardening.md`). Refactor `acg_credentials.js` to fix destructive cleanup (`disconnect()` vs `close()`), handle cold starts (patient polling for `/id` or `/hands-on`), and implement IDP safety guards (jitter, one-tab limit, Auth0 block detection).
