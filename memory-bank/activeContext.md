...
| Replace `waitForURL` predicate with polling loop for `/id` wait | OPEN | `docs/bugs/v1.1.0-bugfix-acg-credentials-id-poll-loop.md`; `waitForURL` function predicate rejects immediately when page is idle at `/id` — replace with 2s poll loop up to 300s |
