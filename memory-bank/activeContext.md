...
| `/id` manual-login path times out at 120s outside Playwright | OPEN (`docs/issues/2026-04-18-acg-credentials-id-wait-timeout-mismatch.md`) | `acg_credentials.js` waits 300s for `/id` sign-in, but the outer runner terminates the script after 120s, so the manual-login path cannot complete as designed |
