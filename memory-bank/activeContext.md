...
- **v1.31.0 WS2 — Hermes GitHub token-expiry advisory: SPEC'D + DISPATCHED TO CODEX 2026-09-07** (user go: "yes, and work with codex to get this completed"). Spec `docs/plans/v1.31.0-hermes-token-expiry-advisory.md`. Hermes posts a once-per-day Slack advisory when `k3dm-hermes-gh-token` is within a window (default 14d) of expiry, auto-silences once a no-expiration classic PAT is in the slot. **Reshaped by a verified correlator finding:** `Correlator.process` only fires on ≥2 degraded sensors (edge-triggered), so the expiry check is a STANDALONE advisory (direct `post_summary` + `state`-keyed daily dedup), NOT a correlator sensor — a naive sensor would be silently swallowed. This is the Hermes-native replacement for the (blocked) calendar/cron reminder for the 2026-12-04 PAT expiry. Awaiting Codex commit SHA on `origin/k3d-manager-v1.31.0`; verify independently (pytest 31, py_compile, SHA on origin) before trusting.
