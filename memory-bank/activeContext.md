...
- **⚠️ main is MISSING the provider-generic platform AppProject fix** (`5a3e6f48`: `platform.yaml.tmpl` `name: ubuntu-k3s`→`name: '*'` + `eso.yaml` destination + `scripts/tests/etc/argocd_platform_project.bats`). Lives only on `k3d-manager-v1.7.2`; preserved as tag **`salvage/platform-appproject-provider-generic`**. main still hardcodes `ubuntu-k3s` → blocks ESO Phase 2 on non-ubuntu (Hostinger) clusters. **Fold into the ESO Phase 2 spec** (it already flags the `argocd_platform_project.bats` runner-wiring gap) or land as a small standalone fix to main first. Also missing from main: `scripts/tests/lib/acg_session_check.bats`.
