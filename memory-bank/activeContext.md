...
- **2026-09-13 — Codex: `_signing_grant_eso_read` false-success fix** — spec `docs/bugs/2026-09-13-signing-grant-eso-read-false-success.md`; dispatched via `codex exec`. Claude verifies SHA on origin, scope (signing.sh, signing.bats, CHANGELOG), shellcheck, BATS. Operator: hostinger grant must run with context `ubuntu-hostinger`.
