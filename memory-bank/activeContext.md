...
- **COMPLETE:** `fix(acg-up)` — `bin/acg-up` now opens `https://argocd.shopping-cart.local` automatically on macOS after the browser listener is healthy, so stale localhost tabs do not remain the default entrypoint. Issue doc: `docs/issues/2026-05-12-acg-up-auto-opens-canonical-argocd-browser.md`.
