...
- **Frontend-login ⚪ is NOT a real limitation — live-disproved the "not scoped for frontend audience" assumption (2026-07-23).** The webhook (`bin/k3dm-webhook:1601-1602`) unconditionally skips the Frontend check when `kc_via_smoke_client` is True. Tested live on the hub: smoke token claims `aud=account`/`azp=k3dm-smoke`, and `GET http://frontend.shopping-cart.local/api/cart` **with** that token → **HTTP 200 real cart JSON**; **without** token → **HTTP 401**. The frontend authenticates the bearer but does NOT enforce `aud`, so the smoke login actually succeeds — the ⚪ throws away real end-to-end coverage. (`frontend` Keycloak client is `directAccessGrantsEnabled=false`, which is why the smoke uses the `k3dm-smoke` fallback in the first place.) Minor dev-only security note: `/api/cart` accepts `aud=account` instead of requiring `aud=frontend`. **Spec filed for Codex:** `docs/bugs/2026-07-23-smoke-frontend-login-attempt-not-skip.md` (single file `bin/k3dm-webhook`; attempt the request, grade 2xx→✅ / smoke-client 401|403→⚪ / else→❌; gates `python3 -m py_compile` + `_agent_audit`; Claude does `make restart-webhook` + live re-verify). NOT yet handed to Codex.
