...
**ESO ExternalSecret storeRef + Vault KV seeding bugfix** — SPECCED (2026-04-05). shopping-cart-data ExternalSecrets use `kind: SecretStore` (none exists — fix to `ClusterSecretStore`); postgres ExternalSecrets use Vault DB engine (`database/creds/…`) which isn't configured — fix to static KV; `bin/acg-up` must seed Vault KV. Multi-repo: shopping-cart-infra `fix/eso-externalsecret-storeref` + k3d-manager `k3d-manager-v1.0.3`. Spec: `docs/plans/v1.0.3-bugfix-eso-externalsecret-storeref.md`. ASSIGNED to Codex.
