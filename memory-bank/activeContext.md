...
## OPEN blockers
1. **istio-ambient dest validation — ROOT-CAUSED + SPECCED (2026-07-19).** `_argocd_deploy_applicationsets` (`argocd.sh:1165`) defaults `APP_CLUSTER_NAME` to `ubuntu-hostinger` (the outlier — `istio_ambient.sh:21` + `vars.sh:65` use `ubuntu-k3s`) and never feeds the already-resolved `_active_app_cluster` back into it, so on a k3s-aws hub istio-ambient + observability-acg (the only appsets using `${APP_CLUSTER_NAME}` as `destination.name`) render against nonexistent `ubuntu-hostinger` → `no clusters with this name`. Note `deploy_istio_ambient` is NOT called in the up flow; the generic `find` loop applies the appset. **Live PROOF:** re-applying via `deploy_istio_ambient --confirm` (renders ubuntu-k3s) cleared the error (`ErrorOccurred=False`), generated all 4 Applications (dest=ubuntu-k3s), **istiod 1/1 Running** on spoke (= `1af15217` istiod right-size pod-level verify PASS) + istio-cni-node 1/1 ×3. Spec: `docs/bugs/2026-07-19-argocd-appset-loop-app-cluster-name-default.md` (fix: derive `APP_CLUSTER_NAME` from `_active_app_cluster`, fallback `ubuntu-k3s`). **UNASSIGNED — ready for Codex.**
2. **istio-cni conf/bin dir mismatch — NOW THE NEXT BLOCKER (live-confirmed 2026-07-19).** After dest-validation clears, ztunnel stays `ContainerCreating`: `failed to find plugin "istio-cni" in path [/var/lib/rancher/k3s/data/cni]` (chart pins rancher paths; Cilium/k3s use `/etc/cni/net.d` + `/opt/cni/bin`). Already fully specced: `docs/bugs/2026-07-17-ambient-istio-cni-conf-bin-dir-mismatch.md` (fix lines 34-36 of istio-ambient.yaml). Repo file still unpatched. **UNASSIGNED.**
