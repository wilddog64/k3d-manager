...
- **v1.17.0 release PR [#107](https://github.com/wilddog64/k3d-manager/pull/107) is OPEN and green, awaiting an approval (2026-07-24).** Stamped `## [1.17.0] - 2026-07-24` (`c53cafbd`); all checks pass on `0e6c6b15`; Copilot's single finding fixed in `3be18f1d` and its thread resolved; `enforce_admins` disabled. **Two reusable lessons from this run:** (1) **`.gitguardian.yaml` does NOT clear the PR check** — `ignored_matches` suppress the *local* ggshield CLI only; the server-side incidents stay OPEN and keep the GitGuardian check red, so every FP needs BOTH the committed ignore entry AND a `POST /v1/incidents/secrets/<id>/ignore` (`{"ignore_reason":"false_positive"}`, token in Keychain service `ggshield`). Cleared 35143325/35142552/35142551 this run; check flipped fail→pass. (2) **Copilot findings need verification, not compliance** — it flagged `base64 -d` as "GNU-only" and prescribed `--decode || -D`; measured, stock macOS `base64 -d` works fine AND `-D` is *invalid under GNU coreutils*, so its remedy would have broken Linux, and the `||` arm is unsound in a pipeline (fallback gets no stdin). `--decode` alone is portable to both. Also caught two pre-flight gaps the skill's checklist exists to catch: CHANGELOG `[Unreleased]` was empty, and new public `keycloak_seed_smoke_user` was unregistered in `docs/api/functions.md` + README plugins table.
