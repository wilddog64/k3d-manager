...
- **OPEN ISSUE:** Keycloak admin token fetch currently returns `401 invalid_grant` against the rebuilt cluster, which blocks live verification of the `argocd` client redirect reconciliation path even though `realm-shopping-cart.json` already includes both Argo CD callback URLs. Spec: `docs/issues/2026-05-11-keycloak-admin-token-reconciliation-failure.md`.
