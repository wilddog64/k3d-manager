...
| Move Playwright log to `logs/` + 7-day retention | COMPLETE (this session) | `docs/bugs/v1.1.0-bugfix-gcp-login-log-dir-and-retention.md`; `gcp.sh` now writes timestamped logs to `~/.local/share/k3d-manager/logs/gcp_login_pw_<ts>.log`; prunes files older than `GCP_LOG_RETENTION_DAYS` (default 7); Claude applied directly |
...
- **"I understand" welcome screen inside Allow loop** — OPEN (`docs/bugs/v1.1.0-bugfix-gcp-login-i-understand-in-poll-loop.md`). After password + Next, Google's enterprise account flow navigates to a "Welcome to your new account" page (confirmed from `gcp_login_pw.log`). The existing `I understand` handler runs before the polling loop when the page is still on `challenge/pwd`; by the time the page navigates, the loop has no handler. Fix: add "I understand" handler inside polling loop + increase password post-Next wait to 2000ms. Spec written; hand off to Codex.
