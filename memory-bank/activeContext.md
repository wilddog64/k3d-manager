...
- **OPEN Bug 7 — LDAP CrashLoopBackOff (LDAP_PORT service link conflict)** — Kubernetes injects `LDAP_PORT=tcp://<ip>:389` from `ldap` Service; osixia/openldap reads it as port, corrupting slapd listen URL. Fix: add `enableServiceLinks: false` to pod spec. Branch: `bug/ldap-port-service-link-conflict`. Spec: `shopping-cart-infra/docs/bugs/2026-05-09-ldap-port-service-link-conflict.md`. Assign to Codex.
