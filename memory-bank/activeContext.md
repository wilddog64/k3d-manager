...
**acg_extend selector fix (v1.0.4):** COMPLETE (`97265...`). Fixed `h4` false positive by sanitizing `extendSelectors`. Implemented robust "trapped UI" handling: script now forces a "Start/Resume" click if the extend button is missing to trigger the modal. Verification passed: script correctly parses TTL and handles panel interactions. Issue: `docs/issues/2026-04-06-acg-extend-h4-selector-bug.md`.
**acg_credentials URL fix (v1.0.4):** IN PROGRESS. Identified that `acg_credentials.js` uses a legacy Pluralsight URL (`/cloud-playground/`) which triggers Cloudflare blocks. Fix: Standardize on `/hands-on/playground/` and prioritize CDP reuse. Issue: `docs/issues/2026-04-08-acg-credentials-url-mismatch.md`.
