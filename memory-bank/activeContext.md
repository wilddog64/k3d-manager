...
- **ACG credential extraction misses visible sandbox** — SOURCE FIX IN LIB-ACG PR #2 (2026-04-28, `https://github.com/wilddog64/lib-acg/pull/2`, CI green on `7c64fb5`). Do not manually patch `scripts/lib/acg/`; it is a subtree. Temporary local verification with the same patch passed: `acg_get_credentials` wrote AWS credentials and STS returned `arn:aws:iam::905293745314:user/cloud_user`. Next k3d-manager step after lib-acg PR merges: pull subtree into `scripts/lib/acg/`, then rerun live `make up`. Spec: `docs/issues/2026-04-28-acg-credentials-cdp-context-miss.md`.
