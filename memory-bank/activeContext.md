...
**acg_extend selector fix (v1.0.4):** PARTIAL (`c21f33d9`). Script successfully connects via CDP and parses TTL, but erroneously clicks informational `h4` text instead of the actual button due to overly broad selectors. **Issue found:** `h4:has-text("Extend Your Session")` causes a false positive match on the sandbox card, blocking the actual extension. Issue: `docs/issues/2026-04-06-acg-extend-h4-selector-bug.md`.
