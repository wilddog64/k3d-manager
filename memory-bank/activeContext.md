...
- **`gcp_get_credentials` CDP launch kills Chrome session** — COMPLETE (`a025f41a`) | `docs/bugs/v1.1.0-bugfix-gcp-cdp-launch-no-kill.md`; `scripts/plugins/gcp.sh` now errors if Chrome is already running without CDP, launches only when Chrome is not running, and no longer kills the user's Chrome session.
