...
- **2026-09-16 — E2E chart service-label collision fixed (working tree).** Failure charts were grouping on Prometheus's scrape-target `service` (`vulnerability-inventory-exporter`) instead of the owning application. They now group and render legends from `exported_service`; the run table remains consolidated run-level metadata.
- **2026-09-16 — E2E Recent runs service list (working tree).** The run table now derives a sorted `Services under test` label from failure ownership, so consolidated runs can show basket/cross-service/order/payment/product-catalog instead of only the legacy product-catalog scope.
