...
- **OPEN Bug 8 — CoreDNS maps keycloak.shopping-cart.local to IngressGateway node IP** — ArgoCD SSO fails (`connection refused` at 192.168.97.2:80); NodeHosts approach is wrong (node IP ≠ service endpoint) and non-durable (k3d overwrites it). Fix: patch Corefile with Keycloak ClusterIP `hosts` block. Spec: `docs/bugs/2026-05-09-argocd-sso-coredns-keycloak-clusterip.md`. Assign to Codex.
- **OPEN Bug 9 — acg-up missing frontend port-forward launchd agent** — frontend is ClusterIP on ubuntu-k3s; no auto-restart agent set up. Fix: add step 13 launchd plist (`localhost:3000 → shopping-cart-apps/frontend:80`). Spec: `docs/bugs/2026-05-09-acg-up-frontend-port-forward.md`. Assign to Codex.
