...
- **GCP provider pre-flight compute check** — COMPLETE (`0a0aa663`). `_gcp_preflight_check_compute` added to `k3s-gcp.sh`; probes `gcloud compute instances list --limit=1` before any instance work; fails fast with actionable message (SA email from key JSON + role to grant). Was already locally implemented — committed as part of docs/bugs restructure.
