...
- **Fix CDP context reuse + process.exit/finally** — PARTIAL (`13194e57`). Attempted to fix browser cleanup, but `_cdpBrowser.close()` remains in the `finally` block, causing hangs.
- **Playwright CDP Hijack Hang** — OPEN (`docs/bugs/2026-04-18-playwright-cdp-hijack-hang.md`). Script wrongly calls `browser.close()` on CDP sessions, attempting to kill the user's running Chrome and causing 300s timeouts.
