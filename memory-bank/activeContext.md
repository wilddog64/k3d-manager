...
- **Fix CDP context reuse + process.exit/finally** — COMPLETE (`13194e57`). `docs/bugs/v1.1.0-bugfix-acg-credentials-cdp-and-singleton.md`; `browserContext` now set from CDP whenever Chrome is running; `throw error` in catch ensures `finally` runs. (Separate bug: `close()` in finally must become `disconnect()` — tracked below.)
...
- **`gcp_get_credentials` CDP no auto-restart** — OPEN | `docs/bugs/v1.1.0-bugfix-gcp-cdp-no-auto-restart.md`; remove all pkill/sleep; when Chrome running without CDP → 4 `_err` lines with exact relaunch command + return 1; when Chrome not running → launch with CDP.
- **macOS Chrome CDP relaunch timeout** — OPEN | `docs/bugs/2026-04-19-gcp-cdp-relaunch-timeout-macos.md`; warned restart path kills and relaunches Chrome, but `open -a "Google Chrome" --args --remote-debugging-port=9222` does not reliably expose CDP on port 9222 within 15s, so `make up` fails before Playwright starts.
