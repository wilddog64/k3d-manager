...
- **ACG AWS functions wrong plugin** — ASSIGNED → Codex (`docs/bugs/v1.1.0-bugfix-acg-aws-functions-wrong-plugin.md`). 9 AWS-specific functions + constants moved from `acg.sh` to `aws.sh`; no circular dep since `acg.sh` already sources `aws.sh`.
