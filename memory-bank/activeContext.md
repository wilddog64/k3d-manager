...
- **CDP guidance log** — OPEN (`docs/bugs/v1.1.0-bugfix-acg-credentials-cdp-guidance-log.md`). When CDP fails the user sees a new browser window but no explanation. Add a second log line telling them to log in there OR restart Chrome with `--remote-debugging-port=9222`.
