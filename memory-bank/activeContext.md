...
- **2026-09-15 — BUG FILED + assigned to Codex: `make update-webhook-slack-secret` argv + fail-open** — IMPLEMENTED (Codex) — awaiting Claude verification. Commit `26e8b81f` pushed to `origin/k3d-manager-v1.34.0`; spec `docs/bugs/2026-09-15-update-webhook-slack-secret-argv-fail-open.md`. Signing secret passed to PlistBuddy argv; `( ...; exit 1)` subshell lets a missing Keychain item write an EMPTY secret and restart the webhook. Fix: plistlib via env + `{ }` fail-closed + backup; 4 new BATS (2 behavioural, PATH-stubbed `security`/`launchctl`, temp HOME). Spec pre-validated by Claude on a scratch copy (10/10; old recipe fails all 4 new tests). `update-webhook-slack-roles` argv ACCEPTED (role map is not a credential; `security -w` has no stdin). Dispatched via `codex exec`; verify SHA on origin + BATS before trusting.
