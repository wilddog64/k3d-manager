...
- **GCP provider pre-flight compute check** — ASSIGNED to Codex. Gemini confirmed ACG sandbox SA lacks all Compute IAM permissions; deploy_cluster fails deep in the flow with cryptic gcloud error. Fix: add `_gcp_preflight_check_compute` that probes `gcloud compute instances list --limit=1` and fails fast with actionable message (SA email from key JSON + role to grant). Spec: `docs/bugs/v1.0.7-bugfix-gcp-provider-fixes.md` (Change 6). Only `scripts/lib/providers/k3s-gcp.sh` changes.
