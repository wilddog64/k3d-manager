...
- **GCP Login Linux Headless OAuth** — SPEC READY (`docs/bugs/2026-04-23-gcp-login-linux-headless-oauth-url-capture.md`). On Linux/ACG sandbox, `gcloud auth login` cannot open a browser — prints the OAuth URL instead. `gcp_login.js` times out waiting for a Chrome tab that never appears. Fix: capture URL from gcloud output, pass via `GCP_AUTH_URL`, navigate directly in Chrome CDP. Assigned to Gemini 2026-04-23.
