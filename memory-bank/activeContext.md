...
- **2026-09-16 — E2E dashboard visibility corrected (pending commit).** Moved the new trend/cause/spec panels above the run and failure tables so they are visible without scrolling. Failure groups/details now hide the Prometheus scrape-target `service` label and present `exported_service` as the owning application Service, eliminating the duplicate exporter-service column. Direct apply/restart was performed; ArgoCD will retain it after the branch commit is pushed.
