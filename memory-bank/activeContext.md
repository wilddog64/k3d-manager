...
- **lib-acg main branch protection** — BLOCKED (2026-04-28). Requested after PR #2 merge, but this Codex session cannot mutate branch protection: GitHub connector lacks a protection update operation, local `gh` auth is invalid, and the only token-like environment variable returns GitHub API `401`. Issue: `docs/issues/2026-04-28-lib-acg-main-branch-protection-auth-blocked.md`.
