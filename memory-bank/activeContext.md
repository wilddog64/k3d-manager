...
- **Playwright ProcessSingleton lock bug** — OPEN (`docs/bugs/2026-04-18-playwright-process-singleton-lock.md`). Aborted runs or infinite loops leave `SingletonLock` on disk, blocking subsequent extractions. Needs `finally` cleanup hardening and global timeouts.
- **ACG Credentials CDP Fallback Bug** — OPEN (`docs/bugs/2026-04-18-acg-credentials-cdp-fallback-bug.md`). Extraction times out on `/id` login if a Pluralsight tab is not already open, because CDP logic wrongly falls back to a clean persistent context instead of opening a new tab.
- **Fix CDP context reuse + process.exit/finally** — OPEN (Codex) | `docs/bugs/v1.1.0-bugfix-acg-credentials-cdp-and-singleton.md`; fixes both: (1) use CDP context whenever Chrome is running, not only when Pluralsight tab is open; (2) `throw error` instead of `process.exit(1)` in catch so `finally` always closes the browser