...
- **`gcp_get_credentials` CDP launch kills Chrome session** — OPEN | `docs/bugs/v1.1.0-bugfix-gcp-cdp-launch-no-kill.md`; must replace `pkill` with `pgrep` check — error if Chrome running without CDP, launch only if Chrome is not running at all.
