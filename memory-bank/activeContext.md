...
**Current task:** Pin gemini model to `gemini-2.5-flash` — **IMPLEMENTED**. Updated `scripts/plugins/antigravity.sh` with `--model gemini-2.5-flash` in 5 call sites. Spec updated to use `gemini-2.5-flash`. BATS tests pending Codex implementation.
