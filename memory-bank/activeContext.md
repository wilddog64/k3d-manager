...
- **COMPLETE:** Added identity troubleshooting helpers for live pod inspection: `bin/vault-exec.sh`, `bin/ldap-search.sh`, `bin/keycloak-logs.sh`, and `bin/ldap-logs.sh`. Shared wrapper logic lives in `scripts/lib/identity_tools.sh`, and the new how-to doc is `docs/howto/identity-troubleshooting.md`.
