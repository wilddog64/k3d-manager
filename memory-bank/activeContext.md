...
- **Next milestone: v1.31.0 branch** (`k3d-manager-v1.31.0` created 2026-09-06 from `8e71692b`, origin tracking). First scoped item **Hermes R4 GitHub App installation-token auth: REJECTED + REVERTED 2026-09-07.** Codex built it (`37c144fe`) but a GitHub App cannot get the permission R4 needs on this personal repo (user-confirmed — the same wall as the Aug 2026 3-app episode). The App code was inert behind a PAT fallback and ended in a bogus "register an app" manual step for a non-problem. Removed `github_app.py` + its test, stripped the App branch from `repairs.py`/`preflight.py`, dropped App docs from the guide, deleted the plan doc. **R4 stays on the `k3dm-hermes-gh-token` PAT — live-verified 2026-09-07** (`bin/k3dm-hermes preflight` → `actions_read:true, actions_write:true, exit=0`; PAT expires 2026-12-04; durable fix for that recurrence is a no-expiration classic PAT in the same slot, not an App). **Kept** the `k3dm-hermes preflight` scope-check tool (rescoped PAT-only) as the one salvageable deliverable. Gates (Claude-run 2026-09-07): pytest 29 passed, py_compile clean, no protected-subtree edits. v1.30.0 release-ledger backfill (CHANGELOG/releases.md/README v1.30.0 rows) landed on this branch (`06164a04`), per the one-release-later cadence.
- **v1.31.0 WS2 — Hermes GitHub token-expiry advisory: IMPLEMENTED 2026-09-07, commit `6ff32c7f` on `origin/k3d-manager-v1.31.0`.** Spec `docs/plans/v1.31.0-hermes-token-expiry-advisory.md`. Hermes posts a once-per-day Slack advisory when `k3dm-hermes-gh-token` is within a window (default 14d) of expiry, auto-silences once a no-expiration classic PAT is in the slot. **Reshaped by a verified correlator finding:** `Correlator.process` only fires on ≥2 degraded sensors (edge-triggered), so the expiry check is a STANDALONE advisory (direct `post_summary` + `state`-keyed daily dedup), NOT a correlator sensor — a naive sensor would be silently swallowed. This is the Hermes-native replacement for the (blocked) calendar/cron reminder for the 2026-12-04 PAT expiry. Gates: pytest 31 passed, py_compile clean; SHA verified on origin.
- **v1.31.0 PR #122 OPEN 2026-09-07** (https://github.com/wilddog64/k3d-manager/pull/122, base `main` ← `k3d-manager-v1.31.0`). Bundles `preflight` tool + token-expiry advisory + R4 App-auth revert + v1.30.0 ledger backfill. CI green on head `30122821`. **Copilot 2 findings both fixed (`30122821`):** preflight `per_page=1`→`30` (verdict was bound to the newest run); write-scope probe now requires non-empty output (empty/transport error no longer misread as write=true; rc is not the gate because a write-capable PAT still gets non-zero rc on `rerun-failed-jobs` against a successful run). +2 tests → 33 passed. Both Copilot threads resolved. **CodeQL 1 finding = FALSE POSITIVE** (`bin/k3dm-hermes:128` — `report` is booleans+`"pat"`, token only in subprocess env; non-blocking, CodeQL not a required check) — **DISMISSED as false positive 2026-09-07 (alert #21), flipping the "Code scanning results / CodeQL" check to pass** (user's "code signing failed" was actually the red Code *scanning* X). Findings: `docs/issues/2026-09-07-copilot-pr122-review-findings.md`. **MERGE-READY 2026-09-07: CI green on final head `75dd2a36`, all checks pass, `enforce_admins` DISABLED (`.enabled: false`). User merges — NEVER auto-merge.** Queued (not started): patch review for the external TwinkleAI/MCP Codex plan in ~/Downloads (add LLM provider + key to `.env.example`, add missing `services/llm/client.ts`+`generate-answer.ts` task, add mock→real `/api/query` cutover task; switch subagent-driven-development→executing-plans, gate `npm run build` to milestones, keep 45× runner headless).
