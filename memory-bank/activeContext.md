...
- **2026-09-15 — BUG FILED + assigned to Codex: `make update-webhook-slack` token echo** — spec `docs/bugs/2026-09-15-update-webhook-slack-token-echo.md`. Target echoes token (PlistBuddy `Add` not `@`-prefixed), passes it in argv, ignores Keychain, requires `SLACK_CHANNEL_ID`. Fix: read Keychain `k3d-manager-slack-bot-token-bot` (or exported env), keep plist channel ID, back up plist, write via plistlib with token in env; new `scripts/tests/bin/makefile_webhook_slack.bats`; CHANGELOG Fixed. Dispatched via `codex exec` (no live Keychain/launchctl). Verify SHA on origin + BATS before trusting. Operator acceptance = user runs `make update-webhook-slack`.
