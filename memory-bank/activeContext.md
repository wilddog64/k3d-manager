...
- **`gcp_get_credentials` missing CDP auto-launch** — OPEN (`docs/bugs/v1.1.0-bugfix-gcp-get-credentials-cdp-auto-launch.md`). `gcp.sh` calls Node directly with no CDP check. Fix: add CDP availability check + `pkill`/`open` Chrome with default profile (no `--user-data-dir`) before calling Node. Mirrors `acg.sh` pattern but uses real Chrome profile.
