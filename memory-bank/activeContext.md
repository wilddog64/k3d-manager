...
- **2026-09-17 — Docs audit of the Grafana/status-fix block (user: "we fixes some issues, and also create and update grafan chart. please review memory-bank and docs").** Reviewed 66 commits since `dadacf31`. memory-bank itself was accurate; the gaps were all in the docs layer and are now fixed: (1) `CHANGELOG.md [Unreleased]` had **zero** coverage of ~40 Grafana commits — added the Hermes Status dashboard, E2E failure groups/details/trends and failure-ratio entries under Added, and the result-set-bounding, scrape-metadata-hiding and poll-age-guard entries under Fixed; (2) `progress.md` still said the E2E Grafana trends spec was "not started" while the three entries below it recorded it implemented and live-verified — corrected to `[x]`; (3) `docs/plans/v1.34.0-e2e-grafana-trends-and-drilldown.md` was the only v1.34.0 plan with no `**Status:**` header — marked IMPLEMENTED (`3c33103a`, `387f019e`); (4) README "Recent entries" was 10 days stale (newest 2026-09-07) against 11 new issue docs — refreshed to the 5 most recent. **v1.34.0 is at the 5-plan cap; any further spec opens v1.35.0.** Not done, needs a decision: `activeContext.md` is 2777 lines and was last compressed 2026-08-24, and Current focus now opens with ~17 one-line 2026-09-17 dashboard-tweak entries — it is being used as a commit log and wants a compression pass to `memory-bank/archive/`.
...
- **2026-09-17 — E2E ratio rollout cleanup.** Recent runs now filters out the intermediate fraction-only ratio series and requires the percentage suffix.
- **2026-09-17 — Checkout load-test dashboard no-data diagnosis.** Prometheus has zero `k6_checkout_load_requests_total_total`, `k6_checkout_load_latency_seconds_p95`, and `k6_vus` series; the dashboard is a planned view awaiting an authenticated remote-write load-test run. Details: `docs/issues/2026-09-17-checkout-loadtest-dashboard-no-data.md`.
- **2026-09-17 — k3dm deployment dashboard no-data diagnosis.** Prometheus has zero `k3dm_deployment_duration_seconds`/`k3dm_deployment_success` series and no Pushgateway service; the dashboard is installed but deployment-event publishing is not wired. Details: `docs/issues/2026-09-17-k3dm-deployment-dashboard-no-data.md`.
- **2026-09-17 — CVE dashboard freeze diagnosis.** `trivy_vulnerability_inventory` has 7,409 series; raw CVE tables were sending all rows to Grafana. Both tables now use `topk(500, ...)`; details: `docs/issues/2026-09-17-cve-dashboard-freeze.md`.
- **2026-09-17 — CVE table cleanup.** Removed the non-informative Service field from both unique-CVE tables.
...
- **2026-09-16 — E2E dashboard visibility corrected (`387f019e`).** Moved the new trend/cause/spec panels above the run and failure tables so they are visible without scrolling. Failure groups/details now hide the Prometheus scrape-target `service` label and present `exported_service` as the owning application Service, eliminating the duplicate exporter-service column. ArgoCD was hard-refreshed to `387f019e` and Grafana restarted successfully. The temporary stale-layout behavior is documented in `docs/issues/2026-09-16-e2e-grafana-dashboard-stale-layout.md`.
- **2026-09-16 — HTTP/2 failure-rate observability explicitly deferred to Tier 2.** Current E2E metrics have no protocol/version label, so Grafana cannot distinguish HTTP/2 failures or calculate an HTTP/2-specific rate. Follow-up and schema requirements are documented in `docs/issues/2026-09-16-http2-failure-rate-tier2-dependency.md`.
- **2026-09-16 — `show-service-passwords` N/A was transient Vault-forward loss.** The target reads all display mirrors through `127.0.0.1:18200`; the Vault port-forward was unavailable during the reported run. The LaunchAgent/listener were later confirmed running and a rerun resolved the credentials. Recovery is `make install-vault-port-forward`; details and the silent-error follow-up are in `docs/issues/2026-09-16-show-service-passwords-vault-forward.md`.
- **2026-09-16 — Service recovery hardening (pending commit).** `show-service-passwords` now probes Vault health and automatically reinstalls/retries the port-forward when needed. `restart-webhook` now uses `launchctl kickstart -k` for loaded jobs and only falls back to bootout/bootstrap when unloaded, avoiding LaunchAgent bootstrap error 5. Findings are documented in `docs/issues/2026-09-16-restart-webhook-bootstrap-io-error.md`.
- **2026-09-16 — Service recovery hardening (`pending commit`).** `show-service-passwords` now validates an authenticated Vault KV lookup (not just HTTP health), automatically reinstalls/retries the port-forward when needed, and fails clearly when the `vault-root` token/context is unavailable. `restart-webhook` now uses `launchctl kickstart -k` for loaded jobs and only falls back to bootout/bootstrap when unloaded, avoiding LaunchAgent bootstrap error 5.
- **2026-09-16 — Status/webhook outage fixed (pending commit).** The webhook process was healthy, but its full health sweep exceeded `cluster-status-summary`'s 90-second request window, producing misleading `UNKNOWN/webhook unavailable`. Added `quick=1` liveness mode and a 30-second summary request; Prometheus HTTP 401 is now a warning because authenticated login is checked separately. Live `make status CLUSTER_PROVIDER=k3s-hostinger` returns WARN with four healthy services and exits 0. Details: `docs/issues/2026-09-16-status-webhook-health-timeout.md`.
- **2026-09-16 — Status token fallback (pending commit).** `cluster-status-summary` now retries the Keychain webhook token when an exported `K3DM_WEBHOOK_TOKEN` is rejected, covering stale shell tokens after rotation. Verified with `K3DM_WEBHOOK_TOKEN=stale-token make status ...`; status returns WARN/rc 0 instead of UNKNOWN.
- **2026-09-16 — Empty webhook Keychain record (pending commit).** The live token lookup returned zero bytes with success, so the Keychain item existed but contained an empty password. `bin/k3dm-webhook-setup` now treats empty values as missing and regenerates them. Grafana remained reachable because its public route is independent of local webhook bearer auth.
- **2026-09-16 — Webhook token outage recovered.** `bin/rotate-webhook-token` had looked up obsolete Keychain service `cloudflare-api-token`; corrected to `k3dm-cloudflare-api-token`. Rotation then succeeded, uploaded the Cloudflare Worker secret, restored a 64-byte local token, and verified the public route. Live status now WARNs only on expected unauthenticated Prometheus readiness; credential display fields are all set.
- **2026-09-16 — User-session Keychain ACL blocker resolved.** The token was intact; the login Keychain was locked/unavailable to the terminal. Unlocking it restored webhook authentication without rotation. Live status now returns ArgoCD/Frontend/Keycloak/Grafana HTTP 200 and only the expected Prometheus authentication warning.
- **2026-09-16 — Keychain diagnostics hardened (pending commit).** Status now reports empty/missing token, Keychain interaction denial, and rejected exported token separately instead of generic webhook unavailable. Webhook setup reports the unlock instruction when a Keychain write is denied.
- **2026-09-16 — E2E table labels clarified (`58376441`).** Recent runs now labels the source revision as `Runner commit`, the tested image as `Application image digest`, the service as `Service under test`, and `api+flows` as `Test suite`. The plan now documents how to interpret each field and how M2 checks out the runner revision.
- **2026-09-16 — E2E chart service-label collision fixed (`f0115c90`).** Failure charts were grouping on Prometheus's scrape-target `service` (`vulnerability-inventory-exporter`) instead of the owning application. They now group and render legends from `exported_service`; the run table remains consolidated run-level metadata.
- **2026-09-16 — E2E Recent runs service list (`2830b32c`).** The exporter derives a sorted `Services under test` label from failure ownership, so consolidated runs can show basket/cross-service/order/payment/product-catalog instead of only the legacy product-catalog scope. Platform-ops was reapplied and Grafana was restarted.
- **2026-09-16 — E2E Recent runs column order (`b0ccb760`).** Run ID and Runner are now ordered as the first two columns for faster correlation with M2 executions. Platform-ops was reapplied and Grafana successfully rolled out.
- **2026-09-16 — E2E failure table column order (`0f877444`).** Failure groups and Failure details now put Run ID and Runner first, matching Recent runs for quick correlation. Live ConfigMap verification confirms all three tables have the requested ordering and Grafana rolled out successfully.
- **2026-09-17 — Hermes findings visibility implemented (`60a04c19`, label fix pending).** Hermes publishes a redacted status ConfigMap; the platform exporter emits Hermes sensor/check/incident metrics; and a dedicated `Hermes Status` Grafana dashboard shows current findings, poll age, incident state, and history. The feed also publishes regular sensors while optional `status_checks` is disabled. The live snapshot was relabeled after the first publication exposed a label-application gap. Design/acceptance record: `docs/issues/2026-09-17-hermes-grafana-visibility.md`.

- **2026-09-16 — E2E failure groups are wired to Grafana, committed as `6c8b834d` and pushed.** E2E summaries classify bounded failures into `service-unreachable`, `timeout`, `contract-drift`, and `assertion` groups; remote publication validates and preserves them; the platform exporter emits `e2e_failure_group_info`; the dashboard adds a Failure groups table and renames the run-level `passed` column to Run result. Existing retained ConfigMaps will not gain groups until a new run publishes the enriched schema. Gates: targeted E2E/observability/remote BATS 112/112, shellcheck clean, `_agent_audit` exit 0.
- **Live follow-up 2026-09-16 — resolved with exporter normalization `8c80aeb9`.** After `make platform-ops`, the exporter was updated to normalize nested JSON and skip invalid payloads. A durable backfill for run `1789549631-2079` now emits five `e2e_failure_group_info` series (cart 11, cross-service 10, payments 9, orders 2, products 1). Grafana should render them on its next refresh. Issue evidence: `docs/issues/2026-09-16-live-e2e-failure-groups-empty.md`.
- **2026-09-16 — E2E test-level drill-down shipped as `facb30fa`.** Added bounded `failure_details` to summaries/events, exporter metric `e2e_failure_info`, and a Grafana Failure details table with redacted file/title/status/error fields. Platform-ops was applied and exporter restarted. Existing backfill has group counts but no test details; the next enriched E2E publication populates the drill-down rows.
- **Live detail backfill 2026-09-16 — verified.** Published the existing redacted sidecar as `e2e-result-m2-detail-1789549631`; exporter now emits 33 `e2e_failure_info` series. Visible patterns include undefined cart/cross-service response fields, payment `Unexpected end of JSON input`, order status fields missing, and product price returned as a string.
- **2026-09-16 — E2E service labels/table cleanup committed as `dece8c63`.** Failure groups/details now infer owning app (`basket`, `order`, `payment`, `product-catalog`, `cross-service`) instead of using the consolidated `product-catalog` run label. Grafana Failure groups/details queries no longer filter on the consolidated service and hide scrape metadata columns (`container`, `endpoint`, `namespace`, `pod`). Platform-ops deployed.
- **Live dashboard reload follow-up 2026-09-16.** The Grafana dashboard ConfigMap remained on the previous panel definition after platform-ops; applying `grafana-dashboard-e2e.yaml` directly corrected both panels. Live ConfigMap now excludes `container`, `endpoint`, `namespace`, and `pod`.
- **Live service-label correction 2026-09-16 — `c691eb92` (merged/pushed as `d83f1940`).** Cross-service filenames now map explicitly to `cross-service` instead of falling back to the consolidated `product-catalog` label. Live detail metric distribution verified: basket 11, cross-service 10, payment 9, order 2, product-catalog 1.
- **2026-09-16 — Removed `Project` from failure tables (`2d79b2ba`).** `api+flows` remains visible only in Recent runs; Failure groups/details now focus on owning service, spec/test, and error data. Dashboard ConfigMap applied live.
- **Live Grafana reload 2026-09-16.** The live ConfigMap initially remained stale; it was reapplied and `kube-prometheus-stack-grafana` restarted successfully. Both failure panels now have `project: true` in `excludeByName`.

- **2026-09-16 — Live SSO/E2E recovery applied.** ArgoCD `argocd-cm` URL was corrected from `https://argocd.shopping-cart.local` to `https://argocd.3ai-talk.org`; in-cluster callback check now returns HTTP 303 with the public callback, and frontend PKCE check returns HTTP 200 with a login form (no invalid-credentials page). The three retained M2 `publication_pending` runs were replayed (`1787838531-2562`, `1789481773-13798`, `1789549631-2079`), all applied, exporter exposes `e2e_run_info` and `e2e_last_run_pass`. k3d-manager has uncommitted `ARGOCD_PUBLIC_URL` template/plugin/test changes. Shopping-cart-infra hook had a missing-`awk` defect; durable branch `fix/keycloak-hook-runtime-tools` pushed through `cb6e414`, with Keycloak-image helper tests passing, but the live hook still reaches BackoffLimitExceeded on a flow-execution Resource-not-found response even though the resulting live flow is correct. Full evidence and limitation: `docs/issues/2026-09-16-live-sso-e2e-recovery.md`. Do not claim durable SSO completion until that branch is merged and the PostSync hook succeeds.

- **2026-09-16 — Hermes scheduled status continuation: implementation corrected and PUSHED `e3e37f9671b9de4a14a87176e494de3a578642fa`; live acceptance pending.** User asked to inspect the inherited uncommitted work, then fix and document the findings. Fixed duplicate unknown-status SMS, raw-record credential redaction, status-specific bug/recurrence context, and local-midnight reminders. Schedule now defaults off until the Prometheus authenticated-probe dependency is verified (`K3DM_HERMES_STATUS_ENABLED=1` opts in). Python compilation clean; Hermes pytest **106 passed in 2.90s**; status-summary BATS **8/8**; `_agent_audit` exit 0, `_agent_lint` exit 0 (AI gate disabled), clean-tree checkpoint safely skipped. Full `test all` stopped at unrelated deployment test 3 blocked in `kubectl get nodes --no-headers` (exit 143), so full-suite acceptance is partial. Findings/output: `docs/issues/2026-09-16-hermes-status-triage-review.md`; existing plan updated, no additional plan or repairs added. Supersedes the earlier assigned/dispatched notes below. PR: none (repository policy).

- **2026-09-16 02:10 PT — Hermes scheduled e2e FIRED ON ITS OWN (first live proof of the Wed/Sat schedule)** — run `1789549631-2079`, runner m2, tier vcluster, runner commit `ec4874fe`. Hermes claimed the Wednesday slot, dispatched, triaged, filed 5 bugs, and pushed them itself as `ad4a4a45` (`Filed-By: k3dm-hermes`). The operator item "Hermes picks up the Wed/Sat schedule on its next poll" is now CLOSED by evidence, not assumption. Result 24 passed / 33 failed / 102 (previous run `1789481773-13798` was 12 passed / 45 failed). **The payment substrate fix is confirmed live**: the 9 payment failures classified as `assertion`, NOT `service-unreachable` — there are zero ECONNREFUSED `:8084` groups this run, so payment is deployed and reachable in the Tier 1 substrate. Remaining groups are contract drift: cart 11, cross-service 10, orders 2, products 1 (`docs/bugs/2026-09-16-e2e-*.md`, all OPEN / unverified rule-based triage). Note the whole run is Tier 1 (`OAUTH2_ENABLED=false`, no Keycloak) so it says nothing about the SSO fix.

- **2026-09-16 — Dispatched to Codex (`codex exec`, background): Hermes scheduled `make status` sensor + triage** (user: "please dispatch to codex for execution"). Spec `docs/plans/v1.34.0-hermes-scheduled-status-triage.md` gained a `## Definition of Done` first (verbatim commit message, pytest + shellcheck gates, rebase-before-push note — `k3dm-hermes` is a second writer on this branch and pushed `ad4a4a45` itself, so non-fast-forward rejections are expected). Implementation is stub-only (pytest, no live cluster), so the Prometheus-401 blocking dependency does NOT block Codex — it blocks *enabling* the schedule live. Verify on return: SHA on `origin/k3d-manager-v1.34.0`, diff limited to the spec's file table, `pytest scripts/tests/hermes/` green, no new `REPAIRS` entry, `overall: unknown` never mapped to `degraded`.
- **2026-09-16 — ASSIGNED TO CODEX: Hermes scheduled `make status` + triage + bounded repair** (user: "if only make status is the only way to verify then can we have hermes agent to schedule to run make status every 39 mins, triage error if there's any. agent fix if the problem is simple; otherwise notify us for deep dive"). Plan `docs/plans/v1.34.0-hermes-scheduled-status-triage.md` (4th v1.34.0 plan doc; cap is 5). Grounding: `make status` is the ONLY detector for a browser-SSO outage — `reachability`/`bin/public-endpoint-probe` scores `keycloak.3ai-talk.org/` healthy on a 2xx and returned 200 through the whole 2026-09-15 outage, and neither e2e tier can see it (`reference_e2e_cannot_detect_browser_sso_outage`). Design: NO new launchd agent — gate inside the existing 300s Hermes poll (`com.k3d-manager.hermes.plist.tmpl` StartInterval 300), so 39 min actually lands at ~40 (39 is not a multiple of 5; accepted, documented, `K3DM_HERMES_STATUS_INTERVAL_MIN` tunable). New `status_checks` sensor consumes `bin/cluster-status --json` (stable contract: `overall`/`errors`/`warnings`/`checks[].id`, ids like `frontend_sso_login`) — never scrape human output. `overall: unknown` stays `unknown`, NOT degraded: that is R1's webhook-down signal and reporting it here would double-page. New pure `scripts/lib/hermes/status_triage.py` classifies by check id; notification is state-change only via `e2e_triage.diff_groups` + reuse of `e2e_bugs.file_bugs`. **"Agent fix if simple" is delivered as proposal + one-click Slack approval, NOT auto-execution** — `repairs.py` is explicit that "no repair runs outside approve()", Phase 3 (cooldowns/budgets/audit) is unimplemented, and the single auto path `auto_remediate_kine` is opt-in + once-per-incident; the plausibly-simple classes are already covered by R2/R3, so this spec adds NO new REPAIRS entry. `sso-auth` is never agent-repairable. BLOCKING DEPENDENCY: the false `✗ Prometheus: HTTP Error 401` red (probe never learned to authenticate after `4f2fda24` added the auth proxy) must be fixed first, or the first scheduled run pages and files a bug immediately and stays red forever.

- **2026-09-15 — SPECS FILED (not assigned): scheduled e2e + e2e root causes + login alignment** (user: Slack e2e failed with payment not deployed, Grafana missed it; "have hermes agent schedule to run e2e-test twice a week, Wednesday and Saturday at 2:00 am … triage … create bugs"; "make status report login successful but I can't login most services except grafana … They should align"). Investigation: run `1789481773-13798` = 45 failed/12 passed/102 (08-27 run 31 failed); 9 payment ECONNREFUSED :8084 (substrate has no payment), 35 contract drift; Grafana miss = `E2E_M2_PUBLISH_BACK_HOST` only in `.envrc`, launchd webhook never loads it → results stuck `publication_pending` on M2 (hub has 0 e2e-result CMs); M2 runner checkout = hand-synced `f5a40988` w/ 631 modified files, substrate differs (basket pin). Login checks use smoke user/admin-initial fallback, not operator accounts; only Grafana aligned (operator: ArgoCD, Keycloak, Frontend SSO, Prometheus/Alertmanager all fail). User decisions: bugs → docs/bugs + Slack; rule-based triage; deploy payment in substrate. Specs (order): `docs/bugs/2026-09-15-e2e-m2-runner-checkout-drift.md` → `docs/bugs/2026-09-15-e2e-substrate-missing-payment.md` → `docs/bugs/2026-09-15-e2e-remote-results-never-reach-grafana.md` → plan #3 `docs/plans/v1.34.0-hermes-scheduled-e2e.md`; independent: `docs/bugs/2026-09-15-status-login-checks-not-operator-accounts.md`. User go 2026-09-15 → ASSIGNED to Codex (one sequential codex exec: drift → payment → grafana → hermes plan → login). Run 1 stopped at spec 1 commit (sandbox denied `.git/index.lock`); Claude verified + committed spec 1 as `3398c766`. Specs 2–5 now dispatched one per codex exec, no-commit mode; Claude verifies + commits each. Spec 2 (payment) verified + committed `cb2982b1`; spec 3 (grafana) verified + committed `45ce5428`; spec 4 (hermes schedule) verified + committed `3ba1b8af` (Claude fixed a pid_alive(0) slot-blocking bug + added the missing file_bugs git tests); spec 5 (login checks) verified + committed `384b0202` (Claude restored the monitoring-paused Grafana softening). ALL 5 SPECS DONE on branch; none live-accepted yet. Operator steps: (1) `printf 'E2E_M2_PUBLISH_BACK_HOST=cliang@m4-air.local\n' > ~/.config/k3d-manager/e2e-remote.env` + `make e2e-replay RUNNER=m2`; (2) `make restart-webhook` + `make status CLUSTER_PROVIDER=k3s-hostinger` (expect red login lines); (3) `make e2e-remote RUNNER=m2` (expect no `:8084` failures); (4) Hermes picks up the Wed/Sat schedule on its next poll. No PR yet — user gate.

- **2026-09-15 — first honest `make status` run: 3 reds, both root causes found (live, read-only)**
  1. **Prometheus public + unauthenticated** — `prometheus.3ai-talk.org` served the hub Prometheus from a RAW port-forward; the `prometheus-web-config` Secret has been INERT since v1.6.1 removed the operator `web.config.file` path (operator denylists it). Spec `docs/bugs/2026-09-15-prometheus-public-endpoint-unauthenticated.md`; fix committed `4f2fda24` — raw PF moves to 19091, `bin/prometheus-auth-proxy` serves 19090 (the port cloudflared publishes) with Vault `k3d-manager/prometheus-basic-auth`. Claude also fixed `prometheus_port_split.bats`, which Codex never ran. LIVE 2026-09-15: user ran `make observability` — proxy plist installed but the auth proxy crash-looped `OSError: [Errno 48] Address already in use` on 19090, because `deploy_observability` does NOT re-render the port-forward plist (only `make install-prometheus-port-forward` does) so the live agent still bound 19090:9090; 19091 dead, 19090 still raw and unauthenticated (`/api/v1/query?query=up` = 200 with no credentials). Documented in the bug spec + Makefile message, `f125d26f`. OPERATOR PENDING: `make install-prometheus-port-forward` then `launchctl kickstart -k "gui/$(id -u)/com.k3d-manager.prometheus-auth-proxy"`.
  2. **All shopping-cart SSO broken (frontend + ArgoCD)** — RESOLVED by PR #98 MERGED 2026-09-16 as `af4b053d`. Keycloak browser flow was broken by the reconcile hook adding `otp-conditional-subflow` as a TOP-LEVEL CONDITIONAL execution beside the ALTERNATIVE `forms` subflow, causing HTTP 400 "We are sorry... Invalid username or password." before any login form. Spec `docs/bugs/2026-09-15-keycloak-browser-flow-regression-blocks-all-sso.md`; fix `c3a2c37` + Copilot follow-up `78403f0` on shopping-cart-infra branch `fix/keycloak-browser-flow-top-level-otp`. CI green, Copilot reviewed (1 finding fixed, thread resolved). `enforce_admins` on shopping-cart-infra main RESTORED to true. Live verification: ArgoCD sync of `identity/keycloak/` and the PostSync reconcile hook run is outstanding (in progress).
  3. **Check label bug (ours)** — the classifier blamed the operator password for a pre-login-form 400. Fixed `9c8f9a17`: no login form -> "authorization request rejected (HTTP n)".
  Follow-up noted, not fixed: realm user `admin` resolves to LDAP `cn=jenkins-admin,ou=users,dc=home,dc=org` while developer/operator map to `uid=<user>`.
- **2026-09-15 — SPEC + assigned to Codex: Hermes SMS pager** — VERIFIED `a18450d4` — diff byte-identical to validated patch, 80/80 pytest, only 6 files; live poll 13:17Z clean with `pages: []` and argocd evidence "credential rejected; re-mint k3dm-hermes-argocd-token". Plan `docs/plans/v1.34.0-hermes-sms-pager.md` (2nd v1.34.0 plan doc). Texts the operator (Gmail SMTP → carrier SMS gateway; Keychain account `k3dm` items `k3dm-hermes-sms-from`/`-app-password`/`-to`, off until provisioned) on: webhook down 2 polls, other sensor `unknown` 6 polls, Hermes poll job raising 2 polls, new critical/high CodeQL/Dependabot alert (hourly, audit token); once down + once recovered, mirrored to Slack, `K3DM_HERMES_SMS_DAILY_BUDGET` (10/day). Code shipped as a pre-validated patch embedded in the spec (80/80 pytest on a clean copy of `0d325d51`). ArgoCD `unknown` since 2026-09-10 23:20Z ROOT-CAUSED: `cicd` ns recreated → new `argocd-secret` (23:54Z) → `k3dm-hermes-argocd-token` rejected (`Unauthenticated: token signature is invalid`); `hermes` account still in argocd-cm. Operator action: re-mint token (`argocd account generate-token --account hermes`) and update Keychain via prompt. Spec also makes the sensor name the rejected credential in evidence. Incident during prep: a failed `cd` put 2 scratch edits into the real repo (`bin/k3dm-hermes` broken import for ~3 min, between polls, no failed poll logged); reverted via `git checkout`. Dispatched via codex exec. F1 `612d8f63` (Claude): per operator "do not dup the tokens", pager reads the existing `k3dm-alertmanager-gmail-app-password` (login-user account, MATCH vs Vault + live Alertmanager Secret) instead of `k3dm-hermes-sms-app-password`; 81/81 pytest. Operator adds only `k3dm-hermes-sms-from` and `-to`.
- **2026-09-15 — BUG FILED + assigned to Codex: `make update-webhook-slack-secret` argv + fail-open** — VERIFIED (Claude) — fix `26e8b81f`: S1+S2 byte-match spec, 12/12 BATS (10 webhook_slack + 2 platform_ops), make -n OK, only 3 spec files touched; operator acceptance pending (user runs `make update-webhook-slack-secret` + a Slack slash command). Commit `26e8b81f` pushed to `origin/k3d-manager-v1.34.0`; spec `docs/bugs/2026-09-15-update-webhook-slack-secret-argv-fail-open.md`. Signing secret passed to PlistBuddy argv; `( ...; exit 1)` subshell lets a missing Keychain item write an EMPTY secret and restart the webhook. Fix: plistlib via env + `{ }` fail-closed + backup; 4 new BATS (2 behavioural, PATH-stubbed `security`/`launchctl`, temp HOME). Spec pre-validated by Claude on a scratch copy (10/10; old recipe fails all 4 new tests). `update-webhook-slack-roles` argv ACCEPTED (role map is not a credential; `security -w` has no stdin). Dispatched via `codex exec`; verify SHA on origin + BATS before trusting.
- **2026-09-15 — BUG FILED + assigned to Codex: `make update-webhook-slack` token echo** — VERIFIED by Claude 2026-09-15 (S1+S2 byte-match spec, 8/8 BATS, make -n sentinel hits 0; CHANGELOG under [Unreleased] Fixed); operator acceptance PASSED 2026-09-15 (no token echoed, webhook restarted, auth.test ok bot B0B8BC72WPK). Commit `39ecff4d` pushed to `origin/k3d-manager-v1.34.0`; spec `docs/bugs/2026-09-15-update-webhook-slack-token-echo.md`. Target now reads Keychain `k3d-manager-slack-bot-token-bot` (or exported env), keeps plist channel ID, backs up plist, and writes via plistlib with token in env; `scripts/tests/bin/makefile_webhook_slack.bats` and CHANGELOG Fixed added. No live Keychain/launchctl access. Operator acceptance = user runs `make update-webhook-slack`.
- **2026-09-15 — `/k3dm` LIVE-VERIFIED in Slack (user: "slack didn't even recoginze /k3dm command. that's the problem you have to fix")** — `/k3dm help` returns the admin target list. Root cause: workspace has TWO Slack apps; `/k3dm` had been saved in the old "k3dm" app (also holds `/acg-up|down|status`, `/argocd-upgrade`, `/cleanup-stale-sandbox`), whose signing secret differs → relay `verifySlack` 401 → Slack "app did not respond". Proven via `wrangler tail --format json` (`status:401`; `pretty` shows only `Ok`) vs a locally-signed probe with Keychain `k3dm-slack-signing-secret` → 200. Fix (user, Slack UI): created `/k3dm` in the "k3d-manager" app (owns `/cluster-status`), deleted from "k3dm" app. No Worker change. Follow-up DONE 2026-09-15: user recreated `/argocd-upgrade` + `/cleanup-stale-sandbox` in the "k3d-manager" app (13 commands). Relay-allowlisted but NOT registered in that app: `/hostinger-status`, `/hermes-auth`. User DELETED the old "k3dm" Slack app 2026-09-15 (its tokens are now revoked; webhook plist `SLACK_BOT_TOKEN` DID belong to it → `account_inactive`; replaced from Keychain `k3d-manager-slack-bot-token-bot` (bot `B0B8BC72WPK`, auth.test ok) via plistlib script (no argv/echo), plist backed up, webhook restarted pid 60707, :7443 unauth → 401). Note: `make update-webhook-slack` echoes the token (PlistBuddy line not `@`-prefixed) — avoid/fix.
- **2026-09-15 — Item "hub self-registered as `ubuntu-k3s-app-cluster`" (user: "work 2 first … with codex")** — NO Codex work needed: already fixed by `198584eb` (spec `docs/bugs/2026-09-13-hub-argocd-foreign-chart-ownership-blocks-helm-upgrade.md` FIXED, live rev 6). Read-only re-check 2026-09-15: no foreign ownership residue in cicd, dex SA gone, no `ubuntu-k3s-platform` app, in-cluster registration carries no `environment` label (v1.33.0 guard). Stale progress lines closed. Remaining v1.34.0: `/k3dm` user steps + release close-out.
- **2026-09-15 — `make restore-google-app-password` (user: "add that target for the purpose of restore password")** — spec `docs/bugs/2026-09-14-restore-google-app-password-target.md`. Reads Keychain `k3dm-alertmanager-gmail-app-password`, GETs Vault `secret/k3d-manager/alertmanager`, replaces only `gmail_app_pw` (fails if `gmail_from`/`sms_gateway` missing), POSTs via python urllib (token+pw in env, not argv), then `$(MAKE) observability`. `.PHONY`, help, CHANGELOG added. `make -n` OK; inline python compiles. **LIVE-VERIFIED 2026-09-15 (operator run):** "Vault updated", observability completed, `alertmanager-smtp-secret unchanged` (Keychain == Vault, so no-op rebuild as expected).
- **2026-09-14 — Scanner notify wording + SMS test (user: "fake test", "yes, please fix that")** — spec `docs/bugs/2026-09-14-argocd-cve-scan-misleading-upgrade-complete-notify.md`; fix `8cea94a5` (pushed; BATS 8/8, shellcheck clean): final block sends info notify + `Done: patched to X` only when ACG/OCI patched, else logs `Done: no cluster patched to X`. LIVE-VERIFIED: user go → `make platform-ops` rc 0, manual run logged `Done: no cluster patched to 10.9.1 (ACG=skipped infra=external OCI=skipped)`. SMS test: first fake `K3dmSmsTest` POST had no `startsAt` → Alertmanager set startsAt=endsAt (instant resolve, never notifies); re-posted 23:55:05Z with startsAt, endsAt 00:15:05Z; sms-critical route group_wait 5m → first send expected ~00:00Z — DID NOT SEND: by 00:14:50Z email notifications/requests/failed all 0, no Alertmanager log lines; alert active + routed to sms-critical (root has no group_by → single group {}, group_interval 2h). Unproven hypothesis: the aggregation group created by the broken first POST is waiting on group_interval. Proposed retest (needs user go): restart alertmanager pod, re-POST with startsAt, wait 5m group_wait. RETEST (user: "go for those items"): pod restarted 00:20:28Z; re-POSTed 00:20:58Z (active, receiver sms-critical); first send attempt 00:25:58Z (group_wait). CORRECTION: Claude first misreported this as success — watcher broke on `alertmanager_notifications_total{integration="email"}`=1, but that counts ATTEMPTS; failed counters only increment after final retry exhaustion. Logs show every attempt (21+) failing: `534 5.7.9 Application-specific password required` from smtp.gmail.com — the stored SMTP password is the normal Google password; account has 2FA. User confirmed no text and nothing in Gmail Sent. FIX (user): create Google App Password, rerun `make alertmanager-secret` in a real terminal; then retest (restart not needed). tmomail.net gateway delivery still unproven after auth is fixed. Lesson: verify alert delivery via Alertmanager logs (notify.go warn/error), never notifications_total alone. END-TO-END VERIFIED 2026-09-15: user set App Password (`make alertmanager-secret`), Claude ran `make observability` (Secret rebuilt from Vault — the make target only writes Vault), config reloaded 00:43:22Z, test alert K3dmSmsTest2 posted; email requests 28 total / 27 failed (1 success), zero `Notify attempt failed` after reload; USER RECEIVED THE TEXT (tmomail.net gateway works). User wants a Keychain backup of the App Password — user-run `security add-generic-password -U -a "$USER" -s k3dm-alertmanager-gmail-app-password -w` (interactive prompt; Claude must not handle the value). That bare `-w` form printed usage on this macOS; user used `read -rs` + `printf ... | security -i` instead. Keychain entry VERIFIED present (metadata only) in login keychain, svce `k3dm-alertmanager-gmail-app-password`, mdat 2026-09-15 00:47:38Z; First user-run compare: keychain≠vault (Claude's command stripped spaces; Vault holds the 19-char spaced form Gmail accepted), vault=alertmanager. User re-seeded Keychain from Vault via `security -i` (value double-quoted) → keychain vs vault MATCH (2026-09-15). All three copies (Keychain, Vault, Alertmanager Secret) consistent. Lesson: always set startsAt on synthetic alerts; a malformed POST can pin the single root group behind group_interval 2h.
- **2026-09-14 — Scanner redeploy (user: "execute first one")** — HOLD resolved with a guard first: `_patch_stage` holds major chart jumps (hostinger 7.8.1→10.9.1) with a warning notify unless `ARGOCD_ALLOW_MAJOR_UPGRADE=true`; spec `docs/bugs/2026-09-14-argocd-cve-scan-unguarded-major-chart-upgrade.md`; fix `96a4a326` (BATS 23/23, shellcheck clean). `make platform-ops` rc 0; live configmap has new script; first manual run FATAL `mkdir /.cache: permission denied` → `HOME=/tmp` fix `4ca63233`; rerun Complete: v3.5.2 scanned, 14 CVE lines, dev stage major upgrade 7.8.1→10.9.1 HELD, hostinger label 7.8.1. Scanner LIVE-VERIFIED. Follow-up (not filed): final notify says "upgrade complete" even when every stage held/skipped. User got no SMS: expected — only critical alerts are 4 Trivy (muted to null); no notify errors in Alertmanager logs; SMTP path still unproven end-to-end (test alert offered).
- **2026-09-14 — Alertmanager SMS LIVE-VERIFIED** — user re-ran `make alertmanager-secret` (real terminal); Claude ran `make observability` (rc 0). CR `configSecret=alertmanager-smtp-secret`; live `/api/v2/status` receivers null + sms-critical; routes: cicd/k3dm-analyze (continue) → Trivy→null → severity=critical→sms-critical; operator `missing to address` errors stopped after secret update 23:22:37Z. Remaining v1.34.0: scanner redeploy (HOLD, user decision), release close-out, `/k3dm` worker deploy + Slack registration (user).
- **2026-09-14 — `/k3dm <target>` Slack command (user: "work with codex to get that done")** — spec `docs/plans/v1.34.0-slack-k3dm-make-command.md` (1st plan doc of v1.34.0; kept on this branch, no v1.35.0 branch exists): server-side allowlist `scripts/lib/webhook/make_targets.py` (17 targets, reader/operator/admin+confirm), relay stamps `/k3dm` admin, webhook caps at caller's `K3DM_SLACK_ROLE_MAP` role, argv-only make, one job at a time. IMPLEMENTED `cfbdddde` (Codex wrote it; sandbox denied `.git`, Claude verified + committed): py 11+6 OK, node 17/17, webhook.bats 64/64, all 17 targets + vars exist in Makefile. Webhook RESTARTED (pid 41272, :7443 listening, unauth → 401). Worker deploy BLOCKED: `deploy-worker.yml` dispatch run 34907951609 failed — repo lacks `CLOUDFLARE_API_TOKEN` GH secret (main runs failing the same way since 2026-08-20); `make deploy-worker` needs Keychain reads (denied to Claude) → user runs `! make deploy-worker`. Then user adds `/k3dm` in Slack app (manifest entry in `docs/howto/slack-slash-commands.md`) and maps users in `K3DM_SLACK_ROLE_MAP` (unmapped = reader).
- **2026-09-14 — Alertmanager SMS fix `de83d01f` (user: "do 1 and 2")** — configSecret moved under alertmanagerSpec + useExistingSecret (hub+ACG values); template routes TrivyCriticalVulnerabilityDetected → null first; operator v0.79.2 falls back to default config when secret absent (hostinger has none), AlertmanagerConfig routes forced continue:true (CVE webhooks intact). User also asked which new make targets (e.g. e2e) belong in Slack — relay allowlist is `workers/slack-relay/index.js` ALLOWED_COMMANDS (14 cmds) + webhook `_ACTION_POLICY`/`_THREAD_COMMAND_MIN_ROLE`; recommendation given, no spec yet. FOLLOW-UP: Vault creds were empty (`!` no stdin) → operator rejects config; guards `5de8cdf8`; user must rerun `make alertmanager-secret` interactively.
- **2026-09-14 — v1.34.0 operator steps run by Claude (user: "can you execute those commands?")** — restart-webhook OK; observability fixed+rerun OK (`b5505368`); platform-ops OK but scanner did no scan → fixed `37118aab` (NOT yet redeployed); AppSets reapplied, all 12 on v1.34.0. HOLD: redeploying the fixed scanner may upgrade hostinger ArgoCD chart 7.8.1→10.9.1 via environment=dev label — needs user go. Alertmanager Vault secret missing (`make alertmanager-secret`, user). Next: release close-out (CHANGELOG version, README releases, PR gates).
- **2026-09-14 — v1.34.0 items 1–6 spec (user: "spec one to six then drive codex")** — ALL 9 CODE-COMPLETE + pushed (HEAD `6003a486`), each Claude-verified (diff = spec, shellcheck no new, BATS green): (1) acg lock state dir `2f2474e9`; (2) hub ESO in `make status` `3eeaef5b`; (3) declarative registration already DONE `b71e587a`; (4) BATS bare `!` negations `81048cea` — conversion exposed real defect: `_signing_restore_vault_from_keychain` passed key-file path to `cosign public-key --key` → now `env://COSIGN_KEY` (verified same pubkey, cosign v3.1.3); (5) per-set live APP_CLUSTER_NAME + CNI on AppSet reapply `397a7fcc` (`ARGOCD_APPSET_IGNORE_LIVE=1` to retarget); (6) app-cluster Prometheus PF 19090 → 19190+offset `31e69c56`; (7) hub ArgoCD ServiceMonitors + promtail ordering `42c8723d`; (9) argocd-cve-scan kubectl/wget bootstrap `6003a486`. Codex sandbox denied `.git/index.lock` on 2/3/4/6 → Claude committed. OPERATOR PENDING: `make restart-webhook` (2, 6); `make observability` (7/8); `make platform-ops` + `kubectl -n platform-ops create job --from=cronjob/argocd-cve-scan argocd-cve-scan-manual` (9); release close-out AppSet reapply via live-preserving `deploy_argocd_applicationsets` (hub + acg) → `argocd_check_values_branch`, confirm federate-acg/acg-prometheus show 19190.
- **v1.33.0 RELEASED** — PR #126 merged 2026-09-14T01:13:54Z (`4b6ce874`), tag v1.33.0 pushed, GitHub release published; enforce_admins restored (verified true); retrospective in `docs/retro/2026-09-13-v1.33.0-retrospective.md`. Branch k3d-manager-v1.34.0 created from merge commit. Pending (v1.34.0 operator work): ApplicationSet reapply entrypoint spec (do NOT blanket reapply — all sets already on v1.33.0; would re-name istio-ambient apps), Slack approval operator steps (KV, secrets, interactivity, launchd env, E2E), BATS negation follow-up (`! grep` mid-test never fails; v1.33.0-bugfix doc filed), signing-grant bug doc status check (docs/bugs/2026-09-13-signing-grant-eso-read-false-success.md live-verified hostinger), live hostinger grant restore (operator).
- **v1.33.0 post-merge verified by Claude (2026-09-13)** — release v1.33.0 is latest at `4b6ce874`; `k3d-manager-v1.34.0` tracks origin; SSH remote restored; ledger diff limited to 7 docs/memory-bank files. GitGuardian incident 37233897 resolved by the user. The Haiku-written retro had factual errors (it claimed "reapply is operator-only", contradicting CLAUDE.md, plus invented causes); corrected in `7a070d4b`.
- **2026-09-13 — two hub bugs spec'd + dispatched to Codex (user go)** — (A) `docs/bugs/2026-09-13-hub-argocd-foreign-chart-ownership-blocks-helm-upgrade.md`: in-cluster `ubuntu-k3s-app-cluster` is intentional (provider k3d; v1.33.0 guard drops `environment` label) — residue = 9 Helm-owned argocd-* cm/secrets with `argocd-controller` managedFields + `argocd.argoproj.io/instance: ubuntu-k3s-platform`, orphan SA `argocd-dex-server`; cm data matches rev 3. Fix = new `argocd_reclaim_release_ownership` (dry-run default; `--confirm` JSON-patch test+remove managedFields entry, verified via `--dry-run=server`). (B) `docs/bugs/2026-09-13-hub-federate-acg-self-scrape-duplicates-series.md`: the '1 1 1 1 1' replica stat is really `federate-acg` scraping `host.internal:19090` = hub Prometheus (launchd PF on k3d-k3d-cluster) → 81.5k series duplicated as `cluster="acg"`, duplicate KubeJobFailed/TargetDown alerts. Fix = `metric_relabel_configs` drop `cluster=acg` + `max()` replica panels. A = `198584eb` verified + pushed by Claude (Codex push failed: sandbox publickey). B = `22feff51` verified + pushed by Claude (first run blocked on spec gap → S2b `b4ae72e9`). Both code-complete. Live: A DONE (helm rev 6 deployed); B DONE (scoped observability AppSet reapplied by user; acg series 82208 → 6 synthetic scrape-health only; replica series 1). `observability-acg` also reapplied 2026-09-14 → `argocd_check_values_branch` rc=0 (all 6 refs on v1.34.0). Note: observability-acg envsubst needs `APP_CLUSTER_NAME=ubuntu-hostinger`.
- **2026-09-13 — hub observability stopgap + spec (user go for 3)** — LIVE: `helm upgrade argocd --reuse-values` created 4 ArgoCD ServiceMonitors (targets up, `count(argocd_app_info)`=37) but release rev 4 = FAILED (SSA label conflict; `argocd-controller` owns instance/version/chart labels on argocd-* cm/secrets because hub-registered `ubuntu-k3s-app-cluster` → kubernetes.default.svc once generated `ubuntu-k3s-platform` argo-cd-7.8.1 into hub cicd, 2026-09-13T13:45Z; secret now lacks `environment` label so inert). promtail applied on hub (4/4, Loki has namespaces). Spec `docs/bugs/2026-09-13-hub-rebuild-skips-argocd-servicemonitors-and-promtail.md` (S1 ensure SMs from live release values after CRD wait, S2 move promtail/dashboard/SM before Vault steps, S3 BATS). deploy_observability evidently never completed post-rebuild: PrometheusRule shopping-cart-apps, Istio VS, alertmanager-smtp-secret absent → operator `make observability`.
- **2026-09-13 — argocd-cve-scan daily 01:30 LA + empty-dashboard triage (user go for schedule)** — `cve-scan-cronjob.yaml` `30 1 * * *` + `timeZone: America/Los_Angeles` + `startingDeadlineSeconds: 3600`; applied LIVE on hub. Triage of 'No data' panels (read-only, NOT fixed): (1) k3dm Deployments + E2E + checkout-loadtest dashboards — no producer since the 2026-09-10 rebuild: hub has no Pushgateway (webhook pushes only for ACG provider), 0 `k3dm.k3d.io/e2e-result` ConfigMaps, no k6 run (and Prometheus remote-write receiver is off). (2) ArgoCD/Image Updater dashboard — `argocd_*` never scraped: ArgoCD Helm rev 3 installed 2026-09-10T23:57:36Z, ServiceMonitor CRD created 23:58:39Z, so the chart's Capabilities check skipped all 4 ServiceMonitors (values have enabled:true); fix = helm upgrade argocd with same values (needs go). Loki panels empty: promtail DaemonSet absent on hub (`scripts/etc/observability/promtail.yaml` never applied post-rebuild; fix = kubectl apply, needs go). Replica stat shows five '1's = kube-state-metrics pod IP churn from restarts (cosmetic; wrap in max()). App CVE Scan 0/0 = no run yet.
- **2026-09-13 — app-cve-scan daily + rebuild cooldown (user go)** — `40a91f0e` on k3d-manager-v1.34.0; spec `docs/bugs/2026-09-13-app-cve-scan-daily-schedule-and-rebuild-cooldown.md`. Schedule `0 1 * * *` with `timeZone: America/Los_Angeles`, `startingDeadlineSeconds: 3600`, and limits 1 CPU/1536Mi. Rebuild dispatch has a per-service cooldown `REBUILD_COOLDOWN_SECONDS=259200`, stored in cm `platform-ops/app-cve-scan-rebuild-state`. BATS 16/16, and the negative check fails as expected. LIVE on hub: CronJob plus `argocd-cve-scan-script` applied imperatively (hub-platform-ops only syncs the kubeconfig ExternalSecret, so no AppSet reapply). `cve-remediation-verify` un-suspended; it had been suspended 2026-09-11T11:24Z by `kubectl patch` with no recorded reason. The CVE dashboard's 'No data' was expected: 0 event ConfigMaps, and the scan had never run since the 2026-09-10 rebuild. First daily run 2026-09-14 01:00 PDT.
