...
- **2026-09-17 — Hermes findings visibility implemented (`60a04c19`, follow-up pending).** Hermes publishes a redacted status ConfigMap; the platform exporter emits Hermes sensor/check/incident metrics; and a dedicated `Hermes Status` Grafana dashboard shows current findings, poll age, incident state, and history. The feed also publishes regular sensors while optional `status_checks` is disabled. Design/acceptance record: `docs/issues/2026-09-17-hermes-grafana-visibility.md`.
