...
- **GCP provider pre-flight compute check** — ASSIGNED to Codex. Gemini confirmed ACG sandbox SA lacks all Compute IAM permissions; deploy_cluster fails deep in the flow with cryptic gcloud error. Fix: add `_gcp_preflight_check_compute` that probes `gcloud compute instances list --limit=1` and fails fast with actionable message (SA email from key JSON + role to grant). Spec: `docs/plans/v1.0.7-bugfix-gcp-provider-fixes.md` (Change 6). Only `scripts/lib/providers/k3s-gcp.sh` changes.
- **`_ensure_gcloud`** — ASSIGNED to Codex (`docs/plans/v1.0.7-ensure-gcloud.md`). Auto-install `gcloud` CLI when missing; replace raw `command -v gcloud` check in `_provider_k3s_gcp_deploy_cluster`; BATS tests. Live smoke test hit `ERROR: [k3s-gcp] gcloud CLI not found in PATH` — this is the final blocker before end-to-end runs cleanly.
