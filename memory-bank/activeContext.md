...
- **ACG credential extraction misses visible sandbox** — SOURCE FIX IN LIB-ACG PR #2 (2026-04-28, `https://github.com/wilddog64/lib-acg/pull/2`). Copilot review comments addressed and resolved; latest code commit `1ddbf7c` has CI green. Do not manually patch `scripts/lib/acg/`; it is a subtree. Merge is currently blocked only because PR #2 remains draft; connector ready-for-review mutation failed, shell GitHub token returned 401, and GitHub rejected merge with `Pull Request is still a draft`. Next k3d-manager step after lib-acg PR merges: pull subtree into `scripts/lib/acg/`, then rerun live `make up`. Spec: `docs/issues/2026-04-28-acg-credentials-cdp-context-miss.md`.
