...
- **Tab hijack + silent CDP failure** — OPEN (`docs/bugs/v1.1.0-bugfix-acg-credentials-newpage-and-cdp-log.md`). When CDP fails, no log emitted — user sees silent `/id` timeout. When CDP connects but no PS tab found, `allPages[0]` hijacks user's open tab. Fix: log CDP failure; use `context.newPage()` instead of `allPages[0]`.
