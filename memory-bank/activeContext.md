...
- **OPEN Bug 15 — Argo CD port-forward flaps after sandbox rebuild** — `localhost:8080` can remain bound while the backend pod sandbox is lost after rebuild, so Safari still cannot reach the login page consistently. Spec: `docs/issues/2026-05-11-acg-up-argocd-port-forward-flapping.md`.
