...
**v0.9.16 IN PROGRESS** — spec `docs/plans/v0.9.16-antigravity-plugin-mcp-refactor.md` implemented in commit `45168cf`: antigravity plugin now uses Antigravity IDE + Playwright MCP over CDP port 9222 (`_ensure_antigravity_ide`, `_ensure_antigravity_mcp_playwright`, `_antigravity_browser_ready`). Follow-up spec `docs/plans/v0.9.16-antigravity-launch-session.md` shipped in commit `e83d89d`, adding `_antigravity_launch` + `_antigravity_ensure_github_session` so the plugin auto-starts Antigravity and confirms GitHub auth.
