...
- **deploy_argocd not loaded + invalid flag** — SPECCED → Codex (`docs/bugs/v1.1.0-bugfix-gcp-deploy-argocd-not-loaded.md`). `argocd.sh` not sourced when `gcp_provision_stack` runs → "command not found" at step 6/7. Also `--skip-ldap` is not a valid `deploy_argocd` flag. Fix: source `argocd.sh` inside the function; remove `--skip-ldap`.
