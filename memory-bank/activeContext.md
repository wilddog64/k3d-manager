...
- **2026-09-16 — User-session Keychain ACL blocker identified.** In the user's terminal, both token reads are zero-length and `security add-generic-password` returns `User interaction is not allowed`, while the launchd webhook remains running and unauthenticated. Recovery requires unlocking/authorizing the login Keychain interactively before local or Cloudflare token rotation; no plaintext fallback should be added.
