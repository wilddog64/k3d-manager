...
- **Dead `_argocd_configure_post_deploy` removal is DONE in `ac729e14` on `origin/k3d-manager-v1.17.0` (2026-07-23)** exactly per `docs/bugs/2026-07-23-argocd-remove-dead-configure-post-deploy.md`: scope held to exactly one file `scripts/plugins/argocd.sh`, deleting only the orphaned wrapper function plus its leading blank line; `_argocd_configure_vault_eso`, `_argocd_seed_vault_admin_secret`, `_argocd_deploy_image_updater`, `deploy_argocd_bootstrap`, `ARGOCD_VIRTUALSERVICE_HOST`, and all template files were left untouched. Gates recorded on this machine: `grep -c '_argocd_configure_post_deploy' scripts/plugins/argocd.sh` → `0`; `bash -n scripts/plugins/argocd.sh` clean; `bats scripts/tests/plugins/argocd_servicemonitors_ensure.bats` → `1..3` / `ok 1` / `ok 2` / `ok 3`; `./scripts/k3d-manager _agent_audit` printed `running under bash version 5.3.15(1)-release`; `git diff --cached --stat` and commit `git show --stat ac729e14` each showed exactly one file changed. `shellcheck -S warning scripts/plugins/argocd.sh` still reports the pre-existing baseline warning `SC2034` at line 425 (`local enable_vault=1` appears unused); `git show HEAD:scripts/plugins/argocd.sh | shellcheck -S warning -` was clean, confirming zero NEW warnings from this spec. PR URL not created per repo rule. The newly-orphaned cascade remains OUT OF SCOPE and still needs its own owner decision if removal is ever desired.
