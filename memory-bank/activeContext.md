...
- **Spec written + committed `d29c84c9`** → `docs/bugs/2026-07-24-trivy-version-pins-not-explicit-everywhere.md`. Bumps to the newest matched set — chart `0.34.0` / operator `0.32.0` / scanner `0.72.0` — across all four config files (hub + ACG variants), and adds BATS coverage for the ACG values file, which had **none** (the existing tag assertion only covered the hub file).
