...
- **Fix CDP context reuse + process.exit/finally** — COMPLETE (`13194e57`) | `docs/bugs/v1.1.0-bugfix-acg-credentials-cdp-and-singleton.md`; `scripts/playwright/acg_credentials.js` now always reuses the first CDP context when Chrome is running and throws from the inner catch so `finally` closes the browser cleanly
