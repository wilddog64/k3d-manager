...
- **GCP full stack bugfix** — COMPLETE (`7360d96e`). `docs/bugs/v1.1.0-bugfix-gcp-provision-stack-ssm-vault.md`; Makefile `provision` now runs `ssm` only for AWS, and `scripts/plugins/vault.sh` handles optional `$1` so `deploy_vault` no longer fails under `set -u`.
