...
- **shopping-cart ImagePullBackOff — no ghcr-pull-secret** — OPEN. All 5 shopping-cart pods fail with ImagePullBackOff on ubuntu-k3s. Root cause: (1) `acg-up` Step 5 exits when `GHCR_PAT` env var unset — no Vault fallback; (2) `services/` kustomize overlays have no `imagePullSecrets` patch. Fix: read PAT from Vault `secret/github/ghcr-pull-token` in Step 5 + add kustomize patch to all 5 services. One-time manual step: `vault kv put secret/github/ghcr-pull-token token=<PAT>`. Spec: `docs/bugs/2026-04-26-shopping-cart-imagepullbackoff-no-ghcr-pull-secret.md`.
