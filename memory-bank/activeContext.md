...
- **COMPLETE:** `fix(acg-up)` — browser auto-open is now deferred until after Step 10f SSO wiring, so Safari is not launched before Keycloak and `/etc/hosts` are ready. Commit `044aae1f`. Issue doc: `docs/issues/2026-05-12-acg-up-browser-auto-open-before-keycloak-ready.md`.
