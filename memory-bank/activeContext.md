...
- **GCP stale auth `invalid_grant` on sandbox rotation** — OPEN (`docs/bugs/v1.1.0-bugfix-gcp-stale-auth-invalid-grant.md`). When ACG sandbox rotates, deleted `cloud_user` causes `invalid_grant` in `gcloud compute instances describe`. Fix: (1) revoke stale accounts before re-extraction in `_gcp_load_credentials`; (2) guard `_provider_k3s_gcp_status` gcloud call with active-account check. Assigned to Codex.
