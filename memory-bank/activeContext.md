...
- **deploy_argocd not loaded + invalid flag** — COMPLETE (`7e65364e`). Spec `docs/bugs/v1.1.0-bugfix-gcp-deploy-argocd-not-loaded.md`. `gcp_provision_stack` now sources `argocd.sh` before calling `deploy_argocd` and drops the invalid `--skip-ldap` flag; `shellcheck scripts/plugins/gcp.sh` + `bats scripts/tests/providers/k3s_gcp.bats` pass.
- **ArgoCD rendered unbound EXIT trap bugfix** — ASSIGNED → Codex (`docs/bugs/v1.1.0-bugfix-argocd-rendered-unbound-exit-trap.md`). `_argocd_deploy_appproject` sets an EXIT trap referencing local `$rendered`; after function returns, trap fires with unbound variable under `set -u`. Fix: explicit `rm -f "$rendered"` + `trap - EXIT` on success path; remove RETURN trap.
