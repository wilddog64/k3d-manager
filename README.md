...
| 2026-07-24 | [Copilot PR #107 Review Findings](docs/issues/2026-07-24-copilot-pr107-review-findings.md) | v1.17.0: `base64 -d`→`--decode` portability across all 7 sites in `keycloak.sh` — Copilot's premise (`-d` is GNU-only) and remedy (`--decode \|\| -D`) were both wrong: `-d` works on current macOS, `-D` is invalid under GNU, and the fallback arm is unsound in a pipeline; `--decode` alone is accepted by both. Plus the merge-blocking GitGuardian check — `.gitguardian.yaml` silences the local CLI only; server-side incidents need separate API/dashboard resolution (4 FPs cleared) |
