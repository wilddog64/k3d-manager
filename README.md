...
|---|---|---|
| 2026-08-10 | [Hostinger rollout deadlock — maxSurge on 2-CPU node](docs/issues/2026-08-10-hostinger-rollout-deadlock-maxsurge-on-2cpu-node.md) | Live-ops (→ v1.25.0-G): `order`/`basket` were wedged mid-rollout for 11h/26h — `replicas=1, maxSurge=1, maxUnavailable=0` needs 2× CPU transiently, but the 2-CPU hostinger node sat at 1910m/2000m (95%) so surge pods `FailedScheduling: Insufficient cpu` forever while the old ReplicaSet kept serving (`1/1` masks it); capacity silently blocks CVE convergence for any surge-strategy rollout. Stopgap: live-patched both to `maxSurge=0/maxUnavailable=1` (delete-old-then-create) → converged. Durable fix (v1.25.0-G): commit `maxSurge=0` into the app git manifests (ArgoCD may selfHeal the live patch away) or bump node CPU |
| 2026-08-09 | [Copilot PR #112 Review Findings](docs/issues/2026-08-09-copilot-pr112-review-findings.md) | v1.23.0: moved the Vault root token off `curl` argv in `make show-service-passwords` (Grafana + Prometheus lookups) to the repo's temp-header-file pattern (`-H "@<file>"`); added a writable `/tmp` `emptyDir` to the `vulnerability-inventory-exporter` so `readOnlyRootFilesystem: true` doesn't break cert-based app-cluster scraping (`tempfile` cert/key path); and quoted `mount_path` with `printf %q` before it crosses `_vault_exec`'s `sh -lc` boundary in `_vault_configure_secret_writer_role` (shell-injection hardening) |
| 2026-08-03 | [Copilot PR #110 Review Findings](docs/issues/2026-08-03-copilot-pr110-review-findings.md) | v1.21.0: fixed `base64 -d`→`--decode` in `make show-service-passwords` (repo portability convention) and dropped the vestigial `python3 -m pytest` gate from the webhook-hardening spec (repo has no pytest tests; an empty collection misleadingly passes — Python coverage runs via BATS `SourceFileLoader`); declined the "real `kubectl get pods` is flaky" finding — `kubectl` is already PATH-stubbed in `setup_file` (`_BATS_STUB_BIN`, default `exit 0`) and inherited through `k3dm-ask-bash`'s `exec /bin/bash`, so the sandbox-allows-read test is deterministic |
| 2026-08-03 | [Stale-ref fixture orphaned by branch cleanup](docs/issues/2026-08-03-stale-test-refs-fixture-orphaned-by-branch-cleanup.md) | v1.21.0 (PR #110): `stale_test_refs.bats` hard-codes historical SHAs (`f03df202`/`4c89dabb`/`e3a75f1f`) that the v1.20.0 branch-cleanup orphaned — they survive as loose objects locally (tests pass) but a fresh CI clone can't reach them, so `check-stale-test-refs.sh` sees `fatal: bad revision`, its `\|\| true` swallows it, and it exits 0 instead of the expected 1; guarded each fixture with a `_require_commit` reachability `skip`; self-contained-fixture rewrite deferred |
| 2026-08-01 | [Copilot PR #109 Review Findings](docs/issues/2026-08-01-copilot-pr109-review-findings.md) | v1.20.0: a wide single-line `.PHONY` edit in `0a316fb6` (adding the `platform-ops` target) clobbered the tail of the list — dropped `uninstall-alertmanager-port-forward` + `install-alertmanager-auth-proxy` and duplicated `uninstall-alertmanager-auth-proxy`, all defined targets; restored so each `.PHONY` entry appears once (`a711bae7`); pre-existing virtual `vuln-scan`/`trivy-scan-report` left out of scope |
| 2026-07-28 | [Copilot Dependabot auto-merge Review Findings](docs/issues/2026-07-28-copilot-dependabot-automerge-review-findings.md) | v1.19.0 (5 shopping-cart PRs): `alert-state` was dead code (needed `alert-lookup: true` + `security-events: read`) so security-update auto-merge never fired — wired it up (maintainer decision: security updates auto-merge at any semver, non-security majors reviewed); `github.actor` gate → `github.event.pull_request.user.login` (robust on reopen); top-level `permissions: {}` + job-level least privilege + `pull_request_target` scoped to `main`; declined a pre-existing v1.18.0 CHANGELOG grouping-wording nit |
