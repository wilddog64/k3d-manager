...
| 2026-07-23 | [Copilot PR #106 Review Findings](docs/issues/2026-07-23-copilot-pr106-review-findings.md) | v1.16.0: `set -e`-safe Cilium wait loop + self-uninstalling `RETURN` traps (6 sites) + portable `xargs` in the stale-ref checker; `GH_TOKEN` out of `wget` argv via mode-600 `--config`; 3 pure-logic plugin BATS suites + `app_cve_scan.bats` added to CI (lint job now `fetch-depth: 0` for the stale-ref check); SSH-string→array in `_ambient_install_cilium` deferred (provider-internal inputs, untested cold-provisioning code) |
