...
| 2026-07-28 | [Copilot PR #108 Review Findings](docs/issues/2026-07-28-copilot-pr108-review-findings.md) | v1.18.0: fixed a `PrometheusRule` alert `description` placeholder (`vulnerabilit(y/ies)`→`vulnerabilities`) that would render verbatim in notifications, and a redundant identical-operand `\|\|` in `webhook.bats`; declined the `make show-service-passwords` dev-credential print (dev-only helper surfacing fixed seed creds; the sensitive admin password is fetched dynamically from the Secret) |
